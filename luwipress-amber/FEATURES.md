# LuwiPress Amber — Theme Feature Overview

**Version:** 1.6.7 · **License:** GPLv2+ · **Target:** WooCommerce stores using LuwiPress

LuwiPress Amber is the first **ecosystem-integrated** WordPress theme: built specifically to surface the LuwiPress AI plugin's intelligence on the storefront. Where most themes stop at layout and typography, Gold turns AI Knowledge Graph signals, customer chat, plugin-detection, and live search suggestions into first-class store features the visitor actually feels.
...
- **Master profile** — artisan / maker bio template (used for `_lwp_amber_maker` field)
...
All widgets live under `Elementor → LuwiPress Amber` and inherit theme tokens automatically.
...
`Görünüm → Özelleştir → LuwiPress Amber` exposes:
...
`Görünüm → LuwiPress Amber` opens a single-page admin dashboard summarising the active integration:
...
Operator opts in via Customizer → LuwiPress Amber → Performance. Reversible by flipping the toggle off.
...
- **Product card override** — Gold's `content-product.php` template replaces WC's default. Eyebrow category, italic maker line (pulled from `pa_master` / `pa_maker` taxonomy or `_lwp_amber_maker` post-meta), Playfair name, gold serif price ladder, sale percentage badge top-left. Entire card is one anchor → product page (no inline buttons fighting for tap-target).
...
12 `apply_filters('luwipress_amber_*')` hooks expose every editorial content array for child-theme overrides — operators don't need to fork the templates to localise text.
...
Operator can disable globally via Customizer → LuwiPress Amber → Animation → Page loader overlay.
...
`Görünüm → LuwiPress Amber → Wizard` (auto-launches on first activation):
...
All tokens override-able from Customizer → LuwiPress Amber → Brand.
