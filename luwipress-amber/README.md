...
**The warm, cinematic member of the LuwiPress theme family** — an AI-powered,
Elementor-ready WordPress theme for premium **travel & experiences** brands
(sister to Gold = editorial luxury, Emerald = B2B consulting). "Golden Hour,
Endless Horizon": a warm charcoal-plum base, an amber→coral sunset gradient
reserved for CTAs and the brand mark, Fraunces + Inter typography, a full
parallel **light mode** (persisted in localStorage), a sticky header with mega
nav, an off-canvas drawer, cinematic "scene" backdrops, and a floating WhatsApp
launcher. Tours are sold as WooCommerce products; full checkout + My Account
suite included.
...
1. Upload the theme: WP Admin → Appearance → Themes → Add New → Upload Theme
   → upload the `luwipress-amber` folder as a `.zip` → **Activate**.
...
3. **Import the Kit**:
   ...
   - Choose `wp-content/themes/luwipress-amber/elementor-kit/kit.json` → Import
...
- **Cart bump**: triggers a `.42 s` scale-bump on any element with
  `[data-lwp-cart-toggle]`, `.luwipress-amber-cart`, `.menu-cart-trigger`, or
...
| Filter | What it does |
|---|---|
| `luwipress_amber_kit_path` | Override path to the bundled Kit folder |
| `luwipress_amber_kit_files()` | PHP helper returning all bundled JSON paths |
...
`Appearance → LuwiPress Amber` shows a single panel that reads from LuwiPress's
