...
add_action( 'wp_enqueue_scripts', function () {
	if ( ! wp_style_is( 'luwipress-amber-tokens', 'enqueued' ) ) return;
	wp_add_inline_style( 'luwipress-amber-tokens', luwipress_amber_inline_token_overrides() );
}, 30 );

// Also inject into the block editor preview.
add_action( 'enqueue_block_editor_assets', function () {
	if ( ! wp_style_is( 'luwipress-amber-editor-tokens', 'enqueued' ) ) return;
	wp_add_inline_style( 'luwipress-amber-editor-tokens', luwipress_amber_inline_token_overrides() );
} );

/**
 * Compose the :root override block.
 */
function luwipress_amber_inline_token_overrides() {
	$mods = [
		'primary'       => get_theme_mod( 'luwipress_amber_color_primary' ),
		'primary-light' => get_theme_mod( 'luwipress_amber_color_primary_light' ),
		'accent'        => get_theme_mod( 'luwipress_amber_color_accent' ),
		'sale'          => get_theme_mod( 'luwipress_amber_color_sale' ),
		'icon-red'      => get_theme_mod( 'luwipress_amber_color_icon_red' ),
		'ink'           => get_theme_mod( 'luwipress_amber_color_ink' ),
		'bg'            => get_theme_mod( 'luwipress_amber_color_bg' ),
		'black'         => get_theme_mod( 'luwipress_amber_color_black' ),
	];
