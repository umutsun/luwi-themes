 ...
 *
 *   - LuwiPress_Amber_Canonical_Audit_Tool          rel=canonical present + self-pointing + no chain
 *   - LuwiPress_Amber_Hreflang_Reciprocity_Tool     hreflang completeness + reciprocal references
 *   - LuwiPress_Amber_Redirect_Chain_Detector_Tool  301 → 301 chains + final-status check
 *   - LuwiPress_Amber_Sitemap_Indexation_Parity_Tool  sitemap entries that are 301/404/noindex
 *   - LuwiPress_Amber_SEO_Triangle_Health_Tool      consolidated SEO pillars audit
 *
...
if ( ! class_exists( 'LuwiPress_Amber_SEO_HTTP_Probe' ) ) :
class LuwiPress_Amber_SEO_HTTP_Probe {

	const CACHE_TTL = 600; // 10 minutes
	const CACHE_KEY_PREFIX = 'lwp_amber_seo_probe_';
...
class LuwiPress_Amber_Canonical_Audit_Tool {

	public static function scan( $args = array(), $tool = array() ) {
		$urls = LuwiPress_Amber_SEO_HTTP_Probe::representative_sample();
		$candidates = array();

		foreach ( $urls as $url ) {
			$res = LuwiPress_Amber_SEO_HTTP_Probe::fetch_html( $url );
			if ( $res['status'] !== 200 ) {
			...
			$trace = LuwiPress_Amber_SEO_HTTP_Probe::trace( $canonical );
...
class LuwiPress_Amber_Hreflang_Reciprocity_Tool {
	...
	public static function scan( $args = array(), $tool = array() ) {
		...
		foreach ( $urls as $url ) {
			...
			foreach ( $tags as $lang => $alt ) {
				...
				if ( $alt === $url ) { continue; }
				$alt_res = LuwiPress_Amber_SEO_HTTP_Probe::fetch_html( $alt );
...
class LuwiPress_Amber_Redirect_Chain_Detector_Tool {
	...
	public static function scan( $args = array(), $tool = array() ) {
		...
		foreach ( $urls as $url ) {
			$trace = LuwiPress_Amber_SEO_HTTP_Probe::trace( $url );
			if ( $trace['hops'] === 0 ) {
				continue; // direct hit
			}
...
class LuwiPress_Amber_Sitemap_Indexation_Parity_Tool {
	...
	public static function scan( $args = array(), $tool = array() ) {
		...
		$slug_map = (array) get_transient( defined( 'LUWIPRESS_AMBER_SLUG_CONFLICT_TRANSIENT' ) ? LUWIPRESS_AMBER_SLUG_CONFLICT_TRANSIENT : 'luwipress_amber_slug_conflicts_v1' );
		...
		return array(
			...
			'meta'       => array(
				...
				'note'             => 'Findings: 301 source in sitemap, redirect chains, or 4xx/5xx pages still listed. Resolve at the SEO plugin level (exclude post types, regen sitemap) — luwipress-amber doesn\'t generate sitemaps itself.',
			),
		);
	}
...
class LuwiPress_Amber_SEO_Triangle_Health_Tool {
	...
	public static function scan( $args = array(), $tool = array() ) {
		...
		$wiring = array(
			'canonical'           => array( 'LuwiPress_Amber_Canonical_Audit_Tool', 'canonical_audit',           'Canonical health' ),
			'hreflang'            => array( 'LuwiPress_Amber_Hreflang_Reciprocity_Tool', 'hreflang_reciprocity_audit','Hreflang reciprocity' ),
			'redirect_chains'     => array( 'LuwiPress_Amber_Redirect_Chain_Detector_Tool', 'redirect_chain_detector', 'Redirect chains' ),
			'sitemap'             => array( 'LuwiPress_Amber_Sitemap_Indexation_Parity_Tool', 'sitemap_indexation_parity','Sitemap parity' ),
			'internal_links'      => array( 'LuwiPress_Amber_Broken_Internal_Links_Tool', 'broken_internal_links',     'Broken internal links' ),
			'slug_conflict'       => array( 'LuwiPress_Amber_Slug_Conflict_Audit_Tool', 'slug_conflict_audit',         'Slug-conflict redirect map' ),
		);
