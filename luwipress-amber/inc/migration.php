...
if ( ! class_exists( 'LuwiPress_Amber_Migration' ) ) :

class LuwiPress_Amber_Migration {

	const PAGE_SLUG       = 'luwipress-amber-migration';
	const ARCHIVE_OPTION  = 'luwipress_amber_migration_archive';
	const NONCE_ACTION    = 'luwipress_amber_migration';
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_luwipress_amber_migration_scan',             [ $this, 'ajax_scan' ] );
		add_action( 'wp_ajax_luwipress_amber_migration_archive',          [ $this, 'ajax_archive' ] );
		add_action( 'wp_ajax_luwipress_amber_migration_swap',             [ $this, 'ajax_swap' ] );
		add_action( 'wp_ajax_luwipress_amber_migration_restore',          [ $this, 'ajax_restore' ] );
	}

	/**
	 * Auto-detected canonical reserves: any page whose slug ends in
	 * `-gold` is treated as our shadow for the base slug. Filter
	 * `luwipress_amber_canonical_reserves` to add bespoke pairs (e.g. when
	...
	public function get_canonical_reserves() {
		 ...
		 */
		return apply_filters( 'luwipress_amber_canonical_reserves', $pairs );
	}

	public function register_admin_page() {
		add_theme_page(
			__( 'LuwiPress Migration', 'luwipress-amber' ),
			__( 'LuwiPress Migration', 'luwipress-amber' ),
			self::CAPABILITY,
			self::PAGE_SLUG,
	...
	public function enqueue_assets( $hook ) {
		...
		wp_enqueue_style(
			'luwipress-amber-ecosystem',
			LUWIPRESS_AMBER_URI . '/assets/css/ecosystem-dashboard.css',
			[],
	...
	public function archive_pages( array $page_ids, $suffix = self::DEFAULT_SUFFIX ) {
		...
		foreach ( $page_ids as $page_id ) {
			...
			update_post_meta( $page_id, '_luwipress_amber_archived_at', time() );
			update_post_meta( $page_id, '_luwipress_amber_original_slug', $old_slug );
	...
	public function swap_canonical_pairs( array $customer_page_ids, $suffix = self::DEFAULT_SUFFIX ) {
		...
		foreach ( $customer_page_ids as $cust_id ) {
			...
			update_post_meta( $cust_id, '_luwipress_amber_archived_at', time() );
			update_post_meta( $cust_id, '_luwipress_amber_original_slug', $canonical );
			update_post_meta( $shadow_id, '_luwipress_amber_promoted_at', time() );
			update_post_meta( $shadow_id, '_luwipress_amber_shadow_slug', $pair['shadow_slug'] );
	...
	public function restore_page( $page_id ) {
		...
		if ( ! $found ) {
			return new WP_Error( 'not-archived', __( 'Page is not in the archive registry.', 'luwipress-amber' ) );
		}

		$page = get_post( $page_id );
		if ( ! $page ) {
			return new WP_Error( 'not-found', __( 'Page no longer exists.', 'luwipress-amber' ) );
		}
		...
		if ( $is_swap ) {
			...
			delete_post_meta( $shadow_id, '_luwipress_amber_promoted_at' );
			delete_post_meta( $shadow_id, '_luwipress_amber_shadow_slug' );
		} else {
			...
			if ( $blocker ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Original slug "%1$s" is currently held by another page (#%2$d "%3$s"). Move that one out of the way first.', 'luwipress-amber' ),
						$target,
						(int) $blocker->ID,
						$blocker->post_title
					)
		...
		delete_post_meta( $page_id, '_luwipress_amber_archived_at' );
		delete_post_meta( $page_id, '_luwipress_amber_original_slug' );
	...
	public function ajax_scan() {
		...
		if ( ! current_user_can( self::CAPABILITY ) ) {
			wp_send_json_error( [ 'message' => __( 'Forbidden', 'luwipress-amber' ) ], 403 );
		}
	...
	public function ajax_archive() {
		...
		if ( empty( $ids ) ) {
			wp_send_json_error( [ 'message' => __( 'No pages selected.', 'luwipress-amber' ) ] );
		}
	...
	public function ajax_swap() {
		...
		$suffix = isset( $_POST['suffix'] ) ? sanitize_title( wp_unslash( $_POST['suffix'] ) ) : self::DEFAULT_SUFFIX;
		if ( empty( $ids ) ) {
			wp_send_json_error( [ 'message' => __( 'No pairs selected.', 'luwipress-amber' ) ] );
		}
	...
	public function ajax_restore() {
		...
		if ( ! $id ) {
			wp_send_json_error( [ 'message' => __( 'Missing page id.', 'luwipress-amber' ) ] );
		}
	...
	public function render_admin_page() {
		if ( ! current_user_can( self::CAPABILITY ) ) {
			wp_die( esc_html__( 'You do not have permission to view this page.', 'luwipress-amber' ) );
		}
		...
		?>
		<div class="wrap lwp-amber-eco">
			<h1><?php esc_html_e( 'LuwiPress Migration Archive', 'luwipress-amber' ); ?></h1>
			<p class="lwp-amber-eco__lead"><?php esc_html_e(
			...
			<section class="lwp-amber-eco__card" style="margin-top:24px;padding:20px 24px;background:#fff;border:1px solid #e8e2d3;border-radius:8px;">
				<h2 style="margin:0 0 8px;font-size:18px;"><?php esc_html_e( 'Detected slug collisions', 'luwipress-amber' ); ?></h2>
				...
				<?php if ( empty( $collisions ) ) : ?>
					<p style="padding:18px;background:#f6f3f2;border-radius:6px;color:#4d4635;">
						<?php esc_html_e( '✓ No collisions found. Your slug structure is already canonical.', 'luwipress-amber' ); ?>
					</p>
				<?php else : ?>
					<form id="lwp-amber-migration-form">
						<table class="widefat striped" style="margin-top:8px;">
							<thead>
								<tr>
									<th style="width:36px;"><input type="checkbox" id="lwp-amber-migration-all" /></th>
									<th><?php esc_html_e( 'Page', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'Slug', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'Lang', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'Will become', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'Term · products', 'luwipress-amber' ); ?></th>
							...
							<tbody>
								<?php foreach ( $collisions as $c ) : ?>
									<tr>
										<td><input type="checkbox" class="lwp-amber-migration-row" value="<?php echo (int) $c['page_id']; ?>" /></td>
										<td>
											<a href="<?php echo esc_url( get_edit_post_link( $c['page_id'] ) ); ?>"><?php echo esc_html( $c['title'] ); ?></a>
											<br><small style="color:#7f7663;">#<?php echo (int) $c['page_id']; ?> · <a href="<?php echo esc_url( get_permalink( $c['page_id'] ) ); ?>" target="_blank" rel="noopener"><?php esc_html_e( 'view', 'luwipress-amber' ); ?></a></small>
										</td>
						...
						<div style="margin-top:16px;display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
							<label>
								<?php esc_html_e( 'Suffix:', 'luwipress-amber' ); ?>
								<input type="text" id="lwp-amber-migration-suffix" value="legacy" style="width:120px;margin-left:6px;" />
							</label>
							<button type="button" class="button button-primary" id="lwp-amber-migration-execute">
								<?php esc_html_e( 'Archive selected', 'luwipress-amber' ); ?>
							</button>
							<span id="lwp-amber-migration-status" style="color:#4d4635;"></span>
						</div>
					</form>
				<?php endif; ?>
			</section>
			...
			<section class="lwp-amber-eco__card" style="margin-top:24px;padding:20px 24px;background:#fff;border:1px solid #e8e2d3;border-radius:8px;">
				<h2 style="margin:0 0 8px;font-size:18px;"><?php esc_html_e( 'Canonical reserves', 'luwipress-amber' ); ?> <small style="font-weight:400;color:#7f7663;">— <?php esc_html_e( 'Type B · shadow-page swap', 'luwipress-amber' ); ?></small></h2>
				<p style="margin:0 0 16px;color:#4d4635;"><?php esc_html_e( 'These slugs (about, contact, journal, masters, …) belong to the theme’s canonical structure and the theme has shipped a *-gold shadow page for each. Run the swap to archive the customer’s legacy page and promote our shadow into the canonical slot in one atomic move.', 'luwipress-amber' ); ?></p>

				<?php if ( empty( $swaps ) ) : ?>
					<p style="padding:18px;background:#f6f3f2;border-radius:6px;color:#4d4635;">
						<?php esc_html_e( '✓ No canonical-reserve swaps pending. Either no shadow pages exist, or the canonical slugs are already ours.', 'luwipress-amber' ); ?>
					</p>
				<?php else : ?>
					<form id="lwp-amber-swap-form">
						<table class="widefat striped" style="margin-top:8px;">
							<thead>
								<tr>
									<th style="width:36px;"><input type="checkbox" id="lwp-amber-swap-all" /></th>
									<th><?php esc_html_e( 'Customer page', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'Holds slug', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'Lang', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'Our shadow', 'luwipress-amber' ); ?></th>
									<th><?php esc_html_e( 'After swap', 'luwipress-amber' ); ?></th>
								</tr>
							</thead>
							<tbody>
								<?php foreach ( $swaps as $s ) : ?>
									<tr>
										<td><input type="checkbox" class="lwp-amber-swap-row" value="<?php echo (int) $s['customer_page_id']; ?>" /></td>
										<td>
						...
						<div style="margin-top:16px;display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
							<button type="button" class="button button-primary" id="lwp-amber-swap-execute">
								<?php esc_html_e( 'Swap selected pairs', 'luwipress-amber' ); ?>
							</button>
							<span id="lwp-amber-swap-status" style="color:#4d4635;"></span>
						</div>
					</form>
				<?php endif; ?>
			</section>
			...
			<section class="lwp-amber-eco__card" style="margin-top:24px;padding:20px 24px;background:#fff;border:1px solid #e8e2d3;border-radius:8px;">
				<h2 style="margin:0 0 8px;font-size:18px;"><?php esc_html_e( 'Archive browser', 'luwipress-amber' ); ?></h2>
				<p style="margin:0 0 16px;color:#4d4635;"><?php esc_html_e( 'Pages you’ve archived. Each one is still published at its <slug>-legacy URL — pull content from it any time, or restore the original slug with one click.', 'luwipress-amber' ); ?></p>

				<?php if ( empty( $registry ) ) : ?>
					<p style="padding:18px;background:#f6f3f2;border-radius:6px;color:#4d4635;">
						<?php esc_html_e( 'No pages archived yet.', 'luwipress-amber' ); ?>
					</p>
				<?php else : ?>
					<table class="widefat striped">
						<thead>
							<tr>
								<th><?php esc_html_e( 'Page', 'luwipress-amber' ); ?></th>
								<th><?php esc_html_e( 'Type', 'luwipress-amber' ); ?></th>
								<th><?php esc_html_e( 'Original slug', 'luwipress-amber' ); ?></th>
								<th><?php esc_html_e( 'Current slug', 'luwipress-amber' ); ?></th>
								<th><?php esc_html_e( 'Archived', 'luwipress-amber' ); ?></th>
								<th style="width:180px;"><?php esc_html_e( 'Actions', 'luwipress-amber' ); ?></th>
							</tr>
						</thead>
						<tbody>
							<?php foreach ( $registry as $row ) :
								...
								$type_label = ( 'canonical_swap' === $reason ) ? __( 'Type B · swap', 'luwipress-amber' ) : __( 'Type A · WC archive', 'luwipress-amber' );
								?>
								<tr data-pid="<?php echo $pid; ?>">
									<td>
										...
										if ( 'canonical_swap' === $reason && ! empty( $row['shadow_page_id'] ) ) {
											...
											printf( ' · ' . esc_html__( 'paired with #%d', 'luwipress-amber' ), (int) $row['shadow_page_id'] );
										}
										?></small>
									</td>
									...
									<td>
										<a class="button button-small" href="<?php echo $page ? esc_url( get_permalink( $pid ) ) : '#'; ?>" target="_blank" rel="noopener"><?php esc_html_e( 'View', 'luwipress-amber' ); ?></a>
										<button type="button" class="button button-small lwp-amber-migration-restore" data-id="<?php echo $pid; ?>"><?php esc_html_e( 'Restore', 'luwipress-amber' ); ?></button>
									</td>
								</tr>
							<?php endforeach; ?>
						</tbody>
		...
		(function () {
			...
			var all = document.getElementById('lwp-amber-migration-all');
			if (all) {
				all.addEventListener('change', function () {
					document.querySelectorAll('.lwp-amber-migration-row').forEach(function (cb) { cb.checked = all.checked; });
				});
			}

			var execBtn = document.getElementById('lwp-amber-migration-execute');
			if (execBtn) {
				execBtn.addEventListener('click', function () {
					var ids = [].slice.call(document.querySelectorAll('.lwp-amber-migration-row:checked')).map(function (cb) { return cb.value; });
					if (!ids.length) {
						alert(<?php echo wp_json_encode( esc_html__( 'Select at least one page first.', 'luwipress-amber' ) ); ?>);
						return;
					}
					if (!confirm(<?php echo wp_json_encode( esc_html__( 'Rename slugs of selected pages? They stay published — only the URL slot is freed.', 'luwipress-amber' ) ); ?>)) return;
					var status = document.getElementById('lwp-amber-migration-status');
					status.textContent = <?php echo wp_json_encode( esc_html__( 'Archiving…', 'luwipress-amber' ) ); ?>;
					execBtn.disabled = true;
					var suffix = (document.getElementById('lwp-amber-migration-suffix').value || 'legacy').trim();
					post('luwipress_amber_migration_archive', { ids: ids, suffix: suffix }, function (err, j) {
						execBtn.disabled = false;
						if (err || !j || !j.success) {
							status.textContent = <?php echo wp_json_encode( esc_html__( 'Failed.', 'luwipress-amber' ) ); ?>;
							return;
						}
						status.textContent = <?php echo wp_json_encode( esc_html__( 'Done. Reloading…', 'luwipress-amber' ) ); ?>;
						setTimeout(function () { location.reload(); }, 600);
					});
				});
			}

			var swapAll = document.getElementById('lwp-amber-swap-all');
			if (swapAll) {
				swapAll.addEventListener('change', function () {
					document.querySelectorAll('.lwp-amber-swap-row').forEach(function (cb) { cb.checked = swapAll.checked; });
				});
			}

			var swapBtn = document.getElementById('lwp-amber-swap-execute');
			if (swapBtn) {
				swapBtn.addEventListener('click', function () {
					var ids = [].slice.call(document.querySelectorAll('.lwp-amber-swap-row:checked')).map(function (cb) { return cb.value; });
					if (!ids.length) {
						alert(<?php echo wp_json_encode( esc_html__( 'Select at least one pair first.', 'luwipress-amber' ) ); ?>);
						return;
					}
					if (!confirm(<?php echo wp_json_encode( esc_html__( 'Swap selected pairs? Customer pages move to <slug>-legacy and our shadow pages take the canonical slugs.', 'luwipress-amber' ) ); ?>)) return;
					var status = document.getElementById('lwp-amber-swap-status');
					status.textContent = <?php echo wp_json_encode( esc_html__( 'Swapping…', 'luwipress-amber' ) ); ?>;
					swapBtn.disabled = true;
					var suffix = (document.getElementById('lwp-amber-migration-suffix') ? document.getElementById('lwp-amber-migration-suffix').value : 'legacy') || 'legacy';
					post('luwipress_amber_migration_swap', { ids: ids, suffix: suffix.trim() }, function (err, j) {
			...
			document.querySelectorAll('.lwp-amber-migration-restore').forEach(function (btn) {
				btn.addEventListener('click', function () {
					var id = btn.getAttribute('data-id');
					if (!confirm(<?php echo wp_json_encode( esc_html__( 'Restore the original slug for this page?', 'luwipress-amber' ) ); ?>)) return;
					btn.disabled = true;
					post('luwipress_amber_migration_restore', { id: id }, function (err, j) {
						btn.disabled = false;
						if (err || !j || !j.success) {
							alert((j && j.data && j.data.message) || <?php echo wp_json_encode( esc_html__( 'Restore failed.', 'luwipress-amber' ) ); ?>);
							return;
						}
						location.reload();
					});
...
LuwiPress_Amber_Migration::instance();
