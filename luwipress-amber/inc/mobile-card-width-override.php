 ...
 *   `<style id="lwp-amber-mobile-90">` block AFTER the kit. Same selector,
 ...
 * with sister plugins.
 *
 * Operator toggle: filter `luwipress_amber_mobile_90_enabled` returning
...
if ( ! function_exists( 'luwipress_amber_emit_mobile_card_width_override' ) ) {
	 ...
	 */
	function luwipress_amber_emit_mobile_card_width_override() {
		if ( is_admin() ) {
			return;
		}
		/**
		 ...
		 */
		if ( ! (bool) apply_filters( 'luwipress_amber_mobile_90_enabled', true ) ) {
			return;
		}
		echo "\n<style id=\"lwp-amber-mobile-90\">\n";
	...
	add_action( 'wp_head', 'luwipress_amber_emit_mobile_card_width_override', 999 );
}
