 ...
 *
 * Customizer toggle: LuwiPress Amber → Performance → Page loader overlay.
 *
...
if ( ! function_exists( 'luwipress_amber_loader_should_render' ) ) {
	 ...
	 * globally via Customizer or via the `luwipress_amber_loader_render`
	 * filter (return false).
	 *
	 * @return bool
	 */
	function luwipress_amber_loader_should_render() {
		if ( is_admin() ) {
			return false;
		}
		...
		$on = (bool) get_theme_mod( 'luwipress_amber_loader_enabled', true );
		return (bool) apply_filters( 'luwipress_amber_loader_render', $on );
	}
}

if ( ! function_exists( 'luwipress_amber_loader_print_head' ) ) {
	 ...
	 */
	function luwipress_amber_loader_print_head() {
		if ( ! luwipress_amber_loader_should_render() ) {
			return;
		}
		?>
<style id="lwp-amber-loader-boot">
...
<script id="lwp-amber-loader-boot-mark" data-no-optimize="1" data-no-defer="true" data-cfasync="false">
...
window.setTimeout(function () {
	...
	add_action( 'wp_head', 'luwipress_amber_loader_print_head', 1 );
}
 ...
 * the unique substring `lwp-amber-loader-boot-mark` (matches both the ID
 ...
 */
if ( ! function_exists( 'luwipress_amber_loader_litespeed_exclude' ) ) {
	function luwipress_amber_loader_litespeed_exclude( $excludes ) {
		if ( ! is_array( $excludes ) ) {
			$excludes = array();
		}
		$excludes[] = 'lwp-amber-loader-boot-mark';
		return $excludes;
	}
	add_filter( 'litespeed_optm_js_defer_exc', 'luwipress_amber_loader_litespeed_exclude' );
	add_filter( 'litespeed_optm_js_excludes', 'luwipress_amber_loader_litespeed_exclude' );
}

if ( ! function_exists( 'luwipress_amber_loader_render' ) ) {
	 ...
	 */
	function luwipress_amber_loader_render() {
		if ( ! luwipress_amber_loader_should_render() ) {
			return;
		}
		?>
<div class="lwp-loader" id="lwp-loader" aria-hidden="true">
	<div class="lwp-loader-inner">
		...
		<span class="lwp-loader-text"><?php esc_html_e( 'Loading', 'luwipress-amber' ); ?></span>
		<span class="lwp-loader-bar"><span></span></span>
	</div>
</div>
		<?php
	}
	add_action( 'wp_body_open', 'luwipress_amber_loader_render', 1 );
}

if ( ! function_exists( 'luwipress_amber_loader_customizer' ) ) {

	/**
	 * Register the on/off toggle in Customize → LuwiPress Amber →
	 ...
	 */
	function luwipress_amber_loader_customizer( $wp_customize ) {
		...
		$wp_customize->add_control( 'luwipress_amber_loader_enabled', [
			...
			'description' => __( 'Show a brand-aligned loader overlay until the page finishes painting. Server-rendered, so it appears on the first frame instead of after the page has flashed.', 'luwipress-amber' ),
			'section'     => $section_id,
			'type'        => 'checkbox',
			'priority'    => 10,
		] );
	}
	add_action( 'customize_register', 'luwipress_amber_loader_customizer', 30 );
}
