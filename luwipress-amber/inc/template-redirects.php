 ...
 * Two behavior modes:
 *
 *   1. AUTO (default off, opt-in via theme_mod `luwipress_amber_resolve_slug_conflicts`)
 ...
 *   - `luwipress_amber_slug_conflict_skip_slugs` — extend / suppress the
 *     detected system-page skip list before the SQL discovery runs.
 *   - `luwipress_amber_slug_conflict_map_pre_cache` — modify the final
 *     map before it's written to the transient (add curated entries,
 *     drop noisy ones).
 *   - `luwipress_amber_hub_redirects` — final composition hook called on
 ...
 *   `luwipress_amber_term_has_population()` which recurses into
 ...
 * the active language).
 *
 * Operator overrides via the `luwipress_amber_hub_redirects` filter
 ...
 * auto-target:
 *
 *     add_filter( 'luwipress_amber_hub_redirects', function ( $map ) {
 ...
 * `luwipress_amber_resolve_slug_conflicts` theme_mod so operators who
...
const LUWIPRESS_AMBER_SLUG_CONFLICT_TRANSIENT = 'luwipress_amber_slug_conflicts_v1';
const LUWIPRESS_AMBER_SLUG_CONFLICT_OPTION    = 'luwipress_amber_resolve_slug_conflicts';

if ( ! function_exists( 'luwipress_amber_skip_redirect_slugs' ) ) {
	 ...
	 * suppress via the `luwipress_amber_slug_conflict_skip_slugs`
	 ...
	 */
	function luwipress_amber_skip_redirect_slugs() {
		$slugs = [];
		 ...
		 */
		$slugs = (array) apply_filters( 'luwipress_amber_slug_conflict_skip_slugs', $slugs );
		return array_values( array_unique( array_filter( array_map( 'strval', $slugs ) ) ) );
	}
}

if ( ! function_exists( 'luwipress_amber_term_has_population' ) ) {
	 ...
	 */
	function luwipress_amber_term_has_population( $term_id, $taxonomy = 'product_cat' ) {
		static $cache = [];
		$term_id = (int) $term_id;
		if ( $term_id <= 0 ) {
			return false;
...
if ( ! function_exists( 'luwipress_amber_term_find_populated_ancestor' ) ) {
	 ...
	 */
	function luwipress_amber_term_find_populated_ancestor( $term_id, $taxonomy = 'product_cat' ) {
		$term_id = (int) $term_id;
		$visited = [];
		while ( $term_id > 0 && ! isset( $visited[ $term_id ] ) ) {
			$visited[ $term_id ] = true;
			if ( luwipress_amber_term_has_population( $term_id, $taxonomy ) ) {
				return $term_id;
			}
			$term = get_term( $term_id, $taxonomy );
			if ( ! $term instanceof \WP_Term || is_wp_error( $term ) ) {
...
if ( ! function_exists( 'luwipress_amber_resolve_slug_conflicts_enabled' ) ) {

	/**
	 * @return bool true when the operator has opted into auto slug-collision
	 *              redirects via Customizer (or theme-mod, or wp-cli).
	 */
	function luwipress_amber_resolve_slug_conflicts_enabled() {
		...
		$mod = get_theme_mod( LUWIPRESS_AMBER_SLUG_CONFLICT_OPTION, null );
		if ( $mod !== null ) {
			return (bool) $mod;
		}
		return (bool) get_option( LUWIPRESS_AMBER_SLUG_CONFLICT_OPTION, false );
	}
}

if ( ! function_exists( 'luwipress_amber_discover_slug_conflicts' ) ) {
	 ...
	 */
	function luwipress_amber_discover_slug_conflicts() {
		if ( ! taxonomy_exists( 'product_cat' ) ) {
			return [];
		}
		$cached = get_transient( LUWIPRESS_AMBER_SLUG_CONFLICT_TRANSIENT );
		if ( is_array( $cached ) ) {
			return $cached;
		}
		...
		if ( is_array( $exact_rows ) ) {
			foreach ( $exact_rows as $r ) {
				...
				if ( luwipress_amber_term_has_population( $tid ) ) {
		...
		if ( function_exists( 'pll_get_post_translations' ) || taxonomy_exists( 'language' ) ) {
			$skip_slugs = luwipress_amber_skip_redirect_slugs();
			...
			foreach ( (array) $page_rows as $r ) {
				...
				foreach ( $siblings as $sib_id ) {
					...
					if ( luwipress_amber_term_has_population( (int) $term->term_id ) ) {
		...
		foreach ( (array) $menus as $menu ) {
			...
			foreach ( $items as $item ) {
				...
				if ( $parent->object === 'product_cat' && ! empty( $parent->object_id ) ) {
					...
					if ( $pt instanceof \WP_Term && luwipress_amber_term_has_population( (int) $pt->term_id ) ) {
		...
		foreach ( $empty_term_candidates as $slug => $empty_tid ) {
			...
			$anc = luwipress_amber_term_find_populated_ancestor( (int) $empty_tid );
			if ( $anc > 0 ) {
				$map[ $slug ] = $anc;
			}
		}
		...
		$map = (array) apply_filters( 'luwipress_amber_slug_conflict_map_pre_cache', $map );

		set_transient( LUWIPRESS_AMBER_SLUG_CONFLICT_TRANSIENT, $map, HOUR_IN_SECONDS );
		return $map;
	}
}

if ( ! function_exists( 'luwipress_amber_get_hub_redirects' ) ) {
	 ...
	 */
	function luwipress_amber_get_hub_redirects() {
		$auto = luwipress_amber_resolve_slug_conflicts_enabled()
			? luwipress_amber_discover_slug_conflicts()
			: [];
		 ...
		 */
		$composed = (array) apply_filters( 'luwipress_amber_hub_redirects', $auto );
		return $composed;
	}
}

if ( ! function_exists( 'luwipress_amber_maybe_redirect_hub' ) ) {
	 ...
	 * non-GET requests.
	 */
	function luwipress_amber_maybe_redirect_hub() {
		...
		$redirects = luwipress_amber_get_hub_redirects();
		if ( empty( $redirects ) ) {
			return;
		}
...
add_action( 'template_redirect', 'luwipress_amber_maybe_redirect_hub', 1 );

if ( ! function_exists( 'luwipress_amber_bust_slug_conflict_cache' ) ) {
	 ...
	 */
	function luwipress_amber_bust_slug_conflict_cache() {
		delete_transient( LUWIPRESS_AMBER_SLUG_CONFLICT_TRANSIENT );
	}
	add_action( 'save_post_page',         'luwipress_amber_bust_slug_conflict_cache' );
	add_action( 'deleted_post',           'luwipress_amber_bust_slug_conflict_cache' );
	add_action( 'edited_product_cat',     'luwipress_amber_bust_slug_conflict_cache' );
	add_action( 'created_product_cat',    'luwipress_amber_bust_slug_conflict_cache' );
	add_action( 'delete_product_cat',     'luwipress_amber_bust_slug_conflict_cache' );
	add_action( 'switch_theme',           'luwipress_amber_bust_slug_conflict_cache' );
	add_action( 'update_option_' . LUWIPRESS_AMBER_SLUG_CONFLICT_OPTION, 'luwipress_amber_bust_slug_conflict_cache' );
}

if ( ! function_exists( 'luwipress_amber_register_slug_conflict_customizer' ) ) {

	/**
	 * Register the opt-in toggle in Customizer → LuwiPress Amber → Performance.
	 * Falls back to creating its own section when the panel hasn't been
	 * registered yet (load-order safety).
	 */
	function luwipress_amber_register_slug_conflict_customizer( $wp_customize ) {
		// Section: re-use Performance panel if it exists, else add ours.
		$section_id = 'luwipress_amber_performance';
		...
		$wp_customize->add_setting( LUWIPRESS_AMBER_SLUG_CONFLICT_OPTION, [
			'type'              => 'theme_mod',
			'default'           => false,
		...
		$conflicts = luwipress_amber_discover_slug_conflicts();
		...
		$desc      = $count > 0
			...
			: __( 'No page/product-category slug conflicts detected. Toggle is harmless to leave on — it only redirects when a conflict actually exists.', 'luwipress-amber' );

		$wp_customize->add_control( LUWIPRESS_AMBER_SLUG_CONFLICT_OPTION, [
			'label'       => __( 'Resolve page/category slug conflicts', 'luwipress-amber' ),
			'description' => $desc,
			'section'     => $section_id,
			'type'        => 'checkbox',
			'priority'    => 20,
	...
	add_action( 'customize_register', 'luwipress_amber_register_slug_conflict_customizer', 30 );
}
