...
class LuwiPress_Amber_Widget_Category_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-category-grid'; }
	public function get_title()       { return __( 'Category Grid', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'product_cat' => __( 'WooCommerce top-level categories', 'luwipress-amber' ),
				],
			]
		);
		$this->add_control(
			'count',
			[
				'label'       => __( 'Number of tiles (when pulling from WC)', 'luwipress-amber' ),
		...
		$this->add_control(
			'count_label',
			[
				'label'       => __( 'Eyebrow label after the count', 'luwipress-amber' ),
				'description' => __( 'e.g. "instruments" → "62 instruments". Leave empty to hide the count line.', 'luwipress-amber' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => __( 'instruments', 'luwipress-amber' ),
				'condition'   => [ 'source' => 'product_cat' ],
			]
		);
		...
		$this->start_controls_section( 'section_items', [
			'label'     => __( 'Tiles', 'luwipress-amber' ),
			'condition' => [ 'source' => 'manual' ],
		] );

		$rep = new Repeater();
		$rep->add_control( 'eyebrow',  [ 'label' => __( 'Eyebrow / count line', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'title',    [ 'label' => __( 'Title', 'luwipress-amber' ),      'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'sub',      [ 'label' => __( 'Sub line', 'luwipress-amber' ),   'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'url',      [ 'label' => __( 'URL', 'luwipress-amber' ),        'type' => Controls_Manager::URL,  'default' => [ 'url' => '' ] ] );
		$rep->add_control( 'image',    [ 'label' => __( 'Background image (optional)', 'luwipress-amber' ), 'type' => Controls_Manager::MEDIA, 'default' => [ 'url' => '' ] ] );
		$rep->add_control( 'grad_from',[ 'label' => __( 'Gradient start (fallback)', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#3d2f1f' ] );
		$rep->add_control( 'grad_to',  [ 'label' => __( 'Gradient end (fallback)', 'luwipress-amber' ),   'type' => Controls_Manager::COLOR, 'default' => '#7a5a2c' ] );
		...
		$this->add_control(
			...
			[
				...
				'default'     => [
					[ 'eyebrow' => '', 'title' => __( 'Category 1', 'luwipress-amber' ), 'sub' => '', 'grad_from' => '#d4b97a', 'grad_to' => '#7a5a2c' ],
					[ 'eyebrow' => '', 'title' => __( 'Category 2', 'luwipress-amber' ), 'sub' => '', 'grad_from' => '#3d2f1f', 'grad_to' => '#7a5a2c' ],
					[ 'eyebrow' => '', 'title' => __( 'Category 3', 'luwipress-amber' ), 'sub' => '', 'grad_from' => '#5a3a2a', 'grad_to' => '#c89a5a' ],
					[ 'eyebrow' => '', 'title' => __( 'Category 4', 'luwipress-amber' ), 'sub' => '', 'grad_from' => '#2a3d3a', 'grad_to' => '#7aa494' ],
				],
			]
		);
