...
class LuwiPress_Amber_Widget_CTA_Banner extends Widget_Base {

	public function get_name()        { return 'lwp-cta-banner'; }
	public function get_title()       { return __( 'CTA Banner', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'eyebrow', [ 'label' => __( 'Eyebrow', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$this->add_control( 'heading', [
			'label'       => __( 'Headline', 'luwipress-amber' ),
			'description' => __( 'Use Enter for line breaks. Wrap with [em]…[/em] for italic gold accent.', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXTAREA,
			'rows'        => 3,
			'default'     => __( 'Build the instrument you have always wanted.', 'luwipress-amber' ),
		] );
		$this->add_control( 'lead', [ 'label' => __( 'Lead', 'luwipress-amber' ), 'type' => Controls_Manager::TEXTAREA, 'rows' => 3, 'default' => '' ] );

		$this->add_control( 'cta1_label', [ 'label' => __( 'Primary CTA — label', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Start a custom order →', 'luwipress-amber' ) ] );
		$this->add_control( 'cta1_url',   [ 'label' => __( 'Primary CTA — URL', 'luwipress-amber' ),   'type' => Controls_Manager::URL,  'default' => [ 'url' => '' ] ] );
		$this->add_control( 'cta2_label', [ 'label' => __( 'Secondary CTA — label', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$this->add_control( 'cta2_url',   [ 'label' => __( 'Secondary CTA — URL', 'luwipress-amber' ),   'type' => Controls_Manager::URL,  'default' => [ 'url' => '' ] ] );

		$this->end_controls_section();

		$this->start_controls_section( 'section_bg', [ 'label' => __( 'Background', 'luwipress-amber' ) ] );

		$this->add_control( 'bg_image', [
			'label' => __( 'Background image (optional)', 'luwipress-amber' ),
			'type'  => Controls_Manager::MEDIA,
			'default' => [ 'url' => '' ],
		] );
		$this->add_control( 'bg_grad_from', [ 'label' => __( 'Gradient start', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612' ] );
		$this->add_control( 'bg_grad_to',   [ 'label' => __( 'Gradient end', 'luwipress-amber' ),   'type' => Controls_Manager::COLOR, 'default' => '#3d2f1f' ] );

		$this->add_control( 'overlay', [
			'label' => __( 'Image overlay strength', 'luwipress-amber' ),
			'description' => __( 'Only applies when a background image is set.', 'luwipress-amber' ),
			'type'  => Controls_Manager::SELECT,
			'default' => 'medium',
			'options' => [
				'none'    => __( 'None', 'luwipress-amber' ),
				'light'   => __( 'Light', 'luwipress-amber' ),
				'medium'  => __( 'Medium', 'luwipress-amber' ),
				'strong'  => __( 'Strong', 'luwipress-amber' ),
			],
		] );

		$this->add_control( 'align', [
			'label' => __( 'Content alignment', 'luwipress-amber' ),
			'type'  => Controls_Manager::SELECT,
			'default' => 'center',
			'options' => [
				'left'   => __( 'Left', 'luwipress-amber' ),
				'center' => __( 'Center', 'luwipress-amber' ),
				'right'  => __( 'Right', 'luwipress-amber' ),
			],
		] );
