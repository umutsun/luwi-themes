...
class LuwiPress_Amber_Widget_Editorial_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-editorial-grid'; }
	public function get_title()       { return __( 'Editorial Grid', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			'count',
			[
				'label'   => __( 'Number of posts', 'luwipress-amber' ),
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'modified' => __( 'Recently updated', 'luwipress-amber' ),
					'rand'     => __( 'Random', 'luwipress-amber' ),
					'comment_count' => __( 'Most discussed', 'luwipress-amber' ),
				],
			]
		);
		$this->add_control(
			'category',
			[
				'label'       => __( 'Filter by category', 'luwipress-amber' ),
				'type'        => Controls_Manager::SELECT2,
				'multiple'    => true,
				'options'     => $this->get_category_options(),
				'description' => __( 'Leave empty for all.', 'luwipress-amber' ),
			]
		);
		...
		/* Heading */
		$this->start_controls_section( 'section_head', [ 'label' => __( 'Section heading', 'luwipress-amber' ) ] );
		$this->add_control(
			'eyebrow',
			[
				'label'   => __( 'Eyebrow', 'luwipress-amber' ),
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'Journal', 'luwipress-amber' ),
			]
		);
		$this->add_control(
			'heading',
			[
				...
				'rows'    => 2,
				'default' => __( 'Notes from the workshop.', 'luwipress-amber' ),
			]
		);
		$this->add_control(
			'all_link',
			[
				'label'   => __( 'View all link', 'luwipress-amber' ),
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'All articles →', 'luwipress-amber' ),
			]
		);
		$this->end_controls_section();
	}
	...
	protected function render() {
		...
		if ( ! $query->have_posts() ) {
			echo '<div class="lwp-eg lwp-eg--empty">' . esc_html__( 'No posts found.', 'luwipress-amber' ) . '</div>';
			return;
		}
		?>
