...
class LuwiPress_Amber_Widget_Featured_Post extends Widget_Base {

	public function get_name()        { return 'lwp-featured-post'; }
	public function get_title()       { return __( 'Featured Post', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Source', 'luwipress-amber' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'default' => 'auto-sticky',
				'options' => [
					'auto-sticky' => __( 'Auto: first sticky post', 'luwipress-amber' ),
					'auto-latest' => __( 'Auto: latest post', 'luwipress-amber' ),
					'manual'      => __( 'Manual: pick post by ID', 'luwipress-amber' ),
				],
			]
		);
		$this->add_control(
			'manual_post_id',
			[
				'label'     => __( 'Post ID', 'luwipress-amber' ),
		...
		$this->add_control(
			'post_type',
			[
				'label'   => __( 'Post type', 'luwipress-amber' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'post',
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::SELECT,
				'default' => 'left',
				'options' => [ 'left' => __( 'Image left', 'luwipress-amber' ), 'right' => __( 'Image right', 'luwipress-amber' ) ],
			]
		);
		$this->add_control(
			'show_excerpt',
			[ 'label' => __( 'Show excerpt', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);
		$this->add_control(
			'show_meta',
			[ 'label' => __( 'Show meta (category · read time)', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);
		$this->add_control(
			'cta_label',
			[ 'label' => __( 'CTA label', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Read on →', 'luwipress-amber' ) ]
		);
		...
		$this->add_control( 'title_color', [ 'label' => __( 'Title color', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
			'selectors' => [ '{{WRAPPER}} .lwp-fp__title' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'eyebrow_color', [ 'label' => __( 'Eyebrow color', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#9A7B3A',
			'selectors' => [ '{{WRAPPER}} .lwp-fp__eyebrow' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'cta_color', [ 'label' => __( 'CTA color', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
	...
	protected function render() {
		...
		if ( ! $post ) {
			echo '<div class="lwp-fp lwp-fp--empty"><p>' . esc_html__( 'No featured post found.', 'luwipress-amber' ) . '</p></div>';
			return;
		}
			...
			<div class="lwp-fp__body">
				<?php if ( $show_meta && ( $cat_label || $read_min ) ) : ?>
					<span class="lwp-fp__eyebrow">
						...
						<?php if ( $read_min ) : ?><?php printf( esc_html( _n( '%d min read', '%d min read', $read_min, 'luwipress-amber' ) ), $read_min ); ?><?php endif; ?>
					</span>
				<?php endif; ?>
