...
class LuwiPress_Amber_Widget_Featured_Strip extends Widget_Base {

	public function get_name()        { return 'lwp-featured-strip'; }
	public function get_title()       { return __( 'Featured Strip', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'count', [
			'label'   => __( 'Max items', 'luwipress-amber' ),
		...
		$this->add_control( 'fallback', [
			'label'       => __( 'Fallback when registry is empty', 'luwipress-amber' ),
			'description' => __( 'What to show when no products are flipped to featured. Falls back automatically — the operator does not see the strip disappear during onboarding.', 'luwipress-amber' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'bestseller',
			'options'     => [
				'bestseller' => __( 'Best-selling products', 'luwipress-amber' ),
				'onsale'     => __( 'On-sale products', 'luwipress-amber' ),
				'recent'     => __( 'Recently added', 'luwipress-amber' ),
				'hide'       => __( 'Hide the strip', 'luwipress-amber' ),
			],
		] );

		$this->add_control( 'show_note', [
			'label'        => __( 'Show operator note chips', 'luwipress-amber' ),
			'description'  => __( 'Renders the operator note from the meta box ("Boxing Week pick", "New arrival") as a small chip on each card.', 'luwipress-amber' ),
		...
		$this->start_controls_section( 'section_head', [ 'label' => __( 'Heading (optional)', 'luwipress-amber' ) ] );
		$this->add_control( 'eyebrow', [ 'label' => __( 'Eyebrow', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Hand-picked', 'luwipress-amber' ) ] );
		$this->add_control( 'heading', [ 'label' => __( 'Heading', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Featured products.', 'luwipress-amber' ) ] );
		$this->end_controls_section();
	}
	...
	protected function get_ids( $s ) {
		...
		if ( function_exists( 'lwp_amber_get_featured_ids' ) ) {
			$ids = lwp_amber_get_featured_ids( [ 'number' => $count ] );
		}
		...
		<section class="lwp-fs">
			...
			<div class="lwp-fs__track">
				<?php foreach ( $ids as $pid ) :
					...
					$note = $show_note ? (string) get_post_meta( $pid, '_luwipress_amber_featured_note', true ) : '';
					?>
