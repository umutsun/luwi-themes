...
class LuwiPress_Amber_Widget_Hero_Split extends Widget_Base {

	public function get_name()        { return 'lwp-hero-split'; }
	public function get_title()       { return __( 'Hero Split', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		/* Left: copy */
		$this->start_controls_section( 'section_copy', [ 'label' => __( 'Copy (left column)', 'luwipress-amber' ) ] );

		$this->add_control( 'eyebrow', [
			'label' => __( 'Eyebrow', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT,
			'default' => __( 'Welcome', 'luwipress-amber' ),
		] );
		$this->add_control( 'heading', [
			'label'       => __( 'Heading (H1)', 'luwipress-amber' ),
			'description' => __( 'Use Enter for line breaks. Wrap with [em]…[/em] for italic gold accent.', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXTAREA,
			'rows'        => 3,
			'default'     => __( "Your headline goes here\nin [em]two lines[/em].", 'luwipress-amber' ),
		] );
		...
		$this->add_control( 'cta1_label', [
			...
			'type'  => Controls_Manager::TEXT,
			'default' => __( 'Shop now →', 'luwipress-amber' ),
		] );
		...
		/* Stats row */
		$this->start_controls_section( 'section_stats', [ 'label' => __( 'Stats row', 'luwipress-amber' ) ] );

		$rep = new Repeater();
		$rep->add_control( 'num',   [ 'label' => __( 'Number', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'label', [ 'label' => __( 'Label', 'luwipress-amber' ),  'type' => Controls_Manager::TEXTAREA, 'rows' => 2, 'default' => '' ] );
		...
		/* Right: image side */
		$this->start_controls_section( 'section_image', [ 'label' => __( 'Image (right column)', 'luwipress-amber' ) ] );

		$this->add_control( 'image', [
			'label' => __( 'Image', 'luwipress-amber' ),
			'type'  => Controls_Manager::MEDIA,
			'default' => [ 'url' => '' ],
		] );
		$this->add_control( 'image_grad_from', [
			'label' => __( 'Gradient start (fallback)', 'luwipress-amber' ),
			'type'  => Controls_Manager::COLOR,
			'default' => '#3d2f1f',
		] );
		$this->add_control( 'image_grad_to', [
			'label' => __( 'Gradient end (fallback)', 'luwipress-amber' ),
			'type'  => Controls_Manager::COLOR,
			'default' => '#7a5a2c',
		] );

		$this->add_control( 'pill_text', [
			'label'       => __( 'Live status pill text', 'luwipress-amber' ),
			'description' => __( 'Small pulsing-dot pill above the image. Leave empty to hide.', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
		] );

		$this->add_control( 'tag_eyebrow', [
			'label' => __( 'Featured tag — eyebrow', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT,
			'default' => __( 'Featured', 'luwipress-amber' ),
		] );
		$this->add_control( 'tag_title', [
			'label' => __( 'Featured tag — title', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT,
			'default' => '',
		] );
		...
		/* Master quote card */
		$this->start_controls_section( 'section_quote', [ 'label' => __( 'Master quote card', 'luwipress-amber' ) ] );

		$this->add_control( 'quote_avatar', [
			'label' => __( 'Avatar image (optional)', 'luwipress-amber' ),
			'type'  => Controls_Manager::MEDIA,
			'default' => [ 'url' => '' ],
		] );
		$this->add_control( 'quote_initial', [
			'label' => __( 'Avatar initial (fallback)', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT,
			'default' => '',
		] );
		$this->add_control( 'quote_name', [
			'label' => __( 'Name', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT,
			'default' => '',
		] );
		$this->add_control( 'quote_role', [
			'label' => __( 'Role', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT,
			'default' => '',
		] );
		$this->add_control( 'quote_text', [
			'label' => __( 'Quote', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXTAREA,
			'rows'  => 2,
			'default' => '',
		] );
