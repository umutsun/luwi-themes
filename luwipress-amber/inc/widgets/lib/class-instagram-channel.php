 ...
 *      setting under LuwiPress Amber → Footer → Social (also exposed in
...
class LuwiPress_Amber_Widget_Instagram_Channel extends Widget_Base {

	public function get_name()        { return 'lwp-instagram-channel'; }
	public function get_title()       { return __( 'Instagram Channel', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'Follow us', 'luwipress-amber' ),
			]
		);
		$this->add_control(
			'heading',
			[
				...
				'description' => __( 'Use Enter for a 2-line title.', 'luwipress-amber' ),
				'type'    => Controls_Manager::TEXTAREA,
				'rows'    => 2,
				'default' => __( "Latest posts\non Instagram.", 'luwipress-amber' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'Instagram profile →', 'luwipress-amber' ),
			]
		);
		$this->add_control(
			'cta_url',
			[
				...
				'description' => __( 'Leave empty to pull the profile URL from Customize → LuwiPress Amber → Footer → Instagram URL (or LuwiPress admin → Theme → Settings → Social).', 'luwipress-amber' ),
		...
		$this->add_control(
			'show_follow',
			[
				'label'        => __( 'Show Follow button', 'luwipress-amber' ),
				'description'  => __( 'Only renders if a profile URL is set (here or in Customize → Footer → Instagram URL).', 'luwipress-amber' ),
		...
		$this->add_control(
			'follow_label',
			[
				'label'     => __( 'Follow label', 'luwipress-amber' ),
				'type'      => Controls_Manager::TEXT,
				'default'   => __( 'Follow', 'luwipress-amber' ),
		...
		$this->start_controls_section( 'section_posts', [ 'label' => __( 'Posts', 'luwipress-amber' ) ] );

		$repeater = new Repeater();
		$repeater->add_control(
			'thumbnail',
			[
				'label'   => __( 'Thumbnail (square)', 'luwipress-amber' ),
				'description' => __( 'Upload a square crop (recommended 600×600 or 1080×1080). Falls back to a solid tile if blank.', 'luwipress-amber' ),
		...
		$repeater->add_control(
			'post_url',
			[
				'label'       => __( 'Instagram post URL', 'luwipress-amber' ),
				'description' => __( 'Full post URL, e.g. https://www.instagram.com/p/AbC123XyZ/', 'luwipress-amber' ),
				'type'        => Controls_Manager::TEXT,
		...
		$repeater->add_control(
			...
			[
				...
				'description' => __( 'Short overlay text shown on hover.', 'luwipress-amber' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'meta',
			[
				'label'       => __( 'Meta line', 'luwipress-amber' ),
				'description' => __( 'Optional small badge (e.g. "Reel · 12k likes" or "📷 Photo").', 'luwipress-amber' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'kind',
			[
				'label'   => __( 'Post type icon', 'luwipress-amber' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'post',
				'options' => [
					'post'     => __( 'Photo', 'luwipress-amber' ),
					'reel'     => __( 'Reel', 'luwipress-amber' ),
					'carousel' => __( 'Carousel', 'luwipress-amber' ),
				],
			]
		);
		...
		$this->add_control(
			'posts',
			[
				'label'       => __( 'Posts', 'luwipress-amber' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $repeater->get_controls(),
				'title_field' => '{{{ caption || post_url || "Post" }}}',
				'default'     => [
					[ 'post_url' => '', 'caption' => '', 'meta' => __( 'Photo', 'luwipress-amber' ),    'kind' => 'post',     'thumbnail' => [ 'url' => '' ] ],
					[ 'post_url' => '', 'caption' => '', 'meta' => __( 'Reel', 'luwipress-amber' ),     'kind' => 'reel',     'thumbnail' => [ 'url' => '' ] ],
					[ 'post_url' => '', 'caption' => '', 'meta' => __( 'Carousel', 'luwipress-amber' ), 'kind' => 'carousel', 'thumbnail' => [ 'url' => '' ] ],
					[ 'post_url' => '', 'caption' => '', 'meta' => __( 'Photo', 'luwipress-amber' ),    'kind' => 'post',     'thumbnail' => [ 'url' => '' ] ],
				],
			]
		);
	...
	protected function resolve_profile_url( $widget_url ) {
		...
		$mod = get_theme_mod( 'luwipress_amber_social_instagram', '' );
	...
	protected function render() {
		...
		$follow_label = trim( (string) ( $s['follow_label'] ?? '' ) ) ?: __( 'Follow', 'luwipress-amber' );
		...
		<section class="lwp-igc" data-columns="<?php echo esc_attr( $columns ); ?>">
					...
					<div class="lwp-igc-head__actions">
						<?php if ( $show_follow ) : ?>
							<a class="lwp-igc-follow"
								...
								aria-label="<?php echo esc_attr( sprintf( __( 'Follow %s on Instagram', 'luwipress-amber' ), $handle ?: '' ) ); ?>">
			...
			<?php if ( empty( $resolved ) ) : ?>
				<div class="lwp-ytc-placeholder" role="note">
					<strong><?php esc_html_e( 'No Instagram posts configured yet.', 'luwipress-amber' ); ?></strong>
					<span><?php esc_html_e( 'Open this widget and upload a square thumbnail + paste the Instagram post URL for each Posts row. This notice is only visible to logged-in editors.', 'luwipress-amber' ); ?></span>
				</div>
			<?php else : ?>
			<div class="lwp-igc-grid">
				<?php foreach ( $resolved as $item ) :
					...
					$href    = $url !== '' ? $url : '#';
					$aria    = $caption !== '' ? sprintf( __( 'Open Instagram post — %s', 'luwipress-amber' ), $caption ) : __( 'Open Instagram post', 'luwipress-amber' );
					?>
