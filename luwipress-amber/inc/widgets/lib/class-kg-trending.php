...
class LuwiPress_Amber_Widget_KG_Trending extends Widget_Base {

	public function get_name()        { return 'lwp-kg-trending'; }
	public function get_title()       { return __( 'Knowledge Graph · Trending', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'mode', [
			'label'   => __( 'Trending', 'luwipress-amber' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'categories',
			'options' => [
				'categories' => __( 'Product categories', 'luwipress-amber' ),
				'products'   => __( 'Products', 'luwipress-amber' ),
			],
		] );
		$this->add_control( 'count', [
			'label'   => __( 'How many', 'luwipress-amber' ),
		...
		$this->add_control( 'show_score', [
			'label'        => __( 'Show KG score', 'luwipress-amber' ),
			'description'  => __( 'Small pill on each card showing the KG ranking score. Useful in editorial contexts to signal "data-driven".', 'luwipress-amber' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => 'yes',
			'default'      => '',
		] );

		$this->add_control( 'eyebrow', [ 'label' => __( 'Eyebrow', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Picking up momentum', 'luwipress-amber' ) ] );
		$this->add_control( 'heading', [ 'label' => __( 'Heading', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Trending right now.', 'luwipress-amber' ) ] );
	...
	protected function fetch_trending( $mode, $count ) {
		...
		$cache_key = 'lwp_amber_kg_trending_' . $mode . '_' . $count;
		...
		<div class="lwp-kgstats lwp-kgstats--upsell" role="region">
			<div class="lwp-kgstats__upsell-inner">
				...
				<h3 class="lwp-kgstats__upsell-title">
					<?php esc_html_e( 'See what is trending — powered by the Knowledge Graph.', 'luwipress-amber' ); ?>
				</h3>
				<p class="lwp-kgstats__upsell-lead">
					<?php esc_html_e( 'LuwiPress reads your store + reviews + traffic signals and ranks categories and products on a live KG score. Install the free plugin to surface it on this widget.', 'luwipress-amber' ); ?>
				</p>
		...
		<section class="lwp-kgt" data-mode="<?php echo esc_attr( $mode ); ?>">
			<?php if ( $eyebrow || $heading ) : ?>
				<header class="lwp-kgt__head">
					...
					<span class="lwp-kgt__badge"><?php esc_html_e( 'Live · LuwiPress KG', 'luwipress-amber' ); ?></span>
				</header>
			<?php endif; ?>
							...
							<?php if ( $cnt ) : ?>
								<span class="lwp-kgt__count"><?php
									echo esc_html( $mode === 'categories'
										? sprintf( _n( '%d product', '%d products', $cnt, 'luwipress-amber' ), $cnt )
										: sprintf( _n( '%d sale', '%d sales', $cnt, 'luwipress-amber' ), $cnt )
									);
								?></span>
							<?php endif; ?>
						</span>
