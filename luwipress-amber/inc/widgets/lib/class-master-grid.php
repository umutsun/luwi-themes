...
class LuwiPress_Amber_Widget_Master_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-master-grid'; }
	public function get_title()       { return __( 'Master Grid', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'Pulls live from the Vendors CPT (LuwiPress → Vendors). Add or edit profiles there; this grid updates automatically.', 'luwipress-amber' ) . '</em>',
			]
		);
		...
		$this->add_control(
			'limit',
			[
				'label'   => __( 'Max profiles', 'luwipress-amber' ),
		...
		$group_options = [ '' => __( '— All vendors —', 'luwipress-amber' ) ];
		...
		$this->add_control(
			'vendor_group',
			[
				'label'       => __( 'Vendor group', 'luwipress-amber' ),
				'description' => __( 'Filter to one vendor group (Luthiers, Team, etc.). Manage groups under LuwiPress → Vendors → Vendor Groups.', 'luwipress-amber' ),
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Comma-separated CPT IDs to show in this order. Leave empty to auto-query (menu_order ASC).', 'luwipress-amber' ),
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'title'      => __( 'Name (A→Z)', 'luwipress-amber' ),
		...
		$this->add_control(
			'show_location',
			[ 'label' => __( 'Show location', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);

		$this->add_control(
			'show_specialty',
			[ 'label' => __( 'Show specialty', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);

		$this->add_control(
			'show_count',
			[
				'label'       => __( 'Show product count', 'luwipress-amber' ),
				'description' => __( 'Counts WC products attributed to this vendor via _lwp_vendor_ids meta.', 'luwipress-amber' ),
		...
		$this->add_control(
			...
			[
				...
				'description'  => __( 'Adds a CTA below the grid linking to the full Vendors archive (e.g. /luthiers/). Useful when the grid is capped at 4–8 and visitors need a path to the rest.', 'luwipress-amber' ),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'default'      => 'yes',
			]
		...
		$this->add_control(
			...
			[
				...
				'type'        => Controls_Manager::TEXT,
				'default'     => __( 'View all vendors →', 'luwipress-amber' ),
				'placeholder' => __( 'View all vendors →', 'luwipress-amber' ),
	...
	protected function render() {
		...
		if ( ! post_type_exists( 'lwp_vendor' ) ) {
			echo '<div class="lwp-mgrid lwp-mgrid--placeholder"><em>' . esc_html__( '[Master Grid] LuwiPress Vendors module is inactive — enable it in LuwiPress → Vendors.', 'luwipress-amber' ) . '</em></div>';
			return;
		}
		...
		$q = new WP_Query( $args );
		if ( ! $q->have_posts() ) {
			echo '<div class="lwp-mgrid lwp-mgrid--empty"><em>' . esc_html__( '[Master Grid] No profiles yet. Add them in LuwiPress → Vendors.', 'luwipress-amber' ) . '</em></div>';
			return;
		}
				...
				if ( $show_count ) {
					...
					if ( $cnt > 0 ) {
						$count_label = sprintf(
							...
							_n( '%d work', '%d works', $cnt, 'luwipress-amber' ),
							$cnt
						);
					}
				}
		...
		if ( ( $s['view_all_link'] ?? 'yes' ) === 'yes' ) {
			...
			if ( $archive_url ) {
				...
				if ( $label === '' ) {
					$label = __( 'View all vendors →', 'luwipress-amber' );
				}
				?>
