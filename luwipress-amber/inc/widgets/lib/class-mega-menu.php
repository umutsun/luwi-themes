<?php
/**
 * Widget: LuwiPress Amber Mega Menu.
 *
 ...
 * (custom field `_lwp_amber_mega_featured` saved per top item).
 *
...
class LuwiPress_Amber_Widget_Mega_Menu extends Widget_Base {

	public function get_name()        { return 'lwp-mega-menu'; }
	public function get_title()       { return __( 'Mega Menu', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			'menu_id',
			[
				'label'       => __( 'Menu', 'luwipress-amber' ),
				'type'        => Controls_Manager::SELECT,
				'options'     => $this->get_menu_options(),
				'description' => __( 'Build the menu in Appearance → Menus, then pick it here. The widget auto-detects which top-level items deserve a mega panel.', 'luwipress-amber' ),
			]
		);
		...
		$this->add_control(
			'mega_threshold',
			[
				'label'       => __( 'Mega panel threshold', 'luwipress-amber' ),
				'type'        => Controls_Manager::NUMBER,
				'default'     => 4,
				'min'         => 2,
				'max'         => 12,
				'description' => __( 'A top-level item becomes a mega panel when it has at least this many children, or when any child has its own children.', 'luwipress-amber' ),
			]
		);
		...
		$this->add_control(
			'mega_columns',
			[
				'label'   => __( 'Mega panel columns', 'luwipress-amber' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'auto',
				'options' => [
					'auto' => __( 'Auto (2 or 3 by item count)', 'luwipress-amber' ),
		...
		$this->add_control(
			'show_counts',
			[
				'label'   => __( 'Show item counts', 'luwipress-amber' ),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
				'description' => __( 'When a menu item points at a product category, show that category\'s product count next to the link.', 'luwipress-amber' ),
			]
		);
		...
		$this->add_control(
			'mobile_collapse',
			[
				'label'   => __( 'Mobile mode', 'luwipress-amber' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'drawer',
				'options' => [
					'drawer'    => __( 'Off-canvas drawer (hamburger)', 'luwipress-amber' ),
					'accordion' => __( 'In-place accordion', 'luwipress-amber' ),
				],
			]
		);
		...
		$this->add_control(
			'top_color',
			[
				'label'     => __( 'Top item color', 'luwipress-amber' ),
		...
		$this->add_control(
			'top_hover_color',
			[
				'label'     => __( 'Top item hover color', 'luwipress-amber' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#735c00',
		...
		$this->add_control(
			'panel_bg',
			[
				'label'     => __( 'Panel background', 'luwipress-amber' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#ffffff',
	...
	private function get_menu_options() {
		$out = [ '' => __( '— Pick a menu —', 'luwipress-amber' ) ];
	...
	protected function render() {
		...
		// Fall back to Customizer (Görünüm → Customize → LuwiPress Amber →
		...
		if ( ! $menu_id ) {
			$menu_id = (int) get_theme_mod( 'luwipress_amber_mega_menu_id', 0 );
		}
		if ( ! $menu_id ) {
			$this->placeholder( __( 'Pick a menu in the panel → (or set the global default in Customize → LuwiPress Amber → Mega menu)', 'luwipress-amber' ) );
			return;
		}
		...
		$threshold = isset( $s['mega_threshold'] ) && (int) $s['mega_threshold'] > 0
			...
			: max( 2, (int) get_theme_mod( 'luwipress_amber_mega_threshold', 4 ) );

		$cols_pref = ! empty( $s['mega_columns'] )
			? $s['mega_columns']
			: (string) get_theme_mod( 'luwipress_amber_mega_columns', 'auto' );

		$show_counts = isset( $s['show_counts'] )
			? ( $s['show_counts'] === 'yes' )
			: (bool) get_theme_mod( 'luwipress_amber_mega_show_counts', true );
	...
	public static function render_navigation_html( $menu_id, $opts = [] ) {
		...
		// from Customize → LuwiPress Amber → Mega menu.
		if ( (bool) get_theme_mod( 'luwipress_amber_mega_auto_inject_blog', true ) ) {
		...
		echo '<nav class="lwp-mm" aria-label="' . esc_attr__( 'Primary', 'luwipress-amber' ) . '">';
	...
	public static function maybe_inject_blog_categories( $node ) {
		...
		if ( ! $is_blog_hub && ! empty( $node['url'] ) ) {
			...
			$last     = end( $segments );
			if ( $last !== false && $last !== '' ) {
				$slugs = function_exists( 'luwipress_amber_get_blog_page_slugs' )
					? luwipress_amber_get_blog_page_slugs()
		...
		$cats = (array) apply_filters( 'luwipress_amber_blog_menu_categories', $cats, $node );
	...
	public static function render_mega_panel( $node, $cols_pref, $show_counts, $head_meta = [] ) {
		...
		if ( $top_url !== '' && ( $top_count !== '' || $top_label !== '' ) ) {
			...
			if ( $top_count !== '' ) {
				printf(
					...
					esc_attr( sprintf( /* translators: %s: count */ _n( '%s item in this collection', '%s items in this collection', (int) $top_count, 'luwipress-amber' ), $top_count ) ),
					esc_html( $top_count )
				);
			}
			printf(
				...
				esc_url( $top_url ),
				esc_html( $top_label ),
				esc_html__( 'View all →', 'luwipress-amber' )
			);
			echo '</div>';
		}
		...
		//   1. Per-menu-item manual override (`_lwp_amber_mega_featured`)
		...
		$featured_id = (int) get_post_meta( $node['id'], '_lwp_amber_mega_featured', true );
		if ( ! $featured_id ) {
	...
	public static function render_featured_slot( $product_id ) {
		...
		$eyebrow    = $is_product ? __( 'Featured', 'luwipress-amber' ) : __( 'Read', 'luwipress-amber' );
		?>
		<a class="lwp-mm-feat" href="<?php echo esc_url( get_permalink( $product_id ) ); ?>">
			...
			<div class="lwp-mm-feat-meta">
				...
				<span class="lwp-mm-feat-link"><?php esc_html_e( 'Open →', 'luwipress-amber' ); ?></span>
			</div>
		</a>
		<?php
	}
	 ...
	 * legacy function (`luwipress_amber_get_hub_redirects`) early-returns
	...
	private static function get_slug_resolver_map() {
		...
		} elseif ( function_exists( 'luwipress_amber_get_hub_redirects' ) ) {
			$cache = (array) luwipress_amber_get_hub_redirects();
		} else {
			$cache = array();
		}
		return $cache;
	...
	private static function registry_featured_for_top( $node ) {
		...
		$ids = lwp_amber_get_featured_ids( [ 'number' => 30 ] );
		if ( empty( $ids ) ) {
			return 0;
		}
