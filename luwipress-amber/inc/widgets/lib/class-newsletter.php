...
class LuwiPress_Amber_Widget_Newsletter extends Widget_Base {

	public function get_name()        { return 'lwp-newsletter'; }
	public function get_title()       { return __( 'Newsletter Signup', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'eyebrow', [ 'label' => __( 'Eyebrow', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Stay in the loop', 'luwipress-amber' ) ] );
		$this->add_control( 'heading', [ 'label' => __( 'Heading', 'luwipress-amber' ), 'type' => Controls_Manager::TEXTAREA, 'rows' => 2, 'default' => __( "New arrivals + offers,\ndelivered to your inbox.", 'luwipress-amber' ) ] );
		$this->add_control( 'lead',    [ 'label' => __( 'Lead', 'luwipress-amber' ),    'type' => Controls_Manager::TEXTAREA, 'rows' => 3, 'default' => __( 'A short letter, once a month. No spam, no resellers.', 'luwipress-amber' ) ] );

		$this->add_control( 'placeholder', [ 'label' => __( 'Email placeholder', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'your@email.com', 'luwipress-amber' ) ] );
		$this->add_control( 'btn_label',   [ 'label' => __( 'Button label', 'luwipress-amber' ),     'type' => Controls_Manager::TEXT, 'default' => __( 'Subscribe', 'luwipress-amber' ) ] );

		$this->add_control( 'gdpr_show', [
			'label'        => __( 'Show GDPR consent checkbox', 'luwipress-amber' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => 'yes',
			'default'      => 'yes',
		] );
		$this->add_control( 'gdpr_text', [
			'label'   => __( 'GDPR consent text', 'luwipress-amber' ),
			'type'    => Controls_Manager::TEXTAREA,
			'rows'    => 2,
			'default' => __( 'I agree to receive your newsletter. I can unsubscribe at any time.', 'luwipress-amber' ),
			'condition' => [ 'gdpr_show' => 'yes' ],
		] );

		$this->add_control( 'success_msg', [
			'label'   => __( 'Success message', 'luwipress-amber' ),
			'type'    => Controls_Manager::TEXTAREA,
			'rows'    => 2,
			'default' => __( 'Thanks for subscribing. Check your inbox to confirm.', 'luwipress-amber' ),
		] );
		$this->add_control( 'error_msg', [
			'label'   => __( 'Error message', 'luwipress-amber' ),
			'type'    => Controls_Manager::TEXT,
			'default' => __( "Couldn't subscribe. Please try again.", 'luwipress-amber' ),
		] );
	...
	protected function detect_crm() {
		if ( function_exists( 'lwp_amber_lp_detector' ) ) {
			$d = lwp_amber_lp_detector();
	...
	protected function render() {
		...
		// Plays nicely with CRM detection — fires `luwipress_amber_newsletter_subscribed`
		// so a future CRM bridge can hook in.
		$rest    = esc_url_raw( rest_url( 'luwipress-amber/v1/subscribe' ) );
		?>
