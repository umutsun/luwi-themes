...
class LuwiPress_Amber_Widget_Process_Steps extends Widget_Base {

	public function get_name()        { return 'lwp-process-steps'; }
	public function get_title()       { return __( 'Process Steps', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_items', [ 'label' => __( 'Steps', 'luwipress-amber' ) ] );

		$rep = new Repeater();
		$rep->add_control( 'title', [ 'label' => __( 'Title', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'body',  [ 'label' => __( 'Body', 'luwipress-amber' ),  'type' => Controls_Manager::TEXTAREA, 'rows' => 3, 'default' => '' ] );
		$rep->add_control( 'icon',  [ 'label' => __( 'Icon (optional, overrides number)', 'luwipress-amber' ), 'type' => Controls_Manager::ICONS, 'default' => [ 'value' => '', 'library' => '' ] ] );

		$this->add_control( 'items', [
			'label'       => __( 'Steps', 'luwipress-amber' ),
			'type'        => Controls_Manager::REPEATER,
			'fields'      => $rep->get_controls(),
			'title_field' => '{{{ title || "Step" }}}',
			'default'     => [
				[ 'title' => __( 'Step 1',  'luwipress-amber' ), 'body' => __( 'Describe the first step of your customer journey here.', 'luwipress-amber' ) ],
				[ 'title' => __( 'Step 2',  'luwipress-amber' ), 'body' => __( 'Second step — what happens after the customer takes step 1.', 'luwipress-amber' ) ],
				[ 'title' => __( 'Step 3',  'luwipress-amber' ), 'body' => __( 'Third step — the build / fulfillment / processing phase.', 'luwipress-amber' ) ],
				[ 'title' => __( 'Step 4',  'luwipress-amber' ), 'body' => __( 'Final step — delivery / handoff / after-sale.', 'luwipress-amber' ) ],
			],
		] );

		$this->add_control( 'show_arrows', [
			'label'        => __( 'Show connecting arrows', 'luwipress-amber' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => 'yes',
			'default'      => 'yes',
		] );
		$this->add_control( 'orientation', [
			'label'   => __( 'Orientation', 'luwipress-amber' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'horizontal',
			'options' => [
				'horizontal' => __( 'Horizontal (cards in a row)', 'luwipress-amber' ),
				'vertical'   => __( 'Vertical (left-aligned list)', 'luwipress-amber' ),
			],
		] );
