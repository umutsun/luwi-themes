...
class LuwiPress_Amber_Widget_Product_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-product-grid'; }
	public function get_title()       { return __( 'Product Grid', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		/* ────────────── Content / Query ────────────── */
		$this->start_controls_section( 'section_query', [ 'label' => __( 'Query', 'luwipress-amber' ) ] );

		$this->add_control( 'heading', [
			'label'       => __( 'Heading (optional)', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => __( 'e.g. Most popular', 'luwipress-amber' ),
		] );
		...
		$this->add_control( 'source', [
			...
			'default' => 'query',
			'options' => [
				'query'    => __( 'Auto query (by orderby)', 'luwipress-amber' ),
				'featured' => __( 'Featured products only', 'luwipress-amber' ),
				'manual'   => __( 'Manual ID list', 'luwipress-amber' ),
			],
		] );

		$this->add_control( 'manual_ids', [
			'label'       => __( 'Product IDs (comma or space separated)', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXTAREA,
			'default'     => '',
			'placeholder' => '123, 456, 789',
			'condition'   => [ 'source' => 'manual' ],
			'description' => __( 'Order in this list is preserved. Limit + Orderby ignored.', 'luwipress-amber' ),
		] );

		$this->add_control( 'limit', [
			'label'     => __( 'Number of products', 'luwipress-amber' ),
		...
		$this->add_control( 'orderby', [
			...
			'default'   => 'popularity',
			'options'   => [
				'popularity' => __( 'Popularity (sales count)', 'luwipress-amber' ),
				'rating'     => __( 'Average rating', 'luwipress-amber' ),
				'date'       => __( 'Newest first', 'luwipress-amber' ),
				'price'      => __( 'Price (low → high)', 'luwipress-amber' ),
				'price-desc' => __( 'Price (high → low)', 'luwipress-amber' ),
				'title'      => __( 'Title A → Z', 'luwipress-amber' ),
				'menu_order' => __( 'Menu order', 'luwipress-amber' ),
		...
		$this->add_control( 'category', [
			'label'       => __( 'Limit to category (slug, optional)', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'darbuka',
			'condition'   => [ 'source!' => 'manual' ],
			'description' => __( 'Single product_cat slug. Leave empty for all categories.', 'luwipress-amber' ),
		] );

		$this->add_control( 'exclude_out_of_stock', [
			'label'        => __( 'Hide out-of-stock', 'luwipress-amber' ),
		...
		$this->add_responsive_control( 'gap', [
			'label'      => __( 'Grid gap', 'luwipress-amber' ),
		...
		$this->add_control( 'extra_class', [
			'label'       => __( 'Extra wrapper class', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'tap-products',
			'description' => __( 'Preserves legacy CSS hooks when migrating from a [products] shortcode (e.g. "tap-products"). Optional.', 'luwipress-amber' ),
		] );
		...
		$this->add_control( 'heading_color', [
			'label'     => __( 'Heading color', 'luwipress-amber' ),
	...
	protected function render() {
		...
		if ( empty( $products ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div class="lwp-product-grid lwp-product-grid--empty"><em>' . esc_html__( '[Product Grid] No products match the current query.', 'luwipress-amber' ) . '</em></div>';
			}
			return;
		}
		...
		<li class="product lwp-pcard">
			<a href="<?php echo esc_url( $link ); ?>" class="lwp-pcard-link">
				<div class="lwp-pcard-media">
					...
					<?php if ( $onsale ) : ?>
						<span class="lwp-pcard-badge"><?php esc_html_e( 'Sale', 'luwipress-amber' ); ?></span>
					<?php endif; ?>
				</div>
