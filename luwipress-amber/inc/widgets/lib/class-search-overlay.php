...
class LuwiPress_Amber_Widget_Search_Overlay extends Widget_Base {

	public function get_name()        { return 'lwp-search-overlay'; }
	public function get_title()       { return __( 'Search Overlay', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		/* Search panel */
		$this->start_controls_section( 'section_search', [ 'label' => __( 'Search panel', 'luwipress-amber' ) ] );

		$this->add_control( 'eyebrow',     [ 'label' => __( 'Eyebrow', 'luwipress-amber' ),     'type' => Controls_Manager::TEXT, 'default' => __( 'Search', 'luwipress-amber' ) ] );
		$this->add_control( 'placeholder', [ 'label' => __( 'Placeholder', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Try a product, master, or category…', 'luwipress-amber' ) ] );
		$this->add_control(
			'search_action_url',
			[
				'label'   => __( 'Form action URL', 'luwipress-amber' ),
				'description' => __( 'Where the form submits. Default is the site root with ?s=…', 'luwipress-amber' ),
		...
		$this->add_control(
			'search_param',
			[
				'label'   => __( 'Search query param', 'luwipress-amber' ),
				'description' => __( 'WP default = "s". WooCommerce-specific can use "post_type=product&s".', 'luwipress-amber' ),
		...
		$rep_block->add_control( 'label', [ 'label' => __( 'Block label', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => 'Trending' ] );
		$rep_block->add_control(
			'chips',
			[
				'label' => __( 'Chips (label|url, one per line)', 'luwipress-amber' ),
				'description' => __( 'Each line: label|url. Example: "Turkish Oud|/?s=turkish+oud"', 'luwipress-amber' ),
		...
		$this->add_control(
			'suggest_blocks',
			[
				'label'       => __( 'Suggestion blocks', 'luwipress-amber' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $rep_block->get_controls(),
				'title_field' => '{{{ label || "Block" }}}',
				'default'     => [
					[ 'label' => __( 'Trending', 'luwipress-amber' ), 'chips' => "Turkish Oud|/?s=turkish+oud\nHandpan|/?s=handpan\nNey|/?s=ney" ],
					[ 'label' => __( 'Categories', 'luwipress-amber' ), 'chips' => "String|/category/string\nPercussions|/category/percussions" ],
				],
			]
		);
		...
		/* Mobile drawer */
		$this->start_controls_section( 'section_mobile', [ 'label' => __( 'Mobile drawer', 'luwipress-amber' ) ] );

		$this->add_control( 'show_mobile_drawer', [ 'label' => __( 'Show mobile drawer', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
		...
		$menus    = $menus ?: [ '' => __( '— No menus found —', 'luwipress-amber' ) ];
		$this->add_control(
			'mobile_menu_id',
			[
				'label'     => __( 'Mobile menu', 'luwipress-amber' ),
				'type'      => Controls_Manager::SELECT,
				'options'   => array_merge( [ '' => __( '— Choose —', 'luwipress-amber' ) ], $menus ),
		...
		$this->add_control(
			'mobile_drawer_title',
			[
				'label' => __( 'Drawer header label', 'luwipress-amber' ),
				'type'  => Controls_Manager::TEXT,
				'default' => 'Menu',
				'condition' => [ 'show_mobile_drawer' => 'yes' ],
			]
		...
		$this->add_control(
			'overlay_bg',
			[ 'label' => __( 'Overlay background', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => 'rgba(12,12,12,0.92)',
				'selectors' => [ '{{WRAPPER}} .lwp-overlay' => 'background: {{VALUE}};' ] ]
		);
		$this->add_control(
			'panel_bg',
			[ 'label' => __( 'Panel background', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#FAF6EE',
				'selectors' => [ '{{WRAPPER}} .lwp-search__panel' => 'background: {{VALUE}};' ] ]
		);
		$this->add_control(
			'chip_bg',
			[ 'label' => __( 'Chip background', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#ffffff',
				'selectors' => [ '{{WRAPPER}} .lwp-search__chip' => 'background: {{VALUE}};' ] ]
		);
		$this->add_control(
			'accent',
			[ 'label' => __( 'Accent', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#9A7B3A',
				'selectors' => [
	...
	protected function render() {
		...
		<div class="lwp-overlay lwp-overlay--search" data-lwp-overlay="search" hidden>
			<div class="lwp-search__panel" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Search', 'luwipress-amber' ); ?>">
				<button class="lwp-overlay__close" type="button" data-lwp-overlay-close aria-label="<?php esc_attr_e( 'Close', 'luwipress-amber' ); ?>">×</button>
				<?php if ( $eyebrow ) : ?>
				...
				<aside class="lwp-drawer">
					...
					<div class="lwp-drawer__body">
						...
						if ( $menu_id ) {
							...
							echo '<p class="lwp-drawer__empty">' . esc_html__( 'Pick a menu in the widget settings.', 'luwipress-amber' ) . '</p>';
						}
						?>
					</div>
				</aside>
