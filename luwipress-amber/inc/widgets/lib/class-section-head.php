...
class LuwiPress_Amber_Widget_Section_Head extends Widget_Base {

	public function get_name()        { return 'lwp-section-head'; }
	public function get_title()       { return __( 'Section Head', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Use Enter for a multi-line title. Wrap a phrase with [em]…[/em] to italicise + gold-tint it.', 'luwipress-amber' ),
		...
		$this->add_control(
			'heading_tag',
			[
				'label'   => __( 'Heading tag', 'luwipress-amber' ),
		...
		$this->start_controls_section( 'section_pills', [ 'label' => __( 'Pill tabs (optional)', 'luwipress-amber' ) ] );

		$rep = new Repeater();
		$rep->add_control(
			'label',
			[ 'label' => __( 'Label', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ]
		);
		$rep->add_control(
			'url',
		...
		$rep->add_control(
			'active',
			[
				'label'        => __( 'Mark as active', 'luwipress-amber' ),
		...
		$this->add_control(
			'pills',
			[
				'label'       => __( 'Pills', 'luwipress-amber' ),
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Light = dark text on light bg. Dark = light text on dark bg (e.g. for the masters section).', 'luwipress-amber' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'light',
				'options' => [
					'light' => __( 'Light', 'luwipress-amber' ),
					'dark'  => __( 'Dark', 'luwipress-amber' ),
				],
			]
		);
		$this->end_controls_section();
