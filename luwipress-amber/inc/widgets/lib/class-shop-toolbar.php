...
class LuwiPress_Amber_Widget_Shop_Toolbar extends Widget_Base {

	public function get_name()        { return 'lwp-shop-toolbar'; }
	public function get_title()       { return __( 'Shop Toolbar', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Toolbar', 'luwipress-amber' ) ] );

		$this->add_control( 'show_count',  [ 'label' => __( 'Show result count', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
		$this->add_control(
			'count_template',
			[
				'label'   => __( 'Count template', 'luwipress-amber' ),
				'description' => __( 'Use %d for number, %s for filter text.', 'luwipress-amber' ),
				'type'    => Controls_Manager::TEXT,
				'default' => __( '%d results', 'luwipress-amber' ),
				'condition' => [ 'show_count' => 'yes' ],
			]
		);

		$this->add_control( 'show_view_toggle', [ 'label' => __( 'Show grid/list toggle', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
		$this->add_control( 'show_sort',         [ 'label' => __( 'Show sort dropdown', 'luwipress-amber' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
		...
		$this->add_control( 'text_color', [ 'label' => __( 'Text color', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#3a342c',
			'selectors' => [ '{{WRAPPER}} .lwp-stb' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'count_strong', [ 'label' => __( 'Count strong color', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
			'selectors' => [ '{{WRAPPER}} .lwp-stb__count strong' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'active_bg', [ 'label' => __( 'Toggle active bg', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
	...
	private function get_wc_sort_options() {
		...
		$options = [
			'menu_order' => __( 'Default sorting', 'luwipress-amber' ),
			'popularity' => __( 'Sort by popularity', 'luwipress-amber' ),
			'rating'     => __( 'Sort by average rating', 'luwipress-amber' ),
			'date'       => __( 'Sort by latest', 'luwipress-amber' ),
			'price'      => __( 'Sort by price: low to high', 'luwipress-amber' ),
			'price-desc' => __( 'Sort by price: high to low', 'luwipress-amber' ),
		];
	...
	protected function render() {
		...
		<div class="lwp-stb">
			...
			<div class="lwp-stb__tools">
				<?php if ( $show_view ) : ?>
					<div class="lwp-stb__view" role="group" aria-label="<?php esc_attr_e( 'View mode', 'luwipress-amber' ); ?>">
						<button type="button" class="lwp-stb__view-btn is-on" data-lwp-view="grid" aria-label="<?php esc_attr_e( 'Grid view', 'luwipress-amber' ); ?>">
						...
						<button type="button" class="lwp-stb__view-btn" data-lwp-view="list" aria-label="<?php esc_attr_e( 'List view', 'luwipress-amber' ); ?>">
