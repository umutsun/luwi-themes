...
class LuwiPress_Amber_Widget_Story_Split extends Widget_Base {

	public function get_name()        { return 'lwp-story-split'; }
	public function get_title()       { return __( 'Story Split', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'left',
				'options' => [
					'left'  => __( 'Image on left', 'luwipress-amber' ),
					'right' => __( 'Image on right', 'luwipress-amber' ),
				],
			]
		);
		...
		/* Image side */
		$this->start_controls_section( 'section_image', [ 'label' => __( 'Image tile', 'luwipress-amber' ) ] );
		...
		$this->add_control(
			'image_alt_grad_from',
			[
				'label'     => __( 'Gradient start (when no image)', 'luwipress-amber' ),
		...
		$this->add_control(
			'image_alt_grad_to',
			[
				'label'     => __( 'Gradient end (when no image)', 'luwipress-amber' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#7a5a2c',
			]
		);
		$this->add_control(
			'card_eyebrow',
			[
				'label'   => __( 'Floating card — eyebrow', 'luwipress-amber' ),
		...
		$this->add_control(
			'card_title',
			[
				'label'   => __( 'Floating card — title', 'luwipress-amber' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		$this->add_control(
			'card_sub',
			[
				'label'   => __( 'Floating card — sub line', 'luwipress-amber' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		...
		/* Story side */
		$this->start_controls_section( 'section_story', [ 'label' => __( 'Story', 'luwipress-amber' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Use Enter for multi-line. Wrap with [em]…[/em] for italic gold accent.', 'luwipress-amber' ),
		...
		$rep = new Repeater();
		$rep->add_control( 'title', [ 'label' => __( 'Title', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT ] );
		$rep->add_control( 'body',  [ 'label' => __( 'Body', 'luwipress-amber' ),  'type' => Controls_Manager::TEXTAREA, 'rows' => 2 ] );

		$this->add_control(
			'bullets',
			[
				'label'       => __( 'Numbered bullets', 'luwipress-amber' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $rep->get_controls(),
				'title_field' => '{{{ title || "Bullet" }}}',
				'default'     => [
					[ 'title' => __( 'Quality first.',          'luwipress-amber' ), 'body' => __( 'A short paragraph that explains your first key value.', 'luwipress-amber' ) ],
					[ 'title' => __( 'Direct from us to you.', 'luwipress-amber' ), 'body' => __( 'Second value proposition — replace this copy from the widget editor.', 'luwipress-amber' ) ],
					[ 'title' => __( 'Worldwide shipping.',     'luwipress-amber' ), 'body' => __( 'Third value — what makes your delivery / fulfillment story strong.', 'luwipress-amber' ) ],
					[ 'title' => __( 'After-sale support.',     'luwipress-amber' ), 'body' => __( 'Fourth value — what happens after the customer buys.', 'luwipress-amber' ) ],
				],
			]
		);
