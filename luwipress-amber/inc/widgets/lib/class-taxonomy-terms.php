...
class LuwiPress_Amber_Widget_Taxonomy_Terms extends Widget_Base {

	public function get_name()        { return 'lwp-taxonomy-terms'; }
	public function get_title()       { return __( 'Taxonomy Terms', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'taxonomy', [
			'label'   => __( 'Taxonomy', 'luwipress-amber' ),
		...
		$this->add_control( 'scope', [
			'label'   => __( 'Which terms', 'luwipress-amber' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'top_level',
			'options' => [
				'top_level'   => __( 'Top-level only (no children)', 'luwipress-amber' ),
				'all'         => __( 'All terms (flat)', 'luwipress-amber' ),
				'children_of' => __( 'Children of a specific term', 'luwipress-amber' ),
			],
		] );

		$this->add_control( 'parent_term', [
			'label'       => __( 'Parent term (ID or slug)', 'luwipress-amber' ),
		...
		$this->add_control( 'hide_empty', [
			'label'        => __( 'Hide empty terms', 'luwipress-amber' ),
		...
		$this->add_control( 'orderby', [
			...
			'default' => 'count',
			'options' => [
				'count'      => __( 'Product / post count', 'luwipress-amber' ),
				'name'       => __( 'Name (A→Z)', 'luwipress-amber' ),
				'slug'       => __( 'Slug', 'luwipress-amber' ),
				'term_order' => __( 'Manual term order', 'luwipress-amber' ),
				'menu_order' => __( 'Menu order (WC)', 'luwipress-amber' ),
			],
		] );
		...
		$this->add_control( 'limit', [
			'label'   => __( 'Max terms', 'luwipress-amber' ),
		...
		$this->add_control( 'include_slugs', [
			'label'       => __( 'Only these slugs (optional)', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'oud, darbuka, qanun',
			'description' => __( 'Comma-separated. Overrides scope/order — shows exactly these, in this order.', 'luwipress-amber' ),
		] );

		$this->add_control( 'exclude_slugs', [
			'label'       => __( 'Exclude these slugs (optional)', 'luwipress-amber' ),
			'type'        => Controls_Manager::TEXT,
		...
		$this->add_control( 'count_label', [
			...
			'type'        => Controls_Manager::TEXT,
			'default'     => __( 'items', 'luwipress-amber' ),
			'description' => __( 'e.g. "items" → "11 items". Leave empty to hide the count line.', 'luwipress-amber' ),
		] );

		$this->add_control( 'show_image', [
			'label'        => __( 'Show term image', 'luwipress-amber' ),
			'description'  => __( 'Uses the term thumbnail (WooCommerce category/brand images). Falls back to a gold gradient.', 'luwipress-amber' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => 'yes',
			'default'      => 'yes',
		] );
		...
		$this->add_control( 'show_description', [
			'label'        => __( 'Show description sub-line', 'luwipress-amber' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => 'yes',
			'default'      => '',
		] );
	...
	protected function render() {
		...
		if ( empty( $terms ) ) {
			if ( class_exists( '\\Elementor\\Plugin' ) && \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div class="lwp-cgrid lwp-cgrid--empty"><em>' . esc_html__( '[Taxonomy Terms] No terms match the current settings.', 'luwipress-amber' ) . '</em></div>';
			}
			return;
		}
