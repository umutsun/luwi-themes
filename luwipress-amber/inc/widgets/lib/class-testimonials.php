...
class LuwiPress_Amber_Widget_Testimonials extends Widget_Base {

	public function get_name()        { return 'lwp-testimonials'; }
	public function get_title()       { return __( 'Testimonials', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_items', [ 'label' => __( 'Reviews', 'luwipress-amber' ) ] );

		$rep = new Repeater();
		$rep->add_control( 'quote',    [ 'label' => __( 'Quote', 'luwipress-amber' ),    'type' => Controls_Manager::TEXTAREA, 'rows' => 4, 'default' => '' ] );
		$rep->add_control( 'name',     [ 'label' => __( 'Author name', 'luwipress-amber' ),  'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'location', [ 'label' => __( 'Location', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'avatar',   [ 'label' => __( 'Avatar (optional)', 'luwipress-amber' ), 'type' => Controls_Manager::MEDIA, 'default' => [ 'url' => '' ] ] );
		$rep->add_control( 'product',  [ 'label' => __( 'Product purchased (optional)', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'rating',   [
			'label' => __( 'Star rating', 'luwipress-amber' ),
			'type'  => Controls_Manager::SELECT,
			'default' => '5',
			'options' => [ '5' => '★★★★★ (5)', '4' => '★★★★☆ (4)', '3' => '★★★☆☆ (3)', '2' => '★★☆☆☆ (2)', '1' => '★☆☆☆☆ (1)' ],
		] );
		$rep->add_control( 'date', [ 'label' => __( 'Review date (YYYY-MM-DD, optional)', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );

		$this->add_control( 'reviews', [
			'label'       => __( 'Reviews', 'luwipress-amber' ),
			'type'        => Controls_Manager::REPEATER,
			'fields'      => $rep->get_controls(),
			'title_field' => '{{{ name || "Review" }}}',
			'default'     => [
				[ 'quote' => __( 'Replace this with a real customer review — it is the single biggest conversion lever you control.', 'luwipress-amber' ), 'name' => __( 'Customer name', 'luwipress-amber' ), 'location' => '', 'product' => '', 'rating' => '5' ],
				[ 'quote' => __( 'Add a second testimonial here. Quotes that mention specific products tend to convert best.', 'luwipress-amber' ),       'name' => __( 'Customer name', 'luwipress-amber' ), 'location' => '', 'product' => '', 'rating' => '5' ],
				[ 'quote' => __( 'A third quote rounds out the social-proof block. Photo + first name + city + product gives the most credibility.', 'luwipress-amber' ), 'name' => __( 'Customer name', 'luwipress-amber' ), 'location' => '', 'product' => '', 'rating' => '5' ],
			],
		] );

		$this->add_control( 'layout', [
			'label'   => __( 'Layout', 'luwipress-amber' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'carousel',
			'options' => [
				'carousel' => __( 'Carousel (scroll-snap)', 'luwipress-amber' ),
				'grid'     => __( 'Grid (3-up desktop)', 'luwipress-amber' ),
			],
		] );

		$this->add_control( 'show_schema', [
			'label'        => __( 'Output Review schema', 'luwipress-amber' ),
			'description'  => __( 'Outputs Schema.org Review JSON-LD so Google + Rank Math can show stars in SERP. Disable if your SEO plugin handles it separately.', 'luwipress-amber' ),
	...
	protected function render_stars( $rating ) {
		...
		$out = '<span class="lwp-tst__stars" aria-label="' . esc_attr( sprintf( __( '%d out of 5 stars', 'luwipress-amber' ), $r ) ) . '">';
		for ( $i = 1; $i <= 5; $i++ ) {
	...
	protected function render() {
		...
		<div class="lwp-tst" data-layout="<?php echo esc_attr( $layout ); ?>">
			...
			<?php if ( $layout === 'carousel' ) : ?>
				<div class="lwp-tst__dots" role="tablist" aria-label="<?php esc_attr_e( 'Testimonials navigation', 'luwipress-amber' ); ?>"></div>
			<?php endif; ?>
		</div>
		<?php
	}
