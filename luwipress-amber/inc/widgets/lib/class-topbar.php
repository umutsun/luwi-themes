...
class LuwiPress_Amber_Widget_Topbar extends Widget_Base {

	public function get_name()        { return 'lwp-topbar'; }
	public function get_title()       { return __( 'Topbar', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		/* Left items */
		$this->start_controls_section( 'section_left', [ 'label' => __( 'Left items', 'luwipress-amber' ) ] );

		$rep_l = new Repeater();
		$rep_l->add_control( 'text', [ 'label' => __( 'Text', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep_l->add_control( 'url',  [ 'label' => __( 'URL (optional)', 'luwipress-amber' ), 'type' => Controls_Manager::URL, 'default' => [ 'url' => '' ] ] );

		$this->add_control(
			'left_items',
			[
				'label'       => __( 'Left items', 'luwipress-amber' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $rep_l->get_controls(),
				'title_field' => '{{{ text || "Item" }}}',
				'default'     => [
					[ 'text' => __( '— Atelier', 'luwipress-amber' ), 'url' => [ 'url' => '' ] ],
		...
		/* Language switcher */
		$this->start_controls_section( 'section_lang', [ 'label' => __( 'Language switcher', 'luwipress-amber' ) ] );

		$this->add_control(
			'show_lang_switcher',
			[
				'label'        => __( 'Show language switcher', 'luwipress-amber' ),
		...
		$this->add_control(
			'lang_mode',
			[
				'label'   => __( 'Switcher source', 'luwipress-amber' ),
				'description' => __( 'Auto = read from WPML/Polylang if active. Manual = use the langs repeater below.', 'luwipress-amber' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'auto',
				'options' => [ 'auto' => __( 'Auto (WPML/Polylang)', 'luwipress-amber' ), 'manual' => __( 'Manual', 'luwipress-amber' ) ],
				'condition' => [ 'show_lang_switcher' => 'yes' ],
			]
		);

		$rep_lang = new Repeater();
		$rep_lang->add_control( 'code',  [ 'label' => __( 'Code', 'luwipress-amber' ),  'type' => Controls_Manager::TEXT, 'default' => 'EN' ] );
		$rep_lang->add_control( 'url',   [ 'label' => __( 'URL', 'luwipress-amber' ),   'type' => Controls_Manager::URL,  'default' => [ 'url' => '/' ] ] );
		$rep_lang->add_control( 'active',[ 'label' => __( 'Active', 'luwipress-amber' ),'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => '' ] );

		$this->add_control(
			'langs',
			[
				'label'       => __( 'Manual languages', 'luwipress-amber' ),
				'type'        => Controls_Manager::REPEATER,
		...
		/* Right utility links */
		$this->start_controls_section( 'section_right', [ 'label' => __( 'Right utility links', 'luwipress-amber' ) ] );

		$rep_r = new Repeater();
		$rep_r->add_control( 'label', [ 'label' => __( 'Label', 'luwipress-amber' ), 'type' => Controls_Manager::TEXT, 'default' => 'Track order →' ] );
		$rep_r->add_control( 'url',   [ 'label' => __( 'URL', 'luwipress-amber' ),   'type' => Controls_Manager::URL,  'default' => [ 'url' => '/track-order' ] ] );

		$this->add_control(
			'right_links',
			[
				'label'       => __( 'Right links', 'luwipress-amber' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $rep_r->get_controls(),
				'title_field' => '{{{ label || "Link" }}}',
				'default'     => [ [ 'label' => __( 'Track order →', 'luwipress-amber' ), 'url' => [ 'url' => '/track-order' ] ] ],
			]
		);
		$this->end_controls_section();
		...
		$this->add_control(
			'text_color',
			[ 'label' => __( 'Text color', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#bbb',
				'selectors' => [ '{{WRAPPER}} .lwp-topbar' => 'color: {{VALUE}};' ] ]
		);
		$this->add_control(
			'accent_color',
			[ 'label' => __( 'Accent (active / hover)', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#D4AF37',
				'selectors' => [
