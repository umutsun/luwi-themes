...
class LuwiPress_Amber_Widget_WC_Gallery extends Widget_Base {

	public function get_name()        { return 'lwp-wc-gallery'; }
	public function get_title()       { return __( 'WC Product Gallery', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Gallery', 'luwipress-amber' ) ] );

		$this->add_control( 'enable_zoom', [
			'label' => __( 'Enable hover zoom', 'luwipress-amber' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		$this->add_control( 'enable_lightbox', [
			'label' => __( 'Enable click-to-lightbox', 'luwipress-amber' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		$this->add_control( 'image_size', [
			'label'   => __( 'Main image size', 'luwipress-amber' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'woocommerce_single',
			'options' => [
				'woocommerce_single'    => __( 'WC Single (default)', 'luwipress-amber' ),
				'large'                 => __( 'Large', 'luwipress-amber' ),
				'full'                  => __( 'Full', 'luwipress-amber' ),
			],
		] );
		...
		$this->add_responsive_control( 'main_max_height', [
			'label' => __( 'Main image max height', 'luwipress-amber' ),
		...
		$this->add_control( 'main_border_radius', [
			'label' => __( 'Main image radius', 'luwipress-amber' ),
		...
		$this->add_responsive_control( 'thumb_size', [
			'label' => __( 'Thumb size', 'luwipress-amber' ),
			'type'  => Controls_Manager::SLIDER,
			'size_units' => [ 'px' ],
		...
		$this->add_control( 'thumb_radius', [
			'label' => __( 'Thumb radius', 'luwipress-amber' ),
			'type'  => Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px' ],
	...
	protected function render() {
		...
		if ( ! $product ) {
			echo '<div class="lwp-wc-gallery lwp-wc-gallery--placeholder"><div class="lwp-wc-gallery__ph">' . esc_html__( '[Product gallery]', 'luwipress-amber' ) . '</div></div>';
			return;
		}
		...
		<div class="<?php echo esc_attr( implode( ' ', $wrapper_classes ) ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 1; transition: opacity .25s ease-in-out;">
			<figure class="woocommerce-product-gallery__wrapper">
				...
				if ( $post_thumbnail ) {
					...
					echo '<div class="woocommerce-product-gallery__image woocommerce-product-gallery__image--placeholder"><a href="' . esc_url( $placeholder ) . '"><img src="' . esc_url( $placeholder ) . '" alt="' . esc_attr__( 'Placeholder', 'luwipress-amber' ) . '" /></a></div>';
				}
