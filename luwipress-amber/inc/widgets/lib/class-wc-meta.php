...
class LuwiPress_Amber_Widget_WC_Meta extends Widget_Base {

	public function get_name()        { return 'lwp-wc-meta'; }
	public function get_title()       { return __( 'WC Product Meta', 'luwipress-amber' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'show_sku', [
			'label' => __( 'Show SKU', 'luwipress-amber' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		$this->add_control( 'sku_label', [
			'label' => __( 'SKU label', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT, 'default' => __( 'SKU', 'luwipress-amber' ),
			'condition' => [ 'show_sku' => 'yes' ],
		] );

		$this->add_control( 'show_categories', [
			'label' => __( 'Show categories', 'luwipress-amber' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		$this->add_control( 'categories_label', [
			'label' => __( 'Categories label', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT, 'default' => __( 'Categories', 'luwipress-amber' ),
			'condition' => [ 'show_categories' => 'yes' ],
		] );

		$this->add_control( 'show_tags', [
			'label' => __( 'Show tags', 'luwipress-amber' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => '',
		] );
		$this->add_control( 'tags_label', [
			'label' => __( 'Tags label', 'luwipress-amber' ),
			'type'  => Controls_Manager::TEXT, 'default' => __( 'Tags', 'luwipress-amber' ),
		...
		$this->add_control( 'value_color', [
			'label' => __( 'Value color', 'luwipress-amber' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
	...
	protected function render() {
		...
		if ( ( $s['show_sku'] ?? '' ) === 'yes' && $product->get_sku() ) {
			$rows[] = [
				'label' => $s['sku_label'] ?? __( 'SKU', 'luwipress-amber' ),
		...
		if ( ( $s['show_categories'] ?? '' ) === 'yes' ) {
			...
			if ( $cats ) {
				$rows[] = [
					'label' => $s['categories_label'] ?? __( 'Categories', 'luwipress-amber' ),
		...
		if ( ( $s['show_tags'] ?? '' ) === 'yes' ) {
			...
			if ( $tags ) {
				$rows[] = [
					'label' => $s['tags_label'] ?? __( 'Tags', 'luwipress-amber' ),
					'value' => $tags,
				];
			}
		}
