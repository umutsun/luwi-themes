 ...
 * `Appearance → LuwiPress Amber` admin page, and shows a one-time activation
...
if ( ! class_exists( 'LuwiPress_Amber_Wizard' ) ) :

class LuwiPress_Amber_Wizard {

	const DISMISSED_OPTION = 'luwipress_amber_wizard_dismissed';
	const COMPLETED_OPTION = 'luwipress_amber_wizard_completed';
	const NONCE            = 'luwipress_amber_wizard';
	const SLUG             = 'luwipress-amber-wizard';
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_luwipress_amber_wizard_step', [ $this, 'handle_ajax_step' ] );
		add_action( 'wp_ajax_luwipress_amber_wizard_dismiss', [ $this, 'handle_ajax_dismiss' ] );
	}
	...
	public function register_admin_page() {
		add_submenu_page(
			'',                                    // hidden — no parent
			__( 'LuwiPress Amber setup', 'luwipress-amber' ),
		...
		add_management_page(
			__( 'LuwiPress Amber setup (re-run)', 'luwipress-amber' ),
			__( 'LuwiPress Amber', 'luwipress-amber' ),
			self::CAPABILITY,
			self::SLUG,
			[ $this, 'render_page' ]
	...
	public function render_activation_notice() {
		...
		?>
		<div class="notice notice-info luwipress-amber-notice is-dismissible" data-lwp-wizard-notice>
			<p style="font-size:14px">
				<strong style="font-family:'Playfair Display',Georgia,serif;font-size:18px;">
					<?php esc_html_e( 'Welcome to LuwiPress Amber.', 'luwipress-amber' ); ?>
				</strong>
				<br>
				<?php echo esc_html( $summary ); ?>
				<?php esc_html_e( 'Run the setup wizard — we\'ll detect what you have and adapt the theme to it (no data is overwritten).', 'luwipress-amber' ); ?>
			</p>
			<p>
				<a href="<?php echo $url; ?>" class="button button-primary">
					<?php esc_html_e( 'Start setup wizard', 'luwipress-amber' ); ?>
				</a>
				<a href="#" class="button button-secondary" data-lwp-wizard-dismiss>
					<?php esc_html_e( 'Skip for now', 'luwipress-amber' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	public function enqueue_assets( $hook ) {
		...
		wp_enqueue_style(
			'luwipress-amber-wizard-notice',
			LUWIPRESS_AMBER_URI . '/assets/wizard-css/notice.css',
			[],
			LUWIPRESS_AMBER_VERSION
		);
		wp_enqueue_script(
			'luwipress-amber-wizard-notice',
			LUWIPRESS_AMBER_URI . '/assets/wizard-js/notice.js',
			[ 'jquery' ],
			LUWIPRESS_AMBER_VERSION,
			true
		);
		wp_localize_script( 'luwipress-amber-wizard-notice', 'LWP_AMBER_WIZARD', [
		...
		wp_enqueue_style(
			'luwipress-amber-wizard',
			LUWIPRESS_AMBER_URI . '/assets/wizard-css/wizard.css',
			[ 'luwipress-amber-wizard-notice' ],
			LUWIPRESS_AMBER_VERSION
		);
		wp_enqueue_script(
			'luwipress-amber-wizard',
			LUWIPRESS_AMBER_URI . '/assets/wizard-js/wizard.js',
			[ 'jquery', 'luwipress-amber-wizard-notice' ],
			LUWIPRESS_AMBER_VERSION,
			true
		);
		wp_localize_script( 'luwipress-amber-wizard', 'LWP_AMBER_WIZARD_DATA', [
			'restUrl' => esc_url_raw( rest_url() ),
			'i18n'    => [
				'detecting'  => __( 'Scanning your site…', 'luwipress-amber' ),
				'mapping'    => __( 'Mapping content to templates…', 'luwipress-amber' ),
				'importing'  => __( 'Importing templates…', 'luwipress-amber' ),
				'finishing'  => __( 'Almost there…', 'luwipress-amber' ),
				'completed'  => __( 'All done!', 'luwipress-amber' ),
				'errored'    => __( 'Something went wrong. Check the WP debug log.', 'luwipress-amber' ),
			],
		] );
	}
	...
	public function handle_ajax_step() {
		...
		switch ( $step ) {
			case 'detect':
				$d = new LuwiPress_Amber_Detector();
				wp_send_json_success( [
					'snapshot' => $d->snapshot(),
			...
			case 'map':
				...
				$d = new LuwiPress_Amber_Detector();
				$snap = $d->snapshot();
				$mapper = new LuwiPress_Amber_Mapper();
			...
			case 'ai_status':
				wp_send_json_success( [
					'available' => LuwiPress_Amber_AI_Content::is_luwipress_available(),
					'enabled'   => LuwiPress_Amber_AI_Content::is_enabled(),
					'slots'     => LuwiPress_Amber_AI_Content::slots(),
					'meta'      => get_option( LuwiPress_Amber_AI_Content::META_OPTION, [] ),
				] );

			case 'ai_toggle':
				$enable = ! empty( $payload['enable'] );
				update_option( LuwiPress_Amber_AI_Content::ENABLED_OPTION, $enable ? 1 : 0 );
				if ( ! $enable ) {
					LuwiPress_Amber_AI_Content::flush_cache();
				}
				wp_send_json_success( [ 'enabled' => $enable ] );

			case 'ai_generate':
				if ( ! LuwiPress_Amber_AI_Content::is_luwipress_available() ) {
					wp_send_json_error( [ 'message' => 'LuwiPress AI engine not installed.' ] );
				}
				update_option( LuwiPress_Amber_AI_Content::ENABLED_OPTION, 1 );
				$results = [];
				foreach ( LuwiPress_Amber_AI_Content::slots() as $slot => $cfg ) {
					$text = LuwiPress_Amber_AI_Content::resolve( $slot );
					$results[ $slot ] = [
						'label' => $cfg['label'],
						'text'  => $text,
			...
			case 'apply':
				...
				$importer = new LuwiPress_Amber_Importer();
...
LuwiPress_Amber_Wizard::instance();
