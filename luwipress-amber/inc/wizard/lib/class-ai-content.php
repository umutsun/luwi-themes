 ...
 *
 * Cache key: `lwp_amber_ai_<slot>_<context_hash>` so the operator
 * regenerates simply by clearing transients (which the wizard offers).
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

class LuwiPress_Amber_AI_Content {

	const CACHE_TTL = WEEK_IN_SECONDS;
	const ENABLED_OPTION = 'luwipress_amber_ai_enabled';
	const PROVIDER_OPTION = 'luwipress_amber_ai_provider';
	const META_OPTION = 'luwipress_amber_ai_meta';
	...
	public static function slots() {
		return [
			'hero_lead' => [
				'label'     => __( 'Hero — lead paragraph', 'luwipress-amber' ),
				'max_words' => 35,
			...
			'about_story' => [
				'label'     => __( 'About — short brand story', 'luwipress-amber' ),
				'max_words' => 90,
			...
			'master_bio' => [
				'label'     => __( 'Master profile — short bio', 'luwipress-amber' ),
				'max_words' => 50,
			...
			'faq_intro' => [
				'label'     => __( 'Contact — FAQ intro', 'luwipress-amber' ),
				'max_words' => 25,
			...
			'shipping_blurb' => [
				'label'     => __( 'Info-bar — shipping note', 'luwipress-amber' ),
				'max_words' => 12,
	...
	private static function site_context() {
		...
		$sector_blob = ! empty( $sectors )
			? implode( ', ', $sectors )
			: ( $tagline ?: __( 'hand-crafted goods', 'luwipress-amber' ) );
	...
	private static function cache_key( $slot, $context = [] ) {
		...
		return 'lwp_amber_ai_' . $slot . '_' . substr( md5( $blob ), 0, 12 );
	}
