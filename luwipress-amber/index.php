...
<main class="lwp-fallback-main" id="primary">
	<header class="lwp-fallback-archive__head">
		<h1>
			...
			if ( is_search() ) {
				...
				printf( esc_html__( 'Search results for: %s', 'luwipress-amber' ), '<em>' . esc_html( get_search_query() ) . '</em>' );
			} elseif ( is_archive() ) {
				the_archive_title();
			} else {
				bloginfo( 'name' );
	...
	<?php if ( have_posts() ) : ?>
				...
				<article class="lwp-fallback-card">
					<?php if ( has_post_thumbnail() ) : ?>
						<a class="lwp-fallback-card__thumb" href="<?php the_permalink(); ?>">
							<?php the_post_thumbnail( 'luwipress-amber-editorial' ); ?>
						</a>
					<?php endif; ?>
		...
		?>
	<?php else : ?>
		<p><?php esc_html_e( 'Nothing here yet.', 'luwipress-amber' ); ?></p>
	<?php endif; ?>
</main>
<?php
get_footer();
