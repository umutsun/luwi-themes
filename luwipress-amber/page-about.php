 ...
 *
 * Each block can also be filtered via `luwipress_amber_about_*`.
 */
...
if ( ! $eyebrow_text ) {
	$eyebrow_text = apply_filters( 'luwipress_amber_about_eyebrow', __( 'Brisighella · Italy', 'luwipress-amber' ) );
}
...
$timeline = apply_filters( 'luwipress_amber_about_timeline', $timeline );

$team = function_exists( 'get_field' ) ? (array) get_field( 'about_team', $post_id ) : [];
$team = apply_filters( 'luwipress_amber_about_team', $team );

$gallery = function_exists( 'get_field' ) ? (array) get_field( 'about_gallery', $post_id ) : [];
$gallery = apply_filters( 'luwipress_amber_about_gallery', $gallery );
?>
...
<main class="lwp-page page-about" id="primary">
	...
	<section style="padding:24px 20px;display:flex;flex-direction:column;gap:14px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( '01 · The mission', 'luwipress-amber' ); ?></span>
	...
	<section style="background:var(--bg-alt);padding:24px 20px;display:flex;flex-direction:column;gap:18px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( '02 · Timeline', 'luwipress-amber' ); ?></span>
		<h2 style="font-family:var(--serif);font-size:22px;font-weight:500;line-height:1.25;letter-spacing:-.005em;margin:0"><?php
			echo wp_kses(
				__( 'Years on the bench, <em>line by line</em>', 'luwipress-amber' ),
				[ 'em' => [] ]
			);
		?></h2>
	...
	<?php if ( $team ) : ?>
	<section style="padding:24px 20px;display:flex;flex-direction:column;gap:14px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( '03 · The team', 'luwipress-amber' ); ?></span>
		<h2 style="font-family:var(--serif);font-size:22px;font-weight:500;margin:0"><?php
			/* translators: editorial team grid heading */
			echo wp_kses(
				__( 'A few hands in <em>one room</em>', 'luwipress-amber' ),
				[ 'em' => [] ]
			);
		?></h2>
	...
	<section style="background:#15110b;color:#cfc7b3;padding:24px 20px;display:flex;flex-direction:column;gap:14px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:#a39c8e"><?php esc_html_e( '04 · Inside the atelier', 'luwipress-amber' ); ?></span>
		<h2 style="font-family:var(--serif);font-size:22px;font-weight:500;color:#fff;margin:0"><?php
			echo wp_kses(
				__( 'From <em>raw wood</em> to first pluck', 'luwipress-amber' ),
				[ 'em' => [] ]
			);
		?></h2>
