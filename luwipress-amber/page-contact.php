 ...
 * @since 1.5.4
 */
$reasons = apply_filters( 'luwipress_amber_contact_reasons', [
	[ 'label' => __( 'An instrument', 'luwipress-amber' ), 'value' => 'instrument' ],
	[ 'label' => __( 'A custom build', 'luwipress-amber' ), 'value' => 'custom' ],
	[ 'label' => __( 'An order', 'luwipress-amber' ), 'value' => 'order' ],
	[ 'label' => __( 'Lessons', 'luwipress-amber' ), 'value' => 'lessons' ],
	[ 'label' => __( 'Press', 'luwipress-amber' ), 'value' => 'press' ],
] );

$contact_rows = apply_filters( 'luwipress_amber_contact_rows', [
	[ 'ic' => '☎', 'label' => get_option( 'luwipress_amber_contact_phone', '' ), 'meta' => __( 'Mon–Sat', 'luwipress-amber' ), 'href' => 'tel:' ],
	[ 'ic' => '✉', 'label' => get_option( 'admin_email', '' ), 'meta' => __( '24h reply', 'luwipress-amber' ), 'href' => 'mailto:' ],
] );

$hours = apply_filters( 'luwipress_amber_contact_hours', [
	[ 'day' => __( 'Tue – Fri', 'luwipress-amber' ), 'val' => '10 – 18' ],
	[ 'day' => __( 'Saturday', 'luwipress-amber' ),   'val' => '10 – 13' ],
	[ 'day' => __( 'Sun & Mon', 'luwipress-amber' ),  'val' => __( 'Closed', 'luwipress-amber' ), 'closed' => true ],
] );
?>
...
<main class="lwp-page page-contact" id="primary">
	...
	<header style="padding:24px 20px 14px;display:flex;flex-direction:column;gap:10px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( 'Get in touch', 'luwipress-amber' ); ?></span>
	...
	<section style="padding:0 20px 4px">
		<span class="eyebrow" style="display:block;margin-bottom:8px;font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( "I'm writing about", 'luwipress-amber' ); ?></span>
		<div class="lwp-reason-chips" role="radiogroup" aria-label="<?php esc_attr_e( 'Reason for contact', 'luwipress-amber' ); ?>">
	...
	<section class="lwp-contact-form" style="padding:18px 20px;display:flex;flex-direction:column;gap:12px">
		<?php
		$form_html = apply_filters( 'luwipress_amber_contact_form_html', '' );
		if ( $form_html ) {
	...
	<?php
	$map_embed = apply_filters( 'luwipress_amber_contact_map_embed', '' );
	$map_lat   = apply_filters( 'luwipress_amber_contact_map_lat', '' );
	...
	<section class="lwp-hours" style="background:var(--bg-alt);padding:18px 20px;display:flex;flex-direction:column;gap:8px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( 'Opening hours', 'luwipress-amber' ); ?></span>
