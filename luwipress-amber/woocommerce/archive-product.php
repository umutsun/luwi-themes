...
<main class="lwp-page lwp-shop-archive" id="primary">
	<div class="lwp-page-container">
		 ...
		 */
		do_action( 'luwipress_amber_archive_intro_before', $is_term ? $current_obj : null );
		?>
		...
		<header class="lwp-archive-header<?php echo $header_image_html ? ' lwp-archive-header--split' : ''; ?>">
			<div class="lwp-archive-header__copy">
				<?php if ( $is_term ) : ?>
					...
					echo esc_html(
						$current_obj->parent
							? __( 'Sub-category', 'luwipress-amber' )
							: __( 'Collection', 'luwipress-amber' )
					);
					?></span>
				<?php endif; ?>
				 ...
				 */
				do_action( 'luwipress_amber_archive_intro_after', $is_term ? $current_obj : null );
				?>
			</div>
		...
		<?php if ( ! empty( $subcats ) ) : ?>
			 ...
			 */
			do_action( 'luwipress_amber_archive_subcat_tiles_before', $subcats, $current_obj );
			?>
			<section class="lwp-archive-subcat-grid" aria-label="<?php esc_attr_e( 'Sub-categories', 'luwipress-amber' ); ?>">
					...
					<a class="lwp-archive-subcat" href="<?php echo esc_url( get_term_link( $sc ) ); ?>">
						...
						<div class="lwp-archive-subcat__meta">
							...
							<span class="lwp-archive-subcat__count">
								...
								printf( esc_html( _n( '%d piece', '%d pieces', (int) $sc->count, 'luwipress-amber' ) ), (int) $sc->count );
								?>
							</span>
						</div>
					</a>
			...
			<?php
			do_action( 'luwipress_amber_archive_subcat_tiles_after', $subcats, $current_obj );
			?>
		<?php endif; ?>

		<div class="lwp-shop-grid-wrap">

			<aside class="lwp-shop-sidebar" aria-label="<?php esc_attr_e( 'Filters', 'luwipress-amber' ); ?>">
				<?php
				...
				} elseif ( function_exists( 'luwipress_amber_render_smart_filters' ) ) {
					luwipress_amber_render_smart_filters( $is_term ? $current_obj : null );
				}
				?>
			</aside>
					...
					if ( get_theme_mod( 'luwipress_amber_shop_loadmore', true ) ) {
						...
						if ( $max_pages > 1 ) {
							...
							printf(
								...
								$current_page,
								$max_pages,
								esc_html__( 'Load more products', 'luwipress-amber' ),
								esc_attr__( 'Loading', 'luwipress-amber' )
							);
						}
					}
				} else {
