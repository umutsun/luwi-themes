...
<form class="woocommerce-cart-form lwp-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<div class="lwp-cart-grid">
		<div class="lwp-cart-list" role="region" aria-label="<?php esc_attr_e( 'Cart items', 'luwipress-amber' ); ?>">
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				<div class="lwp-cart-row <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<div class="lwp-cart-row__name">
						...
						if ( $maker ) {
							echo '<small>' . sprintf( /* translators: maker name */ esc_html__( 'by %s', 'luwipress-amber' ), esc_html( $maker ) ) . '</small>';
						}
					...
					<div class="lwp-cart-row__remove">
						...
						echo apply_filters( // phpcs:ignore
							...
							sprintf(
								...
								esc_attr( sprintf( /* translators: %s: product name */ __( 'Remove %s from cart', 'luwipress-amber' ), wp_strip_all_tags( $_product->get_name() ) ) ),
								esc_attr( $product_id ),
			...
			<div class="lwp-cart-actions">
				<?php if ( wc_coupons_enabled() ) : ?>
					<div class="lwp-cart-coupon">
						<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'luwipress-amber' ); ?>" />
						<button type="submit" class="lwp-btn-secondary" name="apply_coupon" value="<?php esc_attr_e( 'Apply', 'luwipress-amber' ); ?>"><?php esc_html_e( 'Apply', 'luwipress-amber' ); ?></button>
						<?php do_action( 'woocommerce_cart_coupon' ); ?>
					</div>
				<?php endif; ?>
				<button type="submit" class="lwp-btn-secondary" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'luwipress-amber' ); ?>"><?php esc_html_e( 'Update cart', 'luwipress-amber' ); ?></button>
		...
		<aside class="lwp-cart-summary" aria-label="<?php esc_attr_e( 'Order summary', 'luwipress-amber' ); ?>">
...
?>
<div class="lwp-sticky-cart-bar" role="region" aria-label="<?php esc_attr_e( 'Cart total and checkout', 'luwipress-amber' ); ?>">
	<div class="px">
		<span class="lbl"><?php
			/* translators: %d: cart item count */
			printf( esc_html( _n( 'Total · %d item', 'Total · %d items', $cart_count, 'luwipress-amber' ) ), $cart_count );
		?></span>
		<span class="v"><?php echo wp_kses_post( $cart_total ); ?></span>
	</div>
	<a href="<?php echo esc_url( $checkout_url ); ?>" class="button"><?php esc_html_e( 'Checkout →', 'luwipress-amber' ); ?></a>
</div>
