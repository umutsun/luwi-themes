...
$page_title = ( 'billing' === $load_address ) ? __( 'Billing address', 'luwipress-amber' ) : __( 'Shipping address', 'luwipress-amber' );
?>

<h2><?php echo esc_html( $page_title ); ?></h2>
<p class="lwp-acct-sub">
	<?php esc_html_e( 'Update the details we use to ship and bill your orders. Required fields are marked with an asterisk.', 'luwipress-amber' ); ?>
</p>
...
<form method="post" class="lwp-acct-card lwp-edit-address-form woocommerce-EditAccountForm edit-account">
	...
	<div class="lwp-form-actions">
		...
		<button type="submit" class="lwp-btn-primary woocommerce-Button button" name="save_address" value="<?php esc_attr_e( 'Save address', 'luwipress-amber' ); ?>"><?php esc_html_e( 'Save address →', 'luwipress-amber' ); ?></button>
		<a href="<?php echo esc_url( wc_get_account_endpoint_url( 'edit-address' ) ); ?>" class="lwp-btn-link"><?php esc_html_e( '← Cancel', 'luwipress-amber' ); ?></a>
