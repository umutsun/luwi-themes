...
$get_addresses         = apply_filters( 'woocommerce_my_account_get_addresses', [
	'billing'  => __( 'Billing', 'luwipress-amber' ),
	'shipping' => __( 'Shipping', 'luwipress-amber' ),
], $customer_id );
...
<h2><?php esc_html_e( 'Saved addresses', 'luwipress-amber' ); ?></h2>
<p class="lwp-acct-sub"><?php esc_html_e( 'These addresses are used by default at checkout. You can update them at any time, or add a new address from the cart.', 'luwipress-amber' ); ?></p>

<?php if ( ! $ship_to_billing_only ) : ?>
	<p class="lwp-acct-sub" style="margin-top:-16px;font-size:13px;">
		<?php esc_html_e( 'The following addresses will be used on the checkout page by default.', 'luwipress-amber' ); ?>
	</p>
<?php endif; ?>
		...
		<div class="lwp-addr-card">
			<span class="lwp-addr-card__lbl">
				...
				if ( $is_default ) {
					...
					printf( esc_html__( 'Default · %s', 'luwipress-amber' ), esc_html( strtolower( $title ) ) );
				} else {
					echo esc_html( $title );
				}
				?>
			...
			<?php else : ?>
				<p class="lwp-addr-card__empty"><?php esc_html_e( 'You have not set up this type of address yet.', 'luwipress-amber' ); ?></p>
			<?php endif; ?>
			<a href="<?php echo esc_url( wc_get_endpoint_url( 'edit-address', $name ) ); ?>" class="lwp-addr-card__edit">
				<?php echo $address ? esc_html__( 'Edit address →', 'luwipress-amber' ) : esc_html__( 'Add address →', 'luwipress-amber' ); ?>
			</a>
		</div>
	<?php endforeach; ?>
</div>
