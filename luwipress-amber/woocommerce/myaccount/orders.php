...
<?php if ( $has_orders ) : ?>
	...
	<h2><?php esc_html_e( 'All orders', 'luwipress-amber' ); ?></h2>
	<p class="lwp-acct-sub"><?php esc_html_e( "Every order you've placed. Click any row to see line items, tracking and the invoice.", 'luwipress-amber' ); ?></p>
						...
						<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
							<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
									...
									<?php
										echo wp_kses_post(
											sprintf(
												...
												_n( '%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'luwipress-amber' ),
								...
								<?php elseif ( 'order-actions' === $column_id ) : ?>
									<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>" class="lwp-row-act"><?php echo $status === 'processing' || $status === 'on-hold' ? esc_html__( 'Track', 'luwipress-amber' ) : esc_html__( 'View', 'luwipress-amber' ); ?></a>
								<?php endif; ?>
							</td>
						<?php endforeach; ?>
					</tr>
	...
	<?php if ( 1 < $customer_orders->max_num_pages ) : ?>
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
			<?php if ( 1 !== $current_page ) : ?>
				<a class="lwp-btn-secondary" href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page - 1 ) ); ?>"><?php esc_html_e( '← Previous', 'luwipress-amber' ); ?></a>
			<?php endif; ?>
			<?php if ( intval( $customer_orders->max_num_pages ) !== $current_page ) : ?>
				<a class="lwp-btn-secondary" href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page + 1 ) ); ?>"><?php esc_html_e( 'Next →', 'luwipress-amber' ); ?></a>
			<?php endif; ?>
		</div>
	<?php endif; ?>
...
<?php else : ?>
	...
	<div class="woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
		<a class="lwp-btn-primary" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Browse instruments', 'luwipress-amber' ); ?>
		</a>
		<?php esc_html_e( 'No order has been made yet.', 'luwipress-amber' ); ?>
	</div>
