...
if ( ! $blog_title ) {
	$blog_title = __( 'Journal', 'luwipress-emerald' );
}
$blog_subtitle = (string) get_theme_mod(
	'luwipress_emerald_journal_subtitle',
	__( 'Field notes, workshop visits and long reads from the atelier.', 'luwipress-emerald' )
);
?>
...
<main class="lwp-page lwp-journal" id="primary">
	<div class="lwp-page-container">
		...
		<header class="lwp-journal-header">
			<span class="lwp-eyebrow">— <?php esc_html_e( 'Long reads from the atelier', 'luwipress-emerald' ); ?></span>
		...
		<?php if ( have_posts() ) : ?>
			...
			$prev = get_previous_posts_link( '← ' . __( 'Newer', 'luwipress-emerald' ) );
			$next = get_next_posts_link( __( 'Older', 'luwipress-emerald' ) . ' →' );
			if ( $prev || $next ) : ?>
				<nav class="lwp-jnl-pagination" aria-label="<?php esc_attr_e( 'Posts pagination', 'luwipress-emerald' ); ?>">
					...
					<span class="lwp-jnl-pagination__pages">
						...
						printf( esc_html__( 'Page %1$d of %2$d', 'luwipress-emerald' ), $current, $total );
						?>
					</span>
		...
		<?php else : ?>
			...
			<div class="lwp-jnl-empty">
				<p><?php esc_html_e( 'No posts yet — the atelier is still drafting.', 'luwipress-emerald' ); ?></p>
			</div>
