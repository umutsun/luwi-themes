 ...
 *      LuwiPress Emerald and react accordingly.
...
add_action( 'admin_notices', function () {
	...
	<div class="notice notice-warning">
		<p>
			<strong><?php esc_html_e( 'LuwiPress Emerald needs Elementor.', 'luwipress-emerald' ); ?></strong>
			<?php
			printf(
				/* translators: %s: install plugin link */
				esc_html__( 'Install Elementor (free) to use the LuwiPress Emerald templates: %s.', 'luwipress-emerald' ),
				'<a href="' . esc_url( $plugin_url ) . '">' . esc_html__( 'Install Elementor', 'luwipress-emerald' ) . '</a>'
			);
			?>
		</p>
	</div>
...
add_filter( 'body_class', function ( $classes ) {
	$classes[] = 'luwipress-emerald';
	if ( did_action( 'elementor/loaded' ) ) {
		$classes[] = 'luwipress-emerald--elementor';
	}
	return $classes;
} );
...
add_filter( 'template_include', function ( $template ) {
	...
	if ( is_singular() ) {
		...
		if ( $page_tpl === 'elementor_header_footer' || $page_tpl === 'elementor_header_footer.php' ) {
			$ours = LUWIPRESS_EMERALD_DIR . '/elementor_header_footer.php';
			if ( file_exists( $ours ) ) {
				return $ours;
			}
		}
 ...
 * ------------------------------------------------------------------------- */
add_filter( 'luwipress_emerald_kit_path', function () {
	return LUWIPRESS_EMERALD_DIR . '/elementor-kit/';
} );
 ...
 */
function luwipress_emerald_kit_files() {
	$dir = apply_filters( 'luwipress_emerald_kit_path', LUWIPRESS_EMERALD_DIR . '/elementor-kit/' );
