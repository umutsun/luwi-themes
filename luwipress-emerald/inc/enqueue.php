...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_enqueue_script(
		'luwipress-emerald-frontend',
		LUWIPRESS_EMERALD_URI . '/assets/js/frontend.js',
		[],
		$ver,
		true
	);
	...
	if ( function_exists( 'is_woocommerce' ) && ( is_shop() || is_product_taxonomy() || is_product_category() || is_product_tag() ) ) {
		...
		if ( get_theme_mod( 'luwipress_emerald_shop_loadmore', true ) ) {
			$lm_path = get_template_directory() . '/assets/js/loadmore.js';
			$lm_ver  = $ver . '.' . ( file_exists( $lm_path ) ? filemtime( $lm_path ) : '0' );
			wp_enqueue_script(
				'luwipress-emerald-loadmore',
				LUWIPRESS_EMERALD_URI . '/assets/js/loadmore.js',
				[],
				$lm_ver,
				true
			);
			wp_localize_script( 'luwipress-emerald-loadmore', 'LWP_EMERALD_LM', [
				'i18n' => [
					'load_more' => __( 'Load more products', 'luwipress-emerald' ),
					'loading'   => __( 'Loading…', 'luwipress-emerald' ),
					'no_more'   => __( 'You\'ve reached the end.', 'luwipress-emerald' ),
					'error'     => __( 'Couldn\'t load more — try again.', 'luwipress-emerald' ),
				],
				'mode' => (string) get_theme_mod( 'luwipress_emerald_shop_loadmore_mode', 'infinite' ), // infinite (default) | button
			] );
		}
	}
}, 20 );
 ...
 * bootstrap.php's `LWP_EMERALD_ANIM` localise at priority 25). Both
 * localise calls APPEND to the same `-js-extra` script block, so the
 * frontend gets `var LWP_EMERALD_ANIM = {...}; var LuwiEmerald = {...};`.
 *
 ...
 */
add_action( 'wp_enqueue_scripts', function () {
	if ( ! wp_script_is( 'luwipress-emerald-frontend', 'enqueued' ) ) {
		return;
	}
 ...
 */
add_action( 'wp_enqueue_scripts', function () {
	$base_ver = LUWIPRESS_EMERALD_VERSION;
	$dir      = LUWIPRESS_EMERALD_DIR;
	$uri      = LUWIPRESS_EMERALD_URI;
	...
	/*
	 * Emerald 1.0.0 ships a single tokens.css for the design system
	 * (`.emerald-*` classes + emerald palette). Gold's widgets.css +
	 * woo-overrides.css are intentionally NOT enqueued here — they
	 * carry Gold-specific Cormorant Garamond typography and Tapadum
	 * music-store overrides that conflict visually with Emerald.
	 * Operator extends via Customizer (Brand → primary color), via
	 * Elementor Kit CSS, or by adding a child theme override sheet.
	 */
}, 9999 );
...
add_action( 'wp_head', function () {
	?>
<style id="luwipress-emerald-critical-reset">
	...
	body.luwipress-emerald .lwp-mm li{
...
/**
 * Editor styles — surface the Emerald tokens inside the block editor too.
 */
add_action( 'enqueue_block_editor_assets', function () {
	wp_enqueue_style(
		'luwipress-emerald-editor-tokens',
		LUWIPRESS_EMERALD_URI . '/assets/css/tokens.css',
		[],
