 ...
 *     (browser white).
 *
 * Footer color is read from Customizer (`luwipress_emerald_footer_bg` theme
...
if ( ! function_exists( 'luwipress_emerald_resolve_footer_bg_token' ) ) {

	/**
	 * Resolve the footer background color, in priority order:
	 *   1. Customizer theme_mod `luwipress_emerald_footer_bg`
	 *   2. The ink/text token `luwipress_emerald_ink` (default footer skin
	 ...
	 * @return string
	 */
	function luwipress_emerald_resolve_footer_bg_token() {
		$candidates = [
			get_theme_mod( 'luwipress_emerald_footer_bg' ),
			get_theme_mod( 'luwipress_emerald_ink' ),
			'#1b1c1c',
		];
...
if ( ! function_exists( 'luwipress_emerald_print_layout_fixes' ) ) {

	function luwipress_emerald_print_layout_fixes() {
		if ( is_admin() ) {
			return;
		}
		$footer_bg = luwipress_emerald_resolve_footer_bg_token();
		?>
<style id="lwp-emerald-layout-fixes">
...
body{min-height:100vh}
	...
	add_action( 'wp_head', 'luwipress_emerald_print_layout_fixes', 1 );
}

if ( ! function_exists( 'luwipress_emerald_print_archive_hero_fixes' ) ) {
	 ...
	 */
	function luwipress_emerald_print_archive_hero_fixes() {
		if ( is_admin() ) {
			return;
		}
		...
		?>
<style id="lwp-emerald-archive-hero-fix">
...
@media (max-width:900px){
	...
	add_action( 'wp_head', 'luwipress_emerald_print_archive_hero_fixes', 99 );
}

if ( ! function_exists( 'luwipress_emerald_print_pdp_spacing_fix' ) ) {
	 ...
	 * `luwipress_emerald_pdp_spacing_fix` filter returning false.
	 */
	function luwipress_emerald_print_pdp_spacing_fix() {
		if ( is_admin() ) {
			return;
		}
		...
		if ( ! apply_filters( 'luwipress_emerald_pdp_spacing_fix', true ) ) {
			return;
		}
		?>
<style id="lwp-emerald-pdp-spacing-fix">
...
@media (max-width:900px){
	...
	add_action( 'wp_head', 'luwipress_emerald_print_pdp_spacing_fix', 99 );
}

if ( ! function_exists( 'luwipress_emerald_print_singular_top_spacing' ) ) {
	 ...
	 * has been mangled.
	 */
	function luwipress_emerald_print_singular_top_spacing() {
		if ( is_admin() ) {
			return;
		}
		...
		?>
<style id="lwp-emerald-singular-top-spacing">
...
.lwp-journal-tags__label{
	...
	add_action( 'wp_head', 'luwipress_emerald_print_singular_top_spacing', 99 );
}
