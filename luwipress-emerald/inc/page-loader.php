 ...
 *
 * Customizer toggle: LuwiPress Emerald → Performance → Page loader overlay.
 *
...
if ( ! function_exists( 'luwipress_emerald_loader_should_render' ) ) {
	 ...
	 * globally via Customizer or via the `luwipress_emerald_loader_render`
	 * filter (return false).
	 *
	 * @return bool
	 */
	function luwipress_emerald_loader_should_render() {
		if ( is_admin() ) {
			return false;
		}
		...
		$on = (bool) get_theme_mod( 'luwipress_emerald_loader_enabled', true );
		return (bool) apply_filters( 'luwipress_emerald_loader_render', $on );
	}
}

if ( ! function_exists( 'luwipress_emerald_loader_print_head' ) ) {
	 ...
	 */
	function luwipress_emerald_loader_print_head() {
		if ( ! luwipress_emerald_loader_should_render() ) {
			return;
		}
		?>
<style id="lwp-emerald-loader-boot">
...
<script id="lwp-emerald-loader-boot-mark">
...
window.setTimeout(function () {
	...
	add_action( 'wp_head', 'luwipress_emerald_loader_print_head', 1 );
}

if ( ! function_exists( 'luwipress_emerald_loader_render' ) ) {
	 ...
	 */
	function luwipress_emerald_loader_render() {
		if ( ! luwipress_emerald_loader_should_render() ) {
			return;
		}
		?>
<div class="lwp-loader" id="lwp-loader" aria-hidden="true">
	<div class="lwp-loader-inner">
		...
		<span class="lwp-loader-text"><?php esc_html_e( 'Loading', 'luwipress-emerald' ); ?></span>
		<span class="lwp-loader-bar"><span></span></span>
	</div>
</div>
		<?php
	}
	add_action( 'wp_body_open', 'luwipress_emerald_loader_render', 1 );
}

if ( ! function_exists( 'luwipress_emerald_loader_customizer' ) ) {

	/**
	 * Register the on/off toggle in Customize → LuwiPress Emerald →
	 ...
	 */
	function luwipress_emerald_loader_customizer( $wp_customize ) {
		...
		$wp_customize->add_control( 'luwipress_emerald_loader_enabled', [
			...
			'description' => __( 'Show a brand-aligned loader overlay until the page finishes painting. Server-rendered, so it appears on the first frame instead of after the page has flashed.', 'luwipress-emerald' ),
			'section'     => $section_id,
			'type'        => 'checkbox',
			'priority'    => 10,
		] );
	}
	add_action( 'customize_register', 'luwipress_emerald_loader_customizer', 30 );
}
