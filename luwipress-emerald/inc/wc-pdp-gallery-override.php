...
add_action( 'init', function () {
	...
	add_action( 'woocommerce_before_single_product_summary', 'luwipress_emerald_open_sticky_col', 5 );
	add_action( 'woocommerce_before_single_product_summary', 'luwipress_emerald_render_pdp_gallery', 20 );
	add_action( 'woocommerce_before_single_product_summary', 'luwipress_emerald_close_sticky_col', 25 );
} );

function luwipress_emerald_open_sticky_col() {
	if ( ! is_product() ) { return; }
	echo '<div class="lwp-pdp-sticky-col">';
}
function luwipress_emerald_close_sticky_col() {
	if ( ! is_product() ) { return; }
	echo '</div>';
}
 ...
 */
function luwipress_emerald_render_pdp_gallery() {
	if ( ! is_product() ) { return; }
	global $product;
	...
	$count        = count( $all_ids );
	?>
	<div class="lwp-gallery <?php echo $count > 1 ? 'lwp-gallery--multi' : 'lwp-gallery--single'; ?>" data-lwp-gallery tabindex="0" role="region" aria-label="<?php echo esc_attr( sprintf( __( '%s product images', 'luwipress-emerald' ), $product_name ) ); ?>">
				...
				<?php foreach ( $all_ids as $i => $aid ) :
					...
					<li class="lwp-gallery__thumb<?php echo $is_first ? ' is-active' : ''; ?>" role="presentation">
						<button type="button"
						        ...
						        aria-label="<?php echo esc_attr( sprintf( __( 'View image %d', 'luwipress-emerald' ), $i + 1 ) ); ?>">
...
add_action( 'wp_enqueue_scripts', function () {
	...
	$abs  = LUWIPRESS_EMERALD_DIR . $rel;
	$ver  = LUWIPRESS_EMERALD_VERSION . '.' . ( file_exists( $abs ) ? filemtime( $abs ) : '0' );

	wp_enqueue_script(
		'luwipress-emerald-pdp-gallery',
		LUWIPRESS_EMERALD_URI . $rel,
		[],
		$ver,
		true
	);
...
add_filter( 'script_loader_tag', function ( $tag, $handle ) {
	if ( $handle === 'luwipress-emerald-pdp-gallery' ) {
		return str_replace(
			'<script ',
