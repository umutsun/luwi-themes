...
add_action( 'woocommerce_single_product_summary', function () {
	...
	if ( ! $maker ) {
		$m_meta = (string) get_post_meta( $product->get_id(), '_lwp_emerald_maker', true );
		if ( $m_meta ) $maker = $m_meta;
	}
	if ( $maker ) {
		/* translators: %s: master / maker name */
		$parts[] = sprintf( __( 'by %s', 'luwipress-emerald' ), $maker );
	}
...
add_filter( 'woocommerce_get_price_html', function ( $html, $product ) {
	...
	$badge = sprintf(
		...
		'<span class="lwp-pdp-save">' . esc_html__( 'Save %s', 'luwipress-emerald' ) . '</span>',
		wc_price( $saved )
	);
	return $html . ' ' . $badge;
}, 20, 2 );
