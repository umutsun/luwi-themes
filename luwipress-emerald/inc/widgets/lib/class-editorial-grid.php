...
class LuwiPress_Emerald_Widget_Editorial_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-editorial-grid'; }
	public function get_title()       { return __( 'Editorial Grid', 'luwipress-emerald' ); }
	...
	protected function register_controls() {
		$this->start_controls_section( 'section_query', [ 'label' => __( 'Query', 'luwipress-emerald' ) ] );

		$this->add_control(
			'post_type',
			[
				'label'   => __( 'Post type', 'luwipress-emerald' ),
		...
		$this->add_control(
			'count',
			[
				'label'   => __( 'Number of posts', 'luwipress-emerald' ),
		...
		$this->add_control(
			'orderby',
			[
				'label'   => __( 'Order by', 'luwipress-emerald' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'date',
				'options' => [
					'date'     => __( 'Newest first', 'luwipress-emerald' ),
					'modified' => __( 'Recently updated', 'luwipress-emerald' ),
					'rand'     => __( 'Random', 'luwipress-emerald' ),
					'comment_count' => __( 'Most discussed', 'luwipress-emerald' ),
				],
			]
		);
		$this->add_control(
			'category',
			[
				'label'       => __( 'Filter by category', 'luwipress-emerald' ),
				'type'        => Controls_Manager::SELECT2,
				'multiple'    => true,
				'options'     => $this->get_category_options(),
				'description' => __( 'Leave empty for all.', 'luwipress-emerald' ),
			]
		);
		...
		/* Heading */
		$this->start_controls_section( 'section_head', [ 'label' => __( 'Section heading', 'luwipress-emerald' ) ] );
		$this->add_control(
			'eyebrow',
			[
				'label'   => __( 'Eyebrow', 'luwipress-emerald' ),
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'Journal', 'luwipress-emerald' ),
			]
		);
		$this->add_control(
			'heading',
			[
				...
				'rows'    => 2,
				'default' => __( 'Notes from the workshop.', 'luwipress-emerald' ),
			]
		);
		$this->add_control(
			'all_link',
			[
				'label'   => __( 'View all link', 'luwipress-emerald' ),
		...
		$this->add_control(
			'all_label',
			[
				'label'   => __( 'View all label', 'luwipress-emerald' ),
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'All articles →', 'luwipress-emerald' ),
			]
		);
		$this->end_controls_section();
	}
	...
	protected function render() {
		...
		if ( ! $query->have_posts() ) {
			echo '<div class="lwp-eg lwp-eg--empty">' . esc_html__( 'No posts found.', 'luwipress-emerald' ) . '</div>';
			return;
		}
		?>
