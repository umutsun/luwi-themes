...
class LuwiPress_Emerald_Widget_KG_Stats extends Widget_Base {

	public function get_name()        { return 'lwp-kg-stats'; }
	public function get_title()       { return __( 'Knowledge Graph · Stats', 'luwipress-emerald' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_layout', [ 'label' => __( 'Layout', 'luwipress-emerald' ) ] );

		$this->add_control( 'eyebrow', [ 'label' => __( 'Eyebrow', 'luwipress-emerald' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Store at a glance', 'luwipress-emerald' ) ] );
		$this->add_control( 'heading', [ 'label' => __( 'Heading', 'luwipress-emerald' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Live store stats.', 'luwipress-emerald' ) ] );

		$this->add_control( 'show_products',  [ 'label' => __( 'Show Products counter', 'luwipress-emerald' ),  'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
		$this->add_control( 'show_categories',[ 'label' => __( 'Show Categories counter', 'luwipress-emerald' ),'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
		$this->add_control( 'show_authors',   [ 'label' => __( 'Show Authors/Team counter', 'luwipress-emerald' ),   'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );
		$this->add_control( 'show_countries', [ 'label' => __( 'Show Countries-shipped counter', 'luwipress-emerald' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ] );

		$this->add_control( 'lbl_products',   [ 'label' => __( 'Products label', 'luwipress-emerald' ),   'type' => Controls_Manager::TEXT, 'default' => __( 'Products in catalogue', 'luwipress-emerald' ) ] );
		$this->add_control( 'lbl_categories', [ 'label' => __( 'Categories label', 'luwipress-emerald' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Categories', 'luwipress-emerald' ) ] );
		$this->add_control( 'lbl_authors',    [ 'label' => __( 'Authors label', 'luwipress-emerald' ),    'type' => Controls_Manager::TEXT, 'default' => __( 'Team members', 'luwipress-emerald' ) ] );
		$this->add_control( 'lbl_countries',  [ 'label' => __( 'Countries label', 'luwipress-emerald' ),  'type' => Controls_Manager::TEXT, 'default' => __( 'Countries shipped to', 'luwipress-emerald' ) ] );
	...
	protected function fetch_kg() {
		if ( ! function_exists( 'lwp_emerald_lp_active' ) || ! lwp_emerald_lp_active() ) { return null; }
		if ( ! class_exists( 'LuwiPress_Knowledge_Graph' ) ) { return null; }

		$cached = get_transient( 'lwp_emerald_kg_summary' );
		...
		if ( is_array( $data ) ) {
			set_transient( 'lwp_emerald_kg_summary', $data, 5 * MINUTE_IN_SECONDS );
		}
		return $data;
	}
	...
	protected function render_upsell() {
		?>
		<div class="lwp-kgstats lwp-kgstats--upsell" role="region" aria-label="<?php esc_attr_e( 'Install LuwiPress to enable live store stats', 'luwipress-emerald' ); ?>">
			<div class="lwp-kgstats__upsell-inner">
				<span class="lwp-kgstats__upsell-eb"><?php esc_html_e( '— LuwiPress required', 'luwipress-emerald' ); ?></span>
				<h3 class="lwp-kgstats__upsell-title">
					<?php esc_html_e( 'Unlock live Knowledge Graph data.', 'luwipress-emerald' ); ?>
				</h3>
				<p class="lwp-kgstats__upsell-lead">
					<?php esc_html_e( 'Install the free LuwiPress plugin to surface live product counts, category coverage, master roster and countries-shipped on this widget — pulled directly from your store data.', 'luwipress-emerald' ); ?>
				</p>
				<a class="lwp-kgstats__upsell-btn" href="<?php echo esc_url( admin_url( 'plugin-install.php?s=luwipress&tab=search&type=term' ) ); ?>">
					<?php esc_html_e( 'Install LuwiPress (free) →', 'luwipress-emerald' ); ?>
				</a>
			</div>
		</div>
		<?php
	...
	protected function render() {
		...
		if ( ! function_exists( 'lwp_emerald_lp_active' ) || ! lwp_emerald_lp_active() ) {
			$this->render_upsell();
			return;
		}
		...
		<div class="lwp-kgstats" data-live="1">
			<?php if ( $eyebrow || $heading ) : ?>
				<header class="lwp-kgstats__head">
					...
					<span class="lwp-kgstats__badge" title="<?php esc_attr_e( 'Powered by LuwiPress Knowledge Graph', 'luwipress-emerald' ); ?>">
						<?php esc_html_e( 'Live · LuwiPress KG', 'luwipress-emerald' ); ?>
					</span>
				</header>
			<?php endif; ?>
