...
class LuwiPress_Emerald_Widget_Master_Profile extends Widget_Base {

	public function get_name()        { return 'lwp-master-profile'; }
	public function get_title()       { return __( 'Master Profile', 'luwipress-emerald' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			'portrait',
			[
				'label'   => __( 'Portrait', 'luwipress-emerald' ),
		...
		$this->add_control(
			'fallback_initial',
			[
				'label'       => __( 'Fallback initial', 'luwipress-emerald' ),
				'type'        => Controls_Manager::TEXT,
				'description' => __( 'Shown when no portrait is set. Auto-derived from name if empty.', 'luwipress-emerald' ),
			]
		);
		$this->add_control(
			'name',
			[
				...
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'Profile name', 'luwipress-emerald' ),
			]
		);
		$this->add_control(
			'location',
			[
				'label'   => __( 'Location', 'luwipress-emerald' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		$this->add_control(
			'specialty',
			[
				'label'   => __( 'Specialty', 'luwipress-emerald' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		$this->add_control(
			'count',
			[
				'label'   => __( 'Count', 'luwipress-emerald' ),
		...
		$this->add_control(
			'show_count',
			[
				'label'   => __( 'Show instrument count', 'luwipress-emerald' ),
		...
		$this->add_control(
			'profile_url',
			[
				'label'   => __( 'Profile URL', 'luwipress-emerald' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => 'card',
				'options' => [
					'card'     => __( 'Compact card (homepage grid)', 'luwipress-emerald' ),
					'feature'  => __( 'Featured profile (hero portrait + bio)', 'luwipress-emerald' ),
				],
			]
		);
		...
		$this->add_control(
			'gradient_a',
			[
				'label'   => __( 'Fallback gradient — start', 'luwipress-emerald' ),
		...
		$this->add_control(
			'gradient_b',
			[
				'label'   => __( 'Fallback gradient — end', 'luwipress-emerald' ),
				'type'    => Controls_Manager::COLOR,
				'default' => '#d4af37',
			]
		);
	...
	protected function render() {
				...
				<div class="lwp-mp-body">
					<?php if ( ! empty( $s['location'] ) ) : ?>
						<span class="lwp-mp-eyebrow">— <?php
							...
							printf( esc_html__( 'Master luthier · %s', 'luwipress-emerald' ), esc_html( $s['location'] ) );
						?></span>
					<?php endif; ?>
					...
					<?php if ( $s['show_count'] === 'yes' ) : ?>
						<div class="lwp-mp-stats">
							<div><span class="n"><?php echo (int) $s['count']; ?></span><span class="l"><?php esc_html_e( 'Instruments at the atelier', 'luwipress-emerald' ); ?></span></div>
						</div>
					<?php endif; ?>
				</div>
			</div>
		...
		if ( $s['show_count'] === 'yes' && ! empty( $s['count'] ) ) {
			echo '<span class="lwp-mp-count">' . sprintf(
				...
				esc_html__( '%d instruments', 'luwipress-emerald' ),
				(int) $s['count']
			) . '</span>';
		}
		echo '</div></' . $tag . '>';
