 ...
 *      under LuwiPress Emerald → Footer → Social, so the user enters it
...
class LuwiPress_Emerald_Widget_YouTube_Channel extends Widget_Base {

	public function get_name()        { return 'lwp-youtube-channel'; }
	public function get_title()       { return __( 'YouTube Channel', 'luwipress-emerald' ); }
	...
	protected function register_controls() {
		...
		/* Section: Heading */
		$this->start_controls_section( 'section_head', [ 'label' => __( 'Heading', 'luwipress-emerald' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'From our channel', 'luwipress-emerald' ),
			]
		);
		$this->add_control(
			'heading',
			[
				...
				'description' => __( 'Use a line break (Enter) for a 2-line title.', 'luwipress-emerald' ),
				'type'        => Controls_Manager::TEXTAREA,
				'rows'        => 2,
				'default'     => __( "Featured videos\nfrom our YouTube channel.", 'luwipress-emerald' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'YouTube channel →', 'luwipress-emerald' ),
			]
		);
		$this->add_control(
			'cta_url',
			[
				...
				'description' => __( 'Leave empty to pull the channel URL from Customize → LuwiPress Emerald → Footer → YouTube URL.', 'luwipress-emerald' ),
		...
		$this->add_control(
			'show_subscribe',
			[
				'label'        => __( 'Show Subscribe button', 'luwipress-emerald' ),
				'description'  => __( 'Only renders if a channel URL is set (here or in Customize → Footer → YouTube URL).', 'luwipress-emerald' ),
		...
		$this->add_control(
			'subscribe_label',
			[
				'label'     => __( 'Subscribe label', 'luwipress-emerald' ),
				'type'      => Controls_Manager::TEXT,
				'default'   => __( 'Subscribe', 'luwipress-emerald' ),
		...
		$this->start_controls_section( 'section_videos', [ 'label' => __( 'Videos', 'luwipress-emerald' ) ] );

		$repeater = new Repeater();
		$repeater->add_control(
			'video',
			[
				'label'       => __( 'YouTube URL or ID', 'luwipress-emerald' ),
				'description' => __( 'Paste the full URL (youtu.be/… or youtube.com/watch?v=…) or just the 11-char ID.', 'luwipress-emerald' ),
				'type'        => Controls_Manager::TEXT,
		...
		$repeater->add_control(
			'title',
			[
				'label'   => __( 'Title', 'luwipress-emerald' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		$repeater->add_control(
			'byline',
			[
				'label'       => __( 'Byline', 'luwipress-emerald' ),
				'description' => __( 'e.g. "Channel name · 14k views"', 'luwipress-emerald' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'duration',
			[
				'label'       => __( 'Duration badge', 'luwipress-emerald' ),
				'description' => __( 'Optional. Shown as a small badge on the thumbnail (e.g. "3:42"). Leave empty to hide.', 'luwipress-emerald' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'thumb_quality',
			[
				'label'   => __( 'Thumbnail quality', 'luwipress-emerald' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'hq',
				'options' => [
					'hq'      => __( 'HQ (480×360)', 'luwipress-emerald' ),
					'mq'      => __( 'Standard (320×180)', 'luwipress-emerald' ),
					'maxres'  => __( 'Maxres (1280×720, may 404 on old videos)', 'luwipress-emerald' ),
				],
			]
		);
		...
		$this->add_control(
			'videos',
			[
				'label'       => __( 'Videos', 'luwipress-emerald' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $repeater->get_controls(),
				'title_field' => '{{{ title || video || "Video" }}}',
				'default'     => [
					[ 'video' => '', 'title' => __( 'Featured video 1', 'luwipress-emerald' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
					[ 'video' => '', 'title' => __( 'Featured video 2', 'luwipress-emerald' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
					[ 'video' => '', 'title' => __( 'Featured video 3', 'luwipress-emerald' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
					[ 'video' => '', 'title' => __( 'Featured video 4', 'luwipress-emerald' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
				],
			]
		);
		...
		$this->add_control(
			'columns',
			[
				'label'   => __( 'Columns (desktop)', 'luwipress-emerald' ),
				'type'    => Controls_Manager::SELECT,
				'default' => '4',
				'options' => [ '2' => '2', '3' => '3', '4' => '4' ],
			]
		...
		$this->add_control(
			'card_border',
			[
				'label'     => __( 'Card border', 'luwipress-emerald' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
		...
		$this->add_control(
			'title_color',
			[
				'label'     => __( 'Title color', 'luwipress-emerald' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
	...
	protected function resolve_channel_url( $widget_url ) {
		...
		$mod = get_theme_mod( 'luwipress_emerald_social_youtube', '' );
	...
	protected function render() {
		...
		$subscribe_label = trim( (string) ( $s['subscribe_label'] ?? '' ) ) ?: __( 'Subscribe', 'luwipress-emerald' );
		...
		<section class="lwp-ytc" data-columns="<?php echo esc_attr( $columns ); ?>">
					...
					<div class="lwp-ytc-head__actions">
						<?php if ( $show_subscribe ) : ?>
							<a class="lwp-ytc-sub"
								...
								aria-label="<?php echo esc_attr( sprintf( __( '%s on YouTube', 'luwipress-emerald' ), $subscribe_label ) ); ?>">
			...
			<?php if ( empty( $resolved ) ) : ?>
				<div class="lwp-ytc-placeholder" role="note">
					<strong><?php esc_html_e( 'No YouTube videos configured yet.', 'luwipress-emerald' ); ?></strong>
					<span><?php esc_html_e( 'Open this widget and paste a YouTube URL or video ID into each Videos row. This notice is only visible to logged-in editors.', 'luwipress-emerald' ); ?></span>
				</div>
			<?php else : ?>
			<div class="lwp-ytc-grid">
				<?php foreach ( $resolved as $item ) :
					...
					$aria     = $title !== '' ? sprintf( __( 'Play video — %s', 'luwipress-emerald' ), $title ) : __( 'Play video', 'luwipress-emerald' );
					?>
