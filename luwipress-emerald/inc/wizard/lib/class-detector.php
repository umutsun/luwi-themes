...
class LuwiPress_Emerald_Detector {
	...
	private function detect_theme_state() {
		...
		return [
			...
			'is_luwipress_emerald' => stripos( $current->get( 'Name' ), 'luwipress gold' ) !== false,
		];
	}
	...
	public function summary_phrase( $snap ) {
		...
		if ( ! empty( $snap['wc']['active'] ) ) {
			...
			$bits[] = __( 'no WooCommerce yet', 'luwipress-emerald' );
		}

		if ( ! empty( $snap['content']['posts'] ) ) {
			$bits[] = sprintf(
				/* translators: %d posts */
				_n( '%d post', '%d posts', $snap['content']['posts'], 'luwipress-emerald' ),
				$snap['content']['posts']
			);
		}
		...
		if ( ! empty( $snap['i18n']['is_multi'] ) ) {
			$bits[] = sprintf(
				...
				_n( '%d active language', '%d active languages', count( $snap['i18n']['active'] ), 'luwipress-emerald' ),
		...
		if ( ! empty( $snap['masters'] ) ) {
			$bits[] = sprintf(
				...
				_n( '%d master luthier', '%d master luthiers', count( $snap['masters'] ), 'luwipress-emerald' ),
				count( $snap['masters'] )
			);
		}

		if ( empty( $bits ) ) {
			return __( 'Fresh install — perfect for the demo content path.', 'luwipress-emerald' );
		}

		return sprintf(
			/* translators: comma-joined list of detected things */
			__( 'We detected: %s.', 'luwipress-emerald' ),
			implode( ', ', $bits )
		);
	}
}
