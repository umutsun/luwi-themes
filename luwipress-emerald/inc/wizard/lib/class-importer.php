...
class LuwiPress_Emerald_Importer {

	/**
	 * @var LuwiPress_Emerald_Content_Compiler
	 */
	private $compiler;
	...
	public function apply( $path, $brand = [], $opts = [] ) {
		...
		$this->compiler = new LuwiPress_Emerald_Content_Compiler( $this->snapshot );
		...
		// latest compiled content, so old `_lwp_emerald_managed` templates from
		// previous runs become orphans. Trash them now to avoid the user seeing
		// 13+ duplicate "LuwiPress Emerald — Header" entries in Templates → Library.
		...
		if ( function_exists( 'luwipress_emerald_sync_kit' ) ) {
			$kit_result = luwipress_emerald_sync_kit();
			$log['actions'][] = [
				...
				'detail' => is_wp_error( $kit_result )
					...
					: [ 'kit_id' => function_exists( 'luwipress_emerald_get_active_kit_id' ) ? luwipress_emerald_get_active_kit_id() : null ],
			];
		}
		...
		if ( ! empty( $opts['resolve_slug_conflicts'] ) ) {
			$conflicts = isset( $this->snapshot['slug_conflicts'] ) && is_array( $this->snapshot['slug_conflicts'] )
				...
				: [];
			set_theme_mod( 'luwipress_emerald_resolve_slug_conflicts', true );
			$log['actions'][] = [
		...
		update_option( 'luwipress_emerald_wizard_log', $log, false );
	 ...
	 * `_lwp_emerald_managed = 1`. Run at the start of every Apply so the
	 * Templates → Library doesn't accumulate one extra "LuwiPress Emerald —
	 * Header" per wizard run.
	 *
	 * Skip the auto-applied homepage container (id 35754 et al) — anything
	 * with `_lwp_emerald_keep` meta is preserved.
	 */
	private function cleanup_managed_templates( &$log ) {
		...
		foreach ( $query->posts as $tid ) {
			if ( get_post_meta( $tid, '_lwp_emerald_keep', true ) ) continue;
	...
	private function apply_brand( $brand, &$log ) {
		...
		foreach ( $mods as $key => $val ) {
			if ( $val !== null && $val !== '' ) {
				set_theme_mod( 'luwipress_emerald_' . $key, sanitize_hex_color( $val ) ?: $val );
		...
		if ( ! empty( $brand['phone'] ) ) {
			update_option( 'luwipress_emerald_phone', sanitize_text_field( $brand['phone'] ) );
			$log['options'][] = 'luwipress_emerald_phone';
		}
		if ( ! empty( $brand['email'] ) && is_email( $brand['email'] ) ) {
			// Don't overwrite admin_email — store in our namespace instead.
			update_option( 'luwipress_emerald_contact_email', sanitize_email( $brand['email'] ) );
			$log['options'][] = 'luwipress_emerald_contact_email';
		}
	}
	...
	private function kit_dir() {
		return apply_filters( 'luwipress_emerald_kit_path', LUWIPRESS_EMERALD_DIR . '/elementor-kit/' );
	}
	...
	private function import_template( $action ) {
		...
		update_post_meta( $post_id, '_lwp_emerald_managed',        1 );
	...
	private function import_demo_xml( $action ) {
		$file = LUWIPRESS_EMERALD_DIR . '/' . ltrim( $action['file'] ?? '', '/' );
		if ( ! file_exists( $file ) ) {
		...
		if ( ! file_exists( $wp_importer_path ) ) {
			return [
				...
				'hint'    => __( 'Install the "WordPress Importer" plugin from wp-admin → Tools → Import → WordPress.', 'luwipress-emerald' ),
			];
		}
	...
	private function mark_master( $action ) {
		...
		update_term_meta( $term->term_id, '_lwp_emerald_featured_master', 1 );
	...
	private function process_page( $slug, $page ) {
		...
		if ( ( $page['action'] ?? '' ) === 'create_and_import' || ! empty( $page['set_as_home'] ) ) {
			...
			$meta = [ '_lwp_emerald_managed' => 1 ];
			if ( ! empty( $page['parallel_to'] ) ) {
				$meta['_lwp_emerald_parallel_to'] = (int) $page['parallel_to'];
			}
			if ( ! empty( $page['compiler_context']['master_slug'] ) ) {
				$meta['_lwp_emerald_master_slug'] = sanitize_title( $page['compiler_context']['master_slug'] );
			}
	...
	private function apply_template_to_page( $page_id, $kit_file, $context = [] ) {
		...
		update_post_meta( $page_id, '_lwp_emerald_managed',        1 );
