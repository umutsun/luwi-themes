...
<main class="lwp-page lwp-shop-archive" id="primary">
	<div class="lwp-page-container">
		...
		<nav class="lwp-crumb" aria-label="<?php esc_attr_e( 'Breadcrumb', 'luwipress-emerald' ); ?>">
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'Home', 'luwipress-emerald' ); ?></a>
			<?php if ( function_exists( 'wc_get_page_id' ) ) :
				$shop_id = wc_get_page_id( 'shop' );
				if ( $shop_id > 0 && ! is_shop() ) : ?>
					<span class="sep">›</span>
					<a href="<?php echo esc_url( get_permalink( $shop_id ) ); ?>"><?php esc_html_e( 'Shop', 'luwipress-emerald' ); ?></a>
				<?php endif;
			endif; ?>
			<?php
		 ...
		 */
		do_action( 'luwipress_emerald_archive_intro_before', $is_term ? $current_obj : null );
		?>
		...
		<header class="lwp-archive-header<?php echo $header_image_html ? ' lwp-archive-header--split' : ''; ?>">
			<div class="lwp-archive-header__copy">
				<?php if ( $is_term ) : ?>
					...
					echo esc_html(
						$current_obj->parent
							? __( 'Sub-category', 'luwipress-emerald' )
							: __( 'Collection', 'luwipress-emerald' )
					);
					?></span>
				<?php endif; ?>
				 ...
				 */
				do_action( 'luwipress_emerald_archive_intro_after', $is_term ? $current_obj : null );
				?>
			</div>
		...
		<?php if ( ! empty( $subcats ) ) : ?>
			 ...
			 */
			do_action( 'luwipress_emerald_archive_subcat_tiles_before', $subcats, $current_obj );
			?>
			<section class="lwp-archive-subcat-grid" aria-label="<?php esc_attr_e( 'Sub-categories', 'luwipress-emerald' ); ?>">
					...
					<a class="lwp-archive-subcat" href="<?php echo esc_url( get_term_link( $sc ) ); ?>">
						...
						<div class="lwp-archive-subcat__meta">
							...
							<span class="lwp-archive-subcat__count">
								...
								printf( esc_html( _n( '%d piece', '%d pieces', (int) $sc->count, 'luwipress-emerald' ) ), (int) $sc->count );
								?>
							</span>
						</div>
					</a>
			...
			<?php
			do_action( 'luwipress_emerald_archive_subcat_tiles_after', $subcats, $current_obj );
			?>
		<?php endif; ?>

		<div class="lwp-shop-grid-wrap">

			<aside class="lwp-shop-sidebar" aria-label="<?php esc_attr_e( 'Filters', 'luwipress-emerald' ); ?>">
				<?php
				...
				} elseif ( function_exists( 'luwipress_emerald_render_smart_filters' ) ) {
					luwipress_emerald_render_smart_filters( $is_term ? $current_obj : null );
				}
				?>
			</aside>
					...
					if ( get_theme_mod( 'luwipress_emerald_shop_loadmore', true ) ) {
						...
						if ( $max_pages > 1 ) {
							...
							printf(
								...
								$current_page,
								$max_pages,
								esc_html__( 'Load more products', 'luwipress-emerald' ),
								esc_attr__( 'Loading', 'luwipress-emerald' )
							);
						}
					}
				} else {
