...
// present, otherwise falls back to `_lwp_emerald_maker` post-meta. Stores
...
if ( ! $maker ) {
	$maker = (string) get_post_meta( $product_id, '_lwp_emerald_maker', true );
}
	...
	<a href="<?php the_permalink(); ?>" class="lwp-pcard-link" aria-label="<?php echo esc_attr( wp_strip_all_tags( $product->get_name() ) ); ?>">
		...
		<div class="lwp-pcard-img">
			<?php if ( $product->is_on_sale() ) : ?>
				<span class="lwp-pcard-badge"><?php echo $sale_pct ? '−' . esc_html( $sale_pct ) . '%' : esc_html__( 'Sale', 'luwipress-emerald' ); ?></span>
			<?php endif; ?>
			<?php
