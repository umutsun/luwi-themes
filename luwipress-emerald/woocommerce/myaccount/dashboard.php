...
if ( ! $user_id ) {
	echo '<p>' . esc_html__( 'You must be logged in to view your account.', 'luwipress-emerald' ) . '</p>';
	return;
}
...
<header class="lwp-account-greeting">
	<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( 'Account', 'luwipress-emerald' ); ?></span>
	<h1><?php
		/* translators: %s: customer first name (wrapped <em> for italic accent) */
		printf(
			wp_kses( __( 'Welcome back, <em>%s</em>.', 'luwipress-emerald' ), [ 'em' => [] ] ),
	...
	<p class="meta lwp-acct-sub"><?php
		...
		if ( $member_year ) {
			...
			$bits[] = sprintf( esc_html__( 'Member since %s', 'luwipress-emerald' ), esc_html( $member_year ) );
		}
		/* translators: %d: total orders */
		$bits[] = sprintf( esc_html( _n( '%d order', '%d orders', $total_orders, 'luwipress-emerald' ) ), $total_orders );
		/* translators: %d: saved instruments count */
		$bits[] = sprintf( esc_html( _n( '%d favorite', '%d favorites', $saved_count, 'luwipress-emerald' ) ), $saved_count );
...
<div class="lwp-acct-stats">
	<div class="lwp-acct-stat">
		...
		<div class="l"><?php esc_html_e( 'Total orders', 'luwipress-emerald' ); ?></div>
	</div>
	<div class="lwp-acct-stat">
		<div class="n"><?php echo esc_html( $saved_count ); ?></div>
		<div class="l"><?php esc_html_e( 'Saved instruments', 'luwipress-emerald' ); ?></div>
	</div>
	<div class="lwp-acct-stat">
		<div class="n"><?php echo esc_html( $active_addresses ); ?></div>
		<div class="l"><?php esc_html_e( 'Active addresses', 'luwipress-emerald' ); ?></div>
	</div>
	<div class="lwp-acct-stat">
		<div class="n"><?php echo wp_kses_post( wc_price( $lifetime_spend ) ); ?></div>
		<div class="l"><?php esc_html_e( 'Lifetime spend', 'luwipress-emerald' ); ?></div>
	</div>
</div>
...
<?php if ( ! empty( $recent_orders ) ) : ?>
	<div class="lwp-acct-card">
		<h3>
			<?php esc_html_e( 'Recent orders', 'luwipress-emerald' ); ?>
			<a href="<?php echo esc_url( wc_get_account_endpoint_url( 'orders' ) ); ?>"><?php esc_html_e( 'View all →', 'luwipress-emerald' ); ?></a>
		</h3>
		<table class="lwp-acct-table">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Order', 'luwipress-emerald' ); ?></th>
					<th><?php esc_html_e( 'Date', 'luwipress-emerald' ); ?></th>
					<th><?php esc_html_e( 'Item', 'luwipress-emerald' ); ?></th>
					<th><?php esc_html_e( 'Total', 'luwipress-emerald' ); ?></th>
						...
						<td class="lwp-ord-prod"><?php echo esc_html( $item_nm ); ?>
							<?php if ( count( $items ) > 1 ) : ?>
								<small><?php
									...
									printf( esc_html( _n( '+ %d more item', '+ %d more items', count( $items ) - 1, 'luwipress-emerald' ) ), count( $items ) - 1 );
								?></small>
							<?php endif; ?>
						</td>
						<td><?php echo wp_kses_post( $order->get_formatted_order_total() ); ?></td>
						<td><span class="<?php echo esc_attr( $pill_class ); ?>"><?php echo esc_html( wc_get_order_status_name( $status ) ); ?></span></td>
						<td><a href="<?php echo esc_url( $order->get_view_order_url() ); ?>" class="lwp-row-act"><?php esc_html_e( 'View', 'luwipress-emerald' ); ?></a></td>
					</tr>
				<?php endforeach; ?>
			</tbody>
		</table>
...
<?php else : ?>
	<div class="lwp-acct-card">
		<h3><?php esc_html_e( 'Recent orders', 'luwipress-emerald' ); ?></h3>
		<p><?php esc_html_e( "You haven't placed any orders yet. Browse the catalogue to get started.", 'luwipress-emerald' ); ?>
			<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>" class="lwp-row-act" style="margin-left:8px;"><?php esc_html_e( 'Open shop →', 'luwipress-emerald' ); ?></a>
		</p>
	</div>
<?php endif; ?>
...
if ( $default ) :
	...
	<div class="lwp-acct-card">
		<h3>
			<?php esc_html_e( 'Default address', 'luwipress-emerald' ); ?>
			<a href="<?php echo esc_url( wc_get_account_endpoint_url( 'edit-address' ) ); ?>"><?php esc_html_e( 'Edit →', 'luwipress-emerald' ); ?></a>
		</h3>
