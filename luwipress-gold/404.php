 ...
 */
$default_links = [
	[ 'label' => __( 'New arrivals', 'luwipress-gold' ), 'url' => $home_url ],
	[ 'label' => __( 'Master luthiers', 'luwipress-gold' ), 'url' => $home_url ],
	[ 'label' => __( 'Custom commissions', 'luwipress-gold' ), 'url' => $home_url ],
	[ 'label' => __( 'Journal', 'luwipress-gold' ), 'url' => $home_url ],
];

$popular = apply_filters( 'luwipress_gold_404_popular_links', $default_links );
...
<main class="lwp-page" id="primary">
	<section class="error-404 not-found">
		<span class="eyebrow">— <?php esc_html_e( 'Page not found', 'luwipress-gold' ); ?></span>
		<h1 class="digit" aria-label="404">4<em>0</em>4</h1>
		<h2><?php
			/* translators: keep <em> wrapping the closing word for italic accent */
			echo wp_kses(
				__( 'The workshop door <em>is closed</em>.', 'luwipress-gold' ),
				[ 'em' => [] ]
			);
		?></h2>
		<p><?php esc_html_e( 'This page either moved, sold out, or was never here. Try a different door:', 'luwipress-gold' ); ?></p>
	...
	<section class="lwp-404-popular" style="padding:20px;display:flex;flex-direction:column;gap:10px">
		<span class="eyebrow"><?php esc_html_e( 'Most asked-about', 'luwipress-gold' ); ?></span>
	...
	<section class="lwp-404-search" style="background:var(--bg-alt);padding:20px;display:flex;flex-direction:column;gap:10px">
		<span class="eyebrow"><?php esc_html_e( 'Or search', 'luwipress-gold' ); ?></span>
		<form role="search" method="get" action="<?php echo esc_url( $home_url ); ?>" style="display:flex;align-items:center;gap:8px;padding:13px 16px;background:var(--bg);border:1px solid var(--line);border-radius:24px">
			...
			<label for="lwp-404-s" class="screen-reader-text"><?php esc_html_e( 'Search', 'luwipress-gold' ); ?></label>
			<input id="lwp-404-s" name="s" type="search" placeholder="<?php esc_attr_e( 'Search instruments, masters…', 'luwipress-gold' ); ?>" style="flex:1;border:0;outline:none;background:transparent;font-size:16px" />
		</form>
	</section>
</main>
<?php
