 ...
 *
 * The allowlist is filterable via `luwipress_gold_blog_page_slugs` so
...
if ( ! function_exists( 'luwipress_gold_get_blog_page_slugs' ) ) {

	/**
	 * @return string[] Lower-case slug allowlist.
	 */
	function luwipress_gold_get_blog_page_slugs() {
		$defaults = [
			'blog',
			'journal',
			'news',
		...
		return (array) apply_filters( 'luwipress_gold_blog_page_slugs', $defaults );
	}
}

if ( ! function_exists( 'luwipress_gold_find_blog_page_id' ) ) {
	 ...
	 */
	function luwipress_gold_find_blog_page_id() {
		$existing = (int) get_option( 'page_for_posts' );
		if ( $existing > 0 && get_post_status( $existing ) === 'publish' ) {
			return $existing;
		}

		$slugs = array_map( 'strtolower', luwipress_gold_get_blog_page_slugs() );
...
if ( ! function_exists( 'luwipress_gold_page_is_effectively_empty' ) ) {
	 ...
	 * @return bool
	 */
	function luwipress_gold_page_is_effectively_empty( $page_id ) {
...
if ( ! function_exists( 'luwipress_gold_auto_promote_blog_page' ) ) {
	 ...
	 */
	function luwipress_gold_auto_promote_blog_page() {
		if ( wp_doing_ajax() || ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ) {
			return;
		}
		// Already promoted by us? Don't re-fight an operator decision.
		if ( get_option( 'luwipress_gold_blog_promoted' ) ) {
			return;
		}
		...
		$page_id = luwipress_gold_find_blog_page_id();
		if ( $page_id <= 0 ) {
			return;
		}
		...
		update_option( 'luwipress_gold_blog_promoted', $page_id );
	}
	add_action( 'init', 'luwipress_gold_auto_promote_blog_page', 20 );
}

if ( ! function_exists( 'luwipress_gold_inject_blog_loop_into_empty_page' ) ) {
	 ...
	 * @return string
	 */
	function luwipress_gold_inject_blog_loop_into_empty_page( $content ) {
		...
		// Empty enough?
		if ( ! luwipress_gold_page_is_effectively_empty( $page_id ) ) {
			return $content;
		}
		// Fetch recent posts.
		$query = new WP_Query( [
		...
		ob_start();
		?>
		<section class="lwp-journal-grid lwp-journal-grid--fallback" aria-label="<?php esc_attr_e( 'Recent posts', 'luwipress-gold' ); ?>">
				...
				<article class="lwp-journal-card">
					...
					<div class="lwp-journal-card__copy">
						...
						<a class="lwp-journal-card__cta" href="<?php the_permalink(); ?>">
							<?php esc_html_e( 'Read →', 'luwipress-gold' ); ?>
						</a>
					</div>
				</article>
			<?php endwhile; ?>
		</section>
		<style id="lwp-gold-blog-fallback-css">
			...
			.lwp-journal-grid--fallback .lwp-journal-card__title{
				margin:0;font-family:var(--lwp-gold-serif,"Playfair Display",serif);font-size:20px;line-height:1.25;
			}
	...
	add_filter( 'the_content', 'luwipress_gold_inject_blog_loop_into_empty_page', 99 );
}
