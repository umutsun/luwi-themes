 ...
 * container width) to match the LuwiPress Gold token palette so the
 ...
 * 3. manual hook         → operators can call luwipress_gold_sync_kit() from
 ...
 */
function luwipress_gold_sync_kit( $overrides = [] ) {
	if ( ! did_action( 'elementor/loaded' ) ) {
		return new WP_Error( 'elementor_missing', __( 'Elementor is not active — kit sync skipped.', 'luwipress-gold' ) );
	}

	$defaults = luwipress_gold_kit_default_palette();
	$args = wp_parse_args( $overrides, $defaults );

	// Allow Customizer overrides to take precedence — read at sync time.
	$args['primary']   = get_theme_mod( 'luwipress_gold_color_primary',       $args['primary'] );
	$args['secondary'] = get_theme_mod( 'luwipress_gold_color_primary_light', $args['secondary'] );
	$args['text']      = get_theme_mod( 'luwipress_gold_color_ink',           $args['text'] );
	$args['accent']    = get_theme_mod( 'luwipress_gold_color_sale',          $args['accent'] );

	$kit_id = luwipress_gold_get_active_kit_id();
	if ( ! $kit_id ) {
		return new WP_Error( 'no_kit', __( 'No active Elementor kit found.', 'luwipress-gold' ) );
	}

	$settings = luwipress_gold_compose_kit_settings( $args );

	// Path 1: Elementor's own Kit::update_settings — preferred.
	$wrote = luwipress_gold_write_kit_via_api( $kit_id, $settings );

	// Path 2: direct post_meta fallback.
	if ( ! $wrote ) {
		luwipress_gold_write_kit_via_meta( $kit_id, $settings );
	}

	// Trigger CSS regen.
	luwipress_gold_kit_regen_css( $kit_id );

	// Mark sync.
	update_option( 'luwipress_gold_kit_synced_at', time(), false );
 ...
 */
function luwipress_gold_kit_default_palette() {
	return [
		'primary'     => '#735c00',  // Gold
 ...
 */
function luwipress_gold_get_active_kit_id() {
 ...
 */
function luwipress_gold_compose_kit_settings( $args ) {
	return [
		'system_colors' => [
		...
		'custom_colors' => [
			[ '_id' => 'lwp_gold_bright', 'title' => 'Gold Bright', 'color' => $args['secondary'] ],
 ...
 */
function luwipress_gold_write_kit_via_api( $kit_id, $settings ) {
 ...
 */
function luwipress_gold_write_kit_via_meta( $kit_id, $settings ) {
 ...
 */
function luwipress_gold_kit_regen_css( $kit_id ) {
...
add_action( 'after_switch_theme', function () {
	...
	update_option( 'luwipress_gold_kit_sync_pending', 1, false );
} );

// 2. Pending sync — runs once on the first admin_init after activation.
add_action( 'admin_init', function () {
	if ( ! get_option( 'luwipress_gold_kit_sync_pending' ) ) return;
	if ( ! did_action( 'elementor/loaded' ) ) return;
	luwipress_gold_sync_kit();
	delete_option( 'luwipress_gold_kit_sync_pending' );
}, 99 );
