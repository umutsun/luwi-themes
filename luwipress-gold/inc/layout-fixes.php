 ...
 *     (browser white).
 *
 * Footer color is read from Customizer (`luwipress_gold_footer_bg` theme
...
if ( ! function_exists( 'luwipress_gold_resolve_footer_bg_token' ) ) {

	/**
	 * Resolve the footer background color, in priority order:
	 *   1. Customizer theme_mod `luwipress_gold_footer_bg`
	 *   2. The ink/text token `luwipress_gold_ink` (default footer skin
	 ...
	 * @return string
	 */
	function luwipress_gold_resolve_footer_bg_token() {
		$candidates = [
			get_theme_mod( 'luwipress_gold_footer_bg' ),
			get_theme_mod( 'luwipress_gold_ink' ),
			'#1b1c1c',
		];
...
if ( ! function_exists( 'luwipress_gold_print_layout_fixes' ) ) {

	function luwipress_gold_print_layout_fixes() {
		if ( is_admin() ) {
			return;
		}
		$footer_bg = luwipress_gold_resolve_footer_bg_token();
		?>
<style id="lwp-gold-layout-fixes">
...
body{min-height:100vh}
	...
	add_action( 'wp_head', 'luwipress_gold_print_layout_fixes', 1 );
}

if ( ! function_exists( 'luwipress_gold_print_archive_hero_fixes' ) ) {
	 ...
	 */
	function luwipress_gold_print_archive_hero_fixes() {
		if ( is_admin() ) {
			return;
		}
		...
		?>
<style id="lwp-gold-archive-hero-fix">
...
@media (max-width:900px){
	...
	add_action( 'wp_head', 'luwipress_gold_print_archive_hero_fixes', 99 );
}

if ( ! function_exists( 'luwipress_gold_print_pdp_spacing_fix' ) ) {
	 ...
	 * `luwipress_gold_pdp_spacing_fix` filter returning false.
	 */
	function luwipress_gold_print_pdp_spacing_fix() {
		if ( is_admin() ) {
			return;
		}
		...
		if ( ! apply_filters( 'luwipress_gold_pdp_spacing_fix', true ) ) {
			return;
		}
		?>
<style id="lwp-gold-pdp-spacing-fix">
...
@media (max-width:900px){
	...
	add_action( 'wp_head', 'luwipress_gold_print_pdp_spacing_fix', 99 );
}

if ( ! function_exists( 'luwipress_gold_print_singular_top_spacing' ) ) {
	 ...
	 * has been mangled.
	 */
	function luwipress_gold_print_singular_top_spacing() {
		if ( is_admin() ) {
			return;
		}
		...
		?>
<style id="lwp-gold-singular-top-spacing">
...
.lwp-journal-tags__label{
	...
	add_action( 'wp_head', 'luwipress_gold_print_singular_top_spacing', 99 );
}
