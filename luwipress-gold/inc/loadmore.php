 ...
 * }
 */
function lwp_gold_loadmore_render( $args = array() ) {
	global $wp_query;
	$defaults = array(
		'grid'       => '.lwp-jnl-grid',
		'pagination' => '.lwp-jnl-pagination',
		'next'       => '.lwp-jnl-pagination__next a',
		'mode'       => (string) get_theme_mod( 'luwipress_gold_archive_loadmore_mode', 'infinite' ),
	...
	<div class="lwp-loadmore-wrap"
		...
		<button type="button" class="lwp-loadmore-btn" hidden><?php esc_html_e( 'Load more', 'luwipress-gold' ); ?></button>
...
add_action( 'wp_enqueue_scripts', function () {
	 ...
	 */
	if ( ! apply_filters( 'luwipress_gold_archive_loadmore', true, get_queried_object() ) ) {
		return;
	}

	$ver  = defined( 'LUWIPRESS_GOLD_VERSION' ) ? LUWIPRESS_GOLD_VERSION : '1';
	$path = get_template_directory() . '/assets/js/loadmore.js';
	$lmv  = $ver . '.' . ( file_exists( $path ) ? filemtime( $path ) : '0' );

	wp_enqueue_script( 'luwipress-gold-loadmore', LUWIPRESS_GOLD_URI . '/assets/js/loadmore.js', array(), $lmv, true );
	wp_localize_script( 'luwipress-gold-loadmore', 'LWP_GOLD_LM', array(
		'i18n' => array(
			'load_more' => __( 'Load more', 'luwipress-gold' ),
