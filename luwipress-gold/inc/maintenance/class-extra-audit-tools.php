 ...
 *
 *   - LuwiPress_Gold_Broken_Internal_Links_Tool   HEAD-check internal hrefs in post_content
 *   - LuwiPress_Gold_Empty_Term_Archives_Tool     product_cat with 0 published products
 *   - LuwiPress_Gold_WPML_Strings_Tool            WPML String Translation pending entries
 *   - LuwiPress_Gold_Page_Speed_Signals_Tool      autoload size + transient cruft + object cache
 *
...
class LuwiPress_Gold_Broken_Internal_Links_Tool {
...
class LuwiPress_Gold_Empty_Term_Archives_Tool {
...
class LuwiPress_Gold_WPML_Strings_Tool {
...
class LuwiPress_Gold_Page_Speed_Signals_Tool {
	...
	public static function scan( $args = array(), $tool = array() ) {
		...
		$probe_key   = 'lwp_gold_perf_probe_' . wp_generate_password( 6, false );
		...
		if ( $autoload_total > self::AUTOLOAD_WARN_BYTES ) {
			$candidates[] = array(
				'id'    => 'autoload_size',
				'title' => __( 'Autoload payload above safe threshold', 'luwipress-gold' ),
		...
		if ( $expired > self::TRANSIENT_WARN_ROWS ) {
			$candidates[] = array(
				'id'    => 'transient_cruft',
				'title' => __( 'Expired transients not garbage-collected', 'luwipress-gold' ),
		...
		if ( ! $cache_works ) {
			$candidates[] = array(
				...
				'title' => __( 'Object cache write/read failed', 'luwipress-gold' ),
