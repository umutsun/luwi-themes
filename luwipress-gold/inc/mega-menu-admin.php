 ...
 * `_lwp_gold_mega_featured` per top-level menu item and renders that
 ...
 * `_lwp_gold_archive_featured` so the operator's mental model is consistent
...
if ( ! function_exists( 'luwipress_gold_mega_featured_field' ) ) {
	/**
	 ...
	 */
	function luwipress_gold_mega_featured_field( $item_id, $item, $depth = 0, $args = array(), $current = 0 ) {
		// Only top-level items host a mega panel. Hide field for nested items
		// so the menu admin doesn't get cluttered.
		if ( (int) $depth > 0 ) {
			return;
		}
		$current_pid = (int) get_post_meta( $item_id, '_lwp_gold_mega_featured', true );
		$preview     = '';
		if ( $current_pid > 0 ) {
			...
			if ( $post ) {
				...
				$preview = sprintf( '#%d · %s', $current_pid, esc_html__( 'post not found', 'luwipress-gold' ) );
			}
		}
		?>
		<p class="field-lwp-gold-mega-featured description description-wide">
			<label for="lwp-gold-mega-featured-<?php echo (int) $item_id; ?>">
				<strong><?php esc_html_e( 'Mega menu — Featured product (override)', 'luwipress-gold' ); ?></strong><br>
				<input type="number"
				       min="0"
				       id="lwp-gold-mega-featured-<?php echo (int) $item_id; ?>"
				       name="lwp_gold_mega_featured[<?php echo (int) $item_id; ?>]"
				       value="<?php echo $current_pid > 0 ? (int) $current_pid : ''; ?>"
				       class="widefat"
				       placeholder="<?php esc_attr_e( 'Product or post ID — leave empty for auto best-seller', 'luwipress-gold' ); ?>" />
			</label>
			<?php if ( $preview ) : ?>
				<span class="description" style="display:block;margin-top:4px;color:#735c00;">
					<?php
					/* translators: %s: post title preview */
					printf( esc_html__( 'Currently featured: %s', 'luwipress-gold' ), esc_html( $preview ) );
					?>
				</span>
			<?php else : ?>
				<span class="description" style="display:block;margin-top:4px;">
					<?php esc_html_e( 'Auto fallback: most-popular product matching this menu item\'s URL slug.', 'luwipress-gold' ); ?>
				</span>
			<?php endif; ?>
		</p>
		<?php
	}
	add_action( 'wp_nav_menu_item_custom_fields', 'luwipress_gold_mega_featured_field', 10, 5 );
}

if ( ! function_exists( 'luwipress_gold_mega_featured_save' ) ) {
	/**
	 ...
	 */
	function luwipress_gold_mega_featured_save( $menu_id, $menu_item_db_id, $menu_item_args = array() ) {
		unset( $menu_id, $menu_item_args );
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			return;
		}
		// The custom field arrives as $_POST['lwp_gold_mega_featured'][<item_id>]
		// because every menu item posts under a single namespaced array.
		$bag = isset( $_POST['lwp_gold_mega_featured'] ) && is_array( $_POST['lwp_gold_mega_featured'] )
			? $_POST['lwp_gold_mega_featured']
			: array();
		...
		if ( $pid <= 0 ) {
			delete_post_meta( $menu_item_db_id, '_lwp_gold_mega_featured' );
			return;
		}
		...
		update_post_meta( $menu_item_db_id, '_lwp_gold_mega_featured', $pid );
	}
	add_action( 'wp_update_nav_menu_item', 'luwipress_gold_mega_featured_save', 10, 3 );
}
