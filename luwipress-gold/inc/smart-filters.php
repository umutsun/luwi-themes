 ...
 *   - Hook `luwipress_gold_smart_filters_blocks` to add / remove blocks
 *   - Use the `luwipress_gold_smart_filter_quick_toggles` filter to
...
if ( ! function_exists( 'luwipress_gold_render_smart_filters' ) ) {
	 ...
	 */
	function luwipress_gold_render_smart_filters( $current_term = null ) {
		...
		$blocks = apply_filters( 'luwipress_gold_smart_filters_blocks', [
			'active_filters' => true,
			'price'          => true,
			'attributes'     => true,
			'tags'           => true,
		...
		echo '<div class="lwp-smart-filters" role="group" aria-label="' . esc_attr__( 'Filter results', 'luwipress-gold' ) . '">';

		if ( ! empty( $blocks['active_filters'] ) ) {
			luwipress_gold_render_active_filters_chip( $widget_args );
		}
		if ( ! empty( $blocks['price'] ) ) {
			luwipress_gold_render_price_filter( $widget_args );
		}
		if ( ! empty( $blocks['attributes'] ) ) {
			luwipress_gold_render_attribute_filters( $widget_args, $current_term );
		}
		if ( ! empty( $blocks['tags'] ) ) {
			luwipress_gold_render_archive_tag_cloud( $widget_args, $current_term );
		}
		if ( ! empty( $blocks['quick_toggles'] ) ) {
			luwipress_gold_render_quick_toggles( $widget_args );
		}
...
if ( ! function_exists( 'luwipress_gold_render_active_filters_chip' ) ) {

	function luwipress_gold_render_active_filters_chip( $widget_args ) {
		// Native WC widget — only renders when at least one filter is active.
		if ( ! class_exists( 'WC_Widget_Layered_Nav_Filters' ) ) {
			return;
		}
		the_widget( 'WC_Widget_Layered_Nav_Filters', [
			'title' => __( 'Active filters', 'luwipress-gold' ),
		], $widget_args );
	}
}

if ( ! function_exists( 'luwipress_gold_render_price_filter' ) ) {

	function luwipress_gold_render_price_filter( $widget_args ) {
		if ( ! class_exists( 'WC_Widget_Price_Filter' ) ) {
			return;
		}
		the_widget( 'WC_Widget_Price_Filter', [
			'title' => __( 'Price', 'luwipress-gold' ),
		], $widget_args );
	}
}

if ( ! function_exists( 'luwipress_gold_render_attribute_filters' ) ) {
	 ...
	 */
	function luwipress_gold_render_attribute_filters( $widget_args, $current_term = null ) {
...
if ( ! function_exists( 'luwipress_gold_render_archive_tag_cloud' ) ) {
	 ...
	 * @param WP_Term|null $current_term
	 */
	function luwipress_gold_render_archive_tag_cloud( $widget_args, $current_term = null ) {
		...
		echo esc_html__( 'Popular tags', 'luwipress-gold' );
...
if ( ! function_exists( 'luwipress_gold_render_quick_toggles' ) ) {
	 ...
	 * @param array $widget_args
	 */
	function luwipress_gold_render_quick_toggles( $widget_args ) {
		// Operators may extend / replace the toggles via filter.
		$toggles = apply_filters( 'luwipress_gold_smart_filter_quick_toggles', [
			[
				'param' => 'onsale',
				'label' => __( 'On sale only', 'luwipress-gold' ),
			],
			[
				'param' => 'instock',
				'label' => __( 'In stock only', 'luwipress-gold' ),
			],
		] );
		if ( empty( $toggles ) ) {
			return;
		...
		echo $widget_args['before_widget']; // phpcs:ignore
		echo $widget_args['before_title']; // phpcs:ignore
		echo esc_html__( 'Quick filters', 'luwipress-gold' );
		echo $widget_args['after_title']; // phpcs:ignore
...
if ( ! function_exists( 'luwipress_gold_smart_filters_apply_query' ) ) {
	 ...
	 * @param WP_Query $q
	 */
	function luwipress_gold_smart_filters_apply_query( $q ) {
	...
	add_action( 'woocommerce_product_query', 'luwipress_gold_smart_filters_apply_query' );
}

if ( ! function_exists( 'luwipress_gold_smart_filters_print_styles' ) ) {

	/**
	 * Sidebar styling — inline-printed only on product archives so it
	 * doesn't pollute other pages. ~1.5 KB.
	 */
	function luwipress_gold_smart_filters_print_styles() {
		if ( is_admin() ) {
			return;
		}
		...
		?>
<style id="lwp-gold-smart-filters">
...
@media (max-width:900px){
	...
	add_action( 'wp_head', 'luwipress_gold_smart_filters_print_styles', 99 );
}
