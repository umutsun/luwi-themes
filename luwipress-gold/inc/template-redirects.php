 ...
 * Two behavior modes:
 *
 *   1. AUTO (default off, opt-in via theme_mod `luwipress_gold_resolve_slug_conflicts`)
 ...
 *   - `luwipress_gold_slug_conflict_skip_slugs` — extend / suppress the
 *     detected system-page skip list before the SQL discovery runs.
 *   - `luwipress_gold_slug_conflict_map_pre_cache` — modify the final
 ...
 * the active language).
 *
 * Operator overrides via the `luwipress_gold_hub_redirects` filter
 ...
 * auto-target:
 *
 *     add_filter( 'luwipress_gold_hub_redirects', function ( $map ) {
...
const LUWIPRESS_GOLD_SLUG_CONFLICT_OPTION    = 'luwipress_gold_resolve_slug_conflicts';

if ( ! function_exists( 'luwipress_gold_skip_redirect_slugs' ) ) {
	 ...
	 * suppress via the `luwipress_gold_slug_conflict_skip_slugs`
	 ...
	 */
	function luwipress_gold_skip_redirect_slugs() {
		$slugs = [];
		 ...
		 */
		$slugs = (array) apply_filters( 'luwipress_gold_slug_conflict_skip_slugs', $slugs );
...
if ( ! function_exists( 'luwipress_gold_resolve_slug_conflicts_enabled' ) ) {
	 ...
	 */
	function luwipress_gold_resolve_slug_conflicts_enabled() {
		...
		$mod = get_theme_mod( LUWIPRESS_GOLD_SLUG_CONFLICT_OPTION, null );
		if ( $mod !== null ) {
			return (bool) $mod;
		}
		return (bool) get_option( LUWIPRESS_GOLD_SLUG_CONFLICT_OPTION, false );
	}
}

if ( ! function_exists( 'luwipress_gold_discover_slug_conflicts' ) ) {
	 ...
	 */
	function luwipress_gold_discover_slug_conflicts() {
		if ( ! taxonomy_exists( 'product_cat' ) ) {
			return [];
		}
		$cached = get_transient( LUWIPRESS_GOLD_SLUG_CONFLICT_TRANSIENT );
		if ( is_array( $cached ) ) {
			return $cached;
		}
		...
		if ( function_exists( 'pll_get_post_translations' ) || taxonomy_exists( 'language' ) ) {
			$skip_slugs = luwipress_gold_skip_redirect_slugs();
		...
		$map = (array) apply_filters( 'luwipress_gold_slug_conflict_map_pre_cache', $map );

		set_transient( LUWIPRESS_GOLD_SLUG_CONFLICT_TRANSIENT, $map, HOUR_IN_SECONDS );
		return $map;
	}
}
...
if ( ! function_exists( 'luwipress_gold_get_hub_redirects' ) ) {
	 ...
	 */
	function luwipress_gold_get_hub_redirects() {
		$auto = luwipress_gold_resolve_slug_conflicts_enabled()
			? luwipress_gold_discover_slug_conflicts()
			: [];
		 ...
		 */
		$composed = (array) apply_filters( 'luwipress_gold_hub_redirects', $auto );
		return $composed;
	}
}

if ( ! function_exists( 'luwipress_gold_maybe_redirect_hub' ) ) {
	 ...
	 * non-GET requests.
	 */
	function luwipress_gold_maybe_redirect_hub() {
...
add_action( 'template_redirect', 'luwipress_gold_maybe_redirect_hub', 1 );

if ( ! function_exists( 'luwipress_gold_bust_slug_conflict_cache' ) ) {
	 ...
	 */
	function luwipress_gold_bust_slug_conflict_cache() {
		delete_transient( LUWIPRESS_GOLD_SLUG_CONFLICT_TRANSIENT );
	}
	add_action( 'save_post_page',         'luwipress_gold_bust_slug_conflict_cache' );
	add_action( 'deleted_post',           'luwipress_gold_bust_slug_conflict_cache' );
	add_action( 'edited_product_cat',     'luwipress_gold_bust_slug_conflict_cache' );
	add_action( 'created_product_cat',    'luwipress_gold_bust_slug_conflict_cache' );
	add_action( 'delete_product_cat',     'luwipress_gold_bust_slug_conflict_cache' );
	add_action( 'switch_theme',           'luwipress_gold_bust_slug_conflict_cache' );
	add_action( 'update_option_' . LUWIPRESS_GOLD_SLUG_CONFLICT_OPTION, 'luwipress_gold_bust_slug_conflict_cache' );
}

if ( ! function_exists( 'luwipress_gold_register_slug_conflict_customizer' ) ) {

	/**
	 * Register the opt-in toggle in Customizer → LuwiPress Gold → Performance.
	 * Falls back to creating its own section when the panel hasn't been
	 * registered yet (load-order safety).
	 */
	function luwipress_gold_register_slug_conflict_customizer( $wp_customize ) {
		...
		$wp_customize->add_setting( LUWIPRESS_GOLD_SLUG_CONFLICT_OPTION, [
			'type'              => 'theme_mod',
			'default'           => false,
		...
		$conflicts = luwipress_gold_discover_slug_conflicts();
		...
		$desc      = $count > 0
			...
			: __( 'No page/product-category slug conflicts detected. Toggle is harmless to leave on — it only redirects when a conflict actually exists.', 'luwipress-gold' );

		$wp_customize->add_control( LUWIPRESS_GOLD_SLUG_CONFLICT_OPTION, [
			'label'       => __( 'Resolve page/category slug conflicts', 'luwipress-gold' ),
			'description' => $desc,
			'section'     => $section_id,
			'type'        => 'checkbox',
			'priority'    => 20,
	...
	add_action( 'customize_register', 'luwipress_gold_register_slug_conflict_customizer', 30 );
}
