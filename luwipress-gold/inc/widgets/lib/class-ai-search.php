...
class LuwiPress_Gold_Widget_AI_Search extends Widget_Base {

	public function get_name()        { return 'lwp-ai-search'; }
	public function get_title()       { return __( 'AI Search Bar', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'badge', [
			'label'       => __( 'Badge label', 'luwipress-gold' ),
			'description' => __( 'Small pill on the right side of the bar. Defaults to "AI" — only renders when LuwiPress AI is active.', 'luwipress-gold' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => 'AI',
		] );

		$this->add_control( 'show_chips', [
			'label'        => __( 'Show suggestion chips', 'luwipress-gold' ),
			'description'  => __( 'Quick-tap suggested searches shown under the bar.', 'luwipress-gold' ),
		...
		$rep = new Repeater();
		$rep->add_control( 'label', [ 'label' => __( 'Chip label', 'luwipress-gold' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$rep->add_control( 'q',     [ 'label' => __( 'Search query', 'luwipress-gold' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );

		$this->add_control( 'chips', [
			'label'       => __( 'Chips', 'luwipress-gold' ),
		...
		$this->add_control( 'size', [
			'label'   => __( 'Size', 'luwipress-gold' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'large',
			'options' => [
				'medium' => __( 'Medium', 'luwipress-gold' ),
				'large'  => __( 'Large (hero-style)', 'luwipress-gold' ),
			],
		] );
		$this->end_controls_section();
	}
	...
	protected function lp_ai_active() {
		return function_exists( 'lwp_gold_lp_active' ) && lwp_gold_lp_active();
	}
