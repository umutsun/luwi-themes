...
class LuwiPress_Gold_Widget_Contact_CTA extends Widget_Base {

	public function get_name()        { return 'lwp-contact-cta'; }
	public function get_title()       { return __( 'Contact CTA Card', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'WhatsApp number + Telegram username come from LuwiPress → Customer Chat. Update once there; this widget and the chat launcher both refresh.', 'luwipress-gold' ) . '</em>',
			]
		);
		...
		$this->add_control( 'heading', [
			...
			'default' => __( 'Send us a note.', 'luwipress-gold' ),
		] );

		$this->add_control( 'lead', [
			'label' => __( 'Lead', 'luwipress-gold' ),
			'type'  => Controls_Manager::TEXTAREA,
			'default' => __( 'We reply within 1 working day · usually faster. Same-day on WhatsApp.', 'luwipress-gold' ),
		] );

		$this->add_control( 'email', [
			'label' => __( 'Email address', 'luwipress-gold' ),
			'description' => __( 'Used in the email row. Leave empty to hide.', 'luwipress-gold' ),
			'type'  => Controls_Manager::TEXT,
			'default' => '',
		...
		$this->add_control( 'address_line', [
			'label' => __( 'Address footnote (optional)', 'luwipress-gold' ),
			'type'  => Controls_Manager::TEXT,
			'default' => '',
		...
		$this->add_control( 'cta_label', [
			'label' => __( 'CTA button label', 'luwipress-gold' ),
			'type'  => Controls_Manager::TEXT,
			'default' => __( 'Browse the catalog →', 'luwipress-gold' ),
		] );

		$this->add_control( 'cta_url', [
			'label' => __( 'CTA button URL', 'luwipress-gold' ),
			'type'  => Controls_Manager::URL,
			'default' => [ 'url' => '/shop/', 'is_external' => '', 'nofollow' => '' ],
		] );

		$this->add_control( 'show_whatsapp', [
			'label' => __( 'Show WhatsApp', 'luwipress-gold' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		$this->add_control( 'show_telegram', [
			'label' => __( 'Show Telegram', 'luwipress-gold' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		$this->add_control( 'show_email', [
			'label' => __( 'Show Email', 'luwipress-gold' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		$this->add_control( 'show_cta', [
			'label' => __( 'Show CTA button', 'luwipress-gold' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );
		...
		$this->add_control( 'lead_color', [
			'label' => __( 'Lead color', 'luwipress-gold' ),
			'type'  => Controls_Manager::COLOR,
		...
		$this->add_control( 'cta_bg', [
			'label' => __( 'CTA button bg', 'luwipress-gold' ),
			'type'  => Controls_Manager::COLOR,
			'default' => '#9A7B3A',
		...
		$this->add_control( 'cta_color', [
			'label' => __( 'CTA button text', 'luwipress-gold' ),
			'type'  => Controls_Manager::COLOR,
			'default' => '#1A1612',
		...
		$this->add_responsive_control( 'card_padding', [
			'label' => __( 'Card padding', 'luwipress-gold' ),
		...
		$this->add_control( 'card_radius', [
			'label' => __( 'Border radius', 'luwipress-gold' ),
			'type'  => Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px' ],
