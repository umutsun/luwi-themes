...
class LuwiPress_Gold_Widget_Countdown extends Widget_Base {

	public function get_name()        { return 'lwp-countdown'; }
	public function get_title()       { return __( 'Countdown Timer', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_target', [ 'label' => __( 'Target', 'luwipress-gold' ) ] );

		$this->add_control( 'target_date', [
			'label'       => __( 'Target date & time', 'luwipress-gold' ),
			'description' => __( 'Picked in your site timezone (WP general settings) and rendered live in the visitor\'s browser.', 'luwipress-gold' ),
			'type'        => Controls_Manager::DATE_TIME,
			'default'     => gmdate( 'Y-m-d H:i:s', strtotime( '+7 days' ) ),
		] );

		$this->add_control( 'show_units', [
			'label'       => __( 'Units to show', 'luwipress-gold' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'dhms',
			'options'     => [
				'dhms' => __( 'Days · Hours · Minutes · Seconds', 'luwipress-gold' ),
				'dhm'  => __( 'Days · Hours · Minutes', 'luwipress-gold' ),
				'hms'  => __( 'Hours · Minutes · Seconds', 'luwipress-gold' ),
			],
		] );

		$this->add_control( 'lbl_days',    [ 'label' => __( 'Label — days', 'luwipress-gold' ),    'type' => Controls_Manager::TEXT, 'default' => __( 'Days', 'luwipress-gold' ) ] );
		$this->add_control( 'lbl_hours',   [ 'label' => __( 'Label — hours', 'luwipress-gold' ),   'type' => Controls_Manager::TEXT, 'default' => __( 'Hours', 'luwipress-gold' ) ] );
		$this->add_control( 'lbl_minutes', [ 'label' => __( 'Label — minutes', 'luwipress-gold' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Minutes', 'luwipress-gold' ) ] );
		$this->add_control( 'lbl_seconds', [ 'label' => __( 'Label — seconds', 'luwipress-gold' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Seconds', 'luwipress-gold' ) ] );

		$this->add_control( 'expired_msg', [
			'label'   => __( 'Post-expiry message', 'luwipress-gold' ),
			'type'    => Controls_Manager::TEXTAREA,
			'rows'    => 2,
			'default' => __( "It's live. Browse the sale →", 'luwipress-gold' ),
		] );
		$this->add_control( 'expired_url', [
			'label'   => __( 'Post-expiry button URL (optional)', 'luwipress-gold' ),
			'type'    => Controls_Manager::URL,
			'default' => [ 'url' => '' ],
		] );
		$this->add_control( 'expired_btn', [
			'label'   => __( 'Post-expiry button label (optional)', 'luwipress-gold' ),
		...
		$this->add_control( 'align', [
			...
			'type'    => Controls_Manager::SELECT,
			'default' => 'center',
			'options' => [ 'left' => __( 'Left', 'luwipress-gold' ), 'center' => __( 'Center', 'luwipress-gold' ), 'right' => __( 'Right', 'luwipress-gold' ) ],
		] );
		$this->end_controls_section();
	}
