...
class LuwiPress_Gold_Widget_Featured_Strip extends Widget_Base {

	public function get_name()        { return 'lwp-featured-strip'; }
	public function get_title()       { return __( 'Featured Strip', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'count', [
			'label'   => __( 'Max items', 'luwipress-gold' ),
		...
		$this->add_control( 'fallback', [
			'label'       => __( 'Fallback when registry is empty', 'luwipress-gold' ),
			'description' => __( 'What to show when no products are flipped to featured. Falls back automatically — the operator does not see the strip disappear during onboarding.', 'luwipress-gold' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'bestseller',
			'options'     => [
				'bestseller' => __( 'Best-selling products', 'luwipress-gold' ),
				'onsale'     => __( 'On-sale products', 'luwipress-gold' ),
				'recent'     => __( 'Recently added', 'luwipress-gold' ),
				'hide'       => __( 'Hide the strip', 'luwipress-gold' ),
			],
		] );

		$this->add_control( 'show_note', [
			'label'        => __( 'Show operator note chips', 'luwipress-gold' ),
			'description'  => __( 'Renders the operator note from the meta box ("Boxing Week pick", "New arrival") as a small chip on each card.', 'luwipress-gold' ),
	...
	protected function get_ids( $s ) {
		...
		if ( function_exists( 'lwp_gold_get_featured_ids' ) ) {
			$ids = lwp_gold_get_featured_ids( [ 'number' => $count ] );
		}
		...
		<section class="lwp-fs">
			...
			<div class="lwp-fs__track">
				<?php foreach ( $ids as $pid ) :
					...
					$note = $show_note ? (string) get_post_meta( $pid, '_luwipress_gold_featured_note', true ) : '';
					?>
