 ...
 *      setting under LuwiPress Gold → Footer → Social (also exposed in
...
class LuwiPress_Gold_Widget_Instagram_Channel extends Widget_Base {

	public function get_name()        { return 'lwp-instagram-channel'; }
	public function get_title()       { return __( 'Instagram Channel', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		/* Section: Heading */
		$this->start_controls_section( 'section_head', [ 'label' => __( 'Heading', 'luwipress-gold' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'Instagram profile →', 'luwipress-gold' ),
			]
		);
		$this->add_control(
			'cta_url',
		...
		$this->add_control(
			'show_follow',
			[
				'label'        => __( 'Show Follow button', 'luwipress-gold' ),
				'description'  => __( 'Only renders if a profile URL is set (here or in Customize → Footer → Instagram URL).', 'luwipress-gold' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => __( 'Yes', 'luwipress-gold' ),
				'label_off'    => __( 'No', 'luwipress-gold' ),
				'return_value' => 'yes',
				'default'      => 'yes',
			]
		);
		$this->add_control(
			'follow_label',
			[
				'label'     => __( 'Follow label', 'luwipress-gold' ),
				'type'      => Controls_Manager::TEXT,
				'default'   => __( 'Follow', 'luwipress-gold' ),
		...
		$this->start_controls_section( 'section_posts', [ 'label' => __( 'Posts', 'luwipress-gold' ) ] );

		$repeater = new Repeater();
		$repeater->add_control(
			'thumbnail',
			[
				'label'   => __( 'Thumbnail (square)', 'luwipress-gold' ),
				'description' => __( 'Upload a square crop (recommended 600×600 or 1080×1080). Falls back to a solid tile if blank.', 'luwipress-gold' ),
		...
		$repeater->add_control(
			'post_url',
			[
				'label'       => __( 'Instagram post URL', 'luwipress-gold' ),
				'description' => __( 'Full post URL, e.g. https://www.instagram.com/p/AbC123XyZ/', 'luwipress-gold' ),
				'type'        => Controls_Manager::TEXT,
		...
		$repeater->add_control(
			'caption',
			[
				'label'       => __( 'Caption', 'luwipress-gold' ),
				'description' => __( 'Short overlay text shown on hover.', 'luwipress-gold' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'meta',
			[
				'label'       => __( 'Meta line', 'luwipress-gold' ),
				'description' => __( 'Optional small badge (e.g. "Reel · 12k likes" or "📷 Photo").', 'luwipress-gold' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'kind',
			[
				'label'   => __( 'Post type icon', 'luwipress-gold' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'post',
				'options' => [
					'post'     => __( 'Photo', 'luwipress-gold' ),
					'reel'     => __( 'Reel', 'luwipress-gold' ),
					'carousel' => __( 'Carousel', 'luwipress-gold' ),
				],
			]
		);
		...
		$this->add_control(
			'columns',
			[
				'label'   => __( 'Columns (desktop)', 'luwipress-gold' ),
				'type'    => Controls_Manager::SELECT,
				'default' => '4',
		...
		$this->add_control(
			'title_color',
			[
				'label'     => __( 'Title color', 'luwipress-gold' ),
	...
	protected function resolve_profile_url( $widget_url ) {
		...
		$mod = get_theme_mod( 'luwipress_gold_social_instagram', '' );
	...
	protected function render() {
		...
		$follow_label = trim( (string) ( $s['follow_label'] ?? '' ) ) ?: __( 'Follow', 'luwipress-gold' );
		...
		<section class="lwp-igc" data-columns="<?php echo esc_attr( $columns ); ?>">
					...
					<div class="lwp-igc-head__actions">
						<?php if ( $show_follow ) : ?>
							<a class="lwp-igc-follow"
								...
								aria-label="<?php echo esc_attr( sprintf( __( 'Follow %s on Instagram', 'luwipress-gold' ), $handle ?: '' ) ); ?>">
			...
			<?php if ( empty( $resolved ) ) : ?>
				<div class="lwp-ytc-placeholder" role="note">
					<strong><?php esc_html_e( 'No Instagram posts configured yet.', 'luwipress-gold' ); ?></strong>
					<span><?php esc_html_e( 'Open this widget and upload a square thumbnail + paste the Instagram post URL for each Posts row. This notice is only visible to logged-in editors.', 'luwipress-gold' ); ?></span>
				</div>
			<?php else : ?>
			<div class="lwp-igc-grid">
				<?php foreach ( $resolved as $item ) :
					...
					$href    = $url !== '' ? $url : '#';
					$aria    = $caption !== '' ? sprintf( __( 'Open Instagram post — %s', 'luwipress-gold' ), $caption ) : __( 'Open Instagram post', 'luwipress-gold' );
					?>
