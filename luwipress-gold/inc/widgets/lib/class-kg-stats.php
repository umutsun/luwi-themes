...
class LuwiPress_Gold_Widget_KG_Stats extends Widget_Base {

	public function get_name()        { return 'lwp-kg-stats'; }
	public function get_title()       { return __( 'Knowledge Graph · Stats', 'luwipress-gold' ); }
	...
	protected function fetch_kg() {
		...
		$cached = get_transient( 'lwp_gold_kg_summary' );
		...
		if ( is_array( $data ) ) {
			set_transient( 'lwp_gold_kg_summary', $data, 5 * MINUTE_IN_SECONDS );
		}
		return $data;
	}
	...
	protected function render_upsell() {
		?>
		<div class="lwp-kgstats lwp-kgstats--upsell" role="region" aria-label="<?php esc_attr_e( 'Install LuwiPress to enable live store stats', 'luwipress-gold' ); ?>">
			<div class="lwp-kgstats__upsell-inner">
				...
				<h3 class="lwp-kgstats__upsell-title">
					<?php esc_html_e( 'Unlock live Knowledge Graph data.', 'luwipress-gold' ); ?>
				</h3>
				<p class="lwp-kgstats__upsell-lead">
					<?php esc_html_e( 'Install the free LuwiPress plugin to surface live product counts, category coverage, master roster and countries-shipped on this widget — pulled directly from your store data.', 'luwipress-gold' ); ?>
				</p>
	...
	protected function render() {
		...
		if ( ! function_exists( 'lwp_gold_lp_active' ) || ! lwp_gold_lp_active() ) {
			$this->render_upsell();
			return;
		}
		...
		<div class="lwp-kgstats" data-live="1">
			<?php if ( $eyebrow || $heading ) : ?>
				<header class="lwp-kgstats__head">
					...
					<span class="lwp-kgstats__badge" title="<?php esc_attr_e( 'Powered by LuwiPress Knowledge Graph', 'luwipress-gold' ); ?>">
						<?php esc_html_e( 'Live · LuwiPress KG', 'luwipress-gold' ); ?>
					</span>
				</header>
			<?php endif; ?>
