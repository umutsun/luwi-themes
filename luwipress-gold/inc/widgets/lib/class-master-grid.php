...
class LuwiPress_Gold_Widget_Master_Grid extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_query', [ 'label' => __( 'Source', 'luwipress-gold' ) ] );

		$this->add_control(
			'info',
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'Pulls live from the Vendors CPT (LuwiPress → Vendors). Add or edit profiles there; this grid updates automatically.', 'luwipress-gold' ) . '</em>',
			]
		);
		...
		$this->add_control(
			'limit',
			[
				'label'   => __( 'Max profiles', 'luwipress-gold' ),
		...
		$group_options = [ '' => __( '— All vendors —', 'luwipress-gold' ) ];
		...
		$this->add_control(
			'vendor_group',
			[
				'label'       => __( 'Vendor group', 'luwipress-gold' ),
				'description' => __( 'Filter to one vendor group (Luthiers, Team, etc.). Manage groups under LuwiPress → Vendors → Vendor Groups.', 'luwipress-gold' ),
		...
		$this->add_control(
			'include_ids',
			[
				'label'       => __( 'Curated IDs (optional)', 'luwipress-gold' ),
				'description' => __( 'Comma-separated CPT IDs to show in this order. Leave empty to auto-query (menu_order ASC).', 'luwipress-gold' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => 'menu_order',
				'options' => [
					'menu_order' => __( 'Manual (menu order)', 'luwipress-gold' ),
					'date'       => __( 'Date published', 'luwipress-gold' ),
					'title'      => __( 'Name (A→Z)', 'luwipress-gold' ),
		...
		$this->add_control(
			'order',
			[
				'label'   => __( 'Order direction', 'luwipress-gold' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'ASC',
		...
		/* Display section */
		$this->start_controls_section( 'section_display', [ 'label' => __( 'Display', 'luwipress-gold' ) ] );
		...
		$this->add_control(
			'show_location',
			[ 'label' => __( 'Show location', 'luwipress-gold' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);

		$this->add_control(
			'show_specialty',
			[ 'label' => __( 'Show specialty', 'luwipress-gold' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);

		$this->add_control(
			'show_count',
			[
				'label'       => __( 'Show product count', 'luwipress-gold' ),
				'description' => __( 'Counts WC products attributed to this vendor via _lwp_vendor_ids meta.', 'luwipress-gold' ),
		...
		$this->add_control(
			'view_all_link',
			[
				'label'        => __( 'Show "View all" link', 'luwipress-gold' ),
				'description'  => __( 'Adds a CTA below the grid linking to the full Vendors archive (e.g. /luthiers/). Useful when the grid is capped at 4–8 and visitors need a path to the rest.', 'luwipress-gold' ),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'default'      => 'yes',
			]
		...
		$this->add_control(
			...
			[
				...
				'type'        => Controls_Manager::TEXT,
				'default'     => __( 'View all vendors →', 'luwipress-gold' ),
				'placeholder' => __( 'View all vendors →', 'luwipress-gold' ),
	...
	protected function render() {
		...
		if ( ! post_type_exists( 'lwp_vendor' ) ) {
			echo '<div class="lwp-mgrid lwp-mgrid--placeholder"><em>' . esc_html__( '[Master Grid] LuwiPress Vendors module is inactive — enable it in LuwiPress → Vendors.', 'luwipress-gold' ) . '</em></div>';
			return;
		}
		...
		$q = new WP_Query( $args );
		if ( ! $q->have_posts() ) {
			echo '<div class="lwp-mgrid lwp-mgrid--empty"><em>' . esc_html__( '[Master Grid] No profiles yet. Add them in LuwiPress → Vendors.', 'luwipress-gold' ) . '</em></div>';
			return;
		}
				...
				if ( $show_count ) {
					...
					if ( $cnt > 0 ) {
						$count_label = sprintf(
							...
							_n( '%d work', '%d works', $cnt, 'luwipress-gold' ),
							$cnt
						);
					}
				}
		...
		if ( ( $s['view_all_link'] ?? 'yes' ) === 'yes' ) {
			...
			if ( $archive_url ) {
				...
				if ( $label === '' ) {
					$label = __( 'View all vendors →', 'luwipress-gold' );
				}
				?>
