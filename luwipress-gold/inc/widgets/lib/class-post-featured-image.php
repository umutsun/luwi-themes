...
class LuwiPress_Gold_Widget_Post_Featured_Image extends Widget_Base {

	public function get_name()        { return 'lwp-post-featured-image'; }
	public function get_title()       { return __( 'Post Featured Image', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Image', 'luwipress-gold' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'file'  => __( 'Image file', 'luwipress-gold' ),
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					'none'       => __( 'None', 'luwipress-gold' ),
					'attachment' => __( 'Attachment caption', 'luwipress-gold' ),
				],
			]
		);
		...
		$this->add_responsive_control( 'max_height', [
			'label'      => __( 'Max height', 'luwipress-gold' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px', 'vh' ],
			'range'      => [
				'px' => [ 'min' => 100, 'max' => 1200 ],
		...
		$this->add_control( 'object_fit', [
			'label'   => __( 'Object fit', 'luwipress-gold' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'cover',
			'options' => [
				'cover'   => __( 'Cover', 'luwipress-gold' ),
				'contain' => __( 'Contain', 'luwipress-gold' ),
				'fill'    => __( 'Fill', 'luwipress-gold' ),
				'none'    => __( 'None', 'luwipress-gold' ),
			],
	...
	protected function render() {
		...
		$post = get_post();
		if ( ! $post ) {
			echo '<div class="lwp-post-featured-image lwp-post-featured-image--placeholder"><div class="lwp-fpi-ph">' . esc_html__( '[Featured image]', 'luwipress-gold' ) . '</div></div>';
			return;
		}

		$thumb_id = (int) get_post_thumbnail_id( $post );
		if ( ! $thumb_id ) {
			echo '<div class="lwp-post-featured-image lwp-post-featured-image--placeholder"><div class="lwp-fpi-ph">' . esc_html__( '[No featured image]', 'luwipress-gold' ) . '</div></div>';
			return;
		}
