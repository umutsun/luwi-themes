<?php
/**
 * Widget: LuwiPress Gold Product Card.
 *
...
class LuwiPress_Gold_Widget_Product_Card extends Widget_Base {

	public function get_name()        { return 'lwp-product-card'; }
	public function get_title()       { return __( 'Product Card', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			'product_id',
			[
				'label'       => __( 'Product', 'luwipress-gold' ),
				'type'        => Controls_Manager::SELECT2,
				'options'     => $this->get_product_options(),
				'description' => __( 'Pick the product to render.', 'luwipress-gold' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			'show_badge',
			[
				'label'        => __( 'Show sale badge', 'luwipress-gold' ),
		...
		$this->add_control(
			'show_favourite',
			[
				'label'   => __( 'Show favourite heart', 'luwipress-gold' ),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
			]
		);
		...
		$this->add_control(
			'show_quick_add',
			[
				'label'   => __( 'Show quick-add on hover', 'luwipress-gold' ),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
			]
		);
		...
		$this->add_control(
			'show_maker',
			[
				'label'       => __( 'Show maker name', 'luwipress-gold' ),
				'type'        => Controls_Manager::SWITCHER,
				'default'     => 'yes',
				'description' => __( 'Reads the pa_luthier attribute if available.', 'luwipress-gold' ),
			]
		);
		...
		$this->add_control(
			'card_radius',
			[
				'label'   => __( 'Card corner radius', 'luwipress-gold' ),
		...
		$this->add_control(
			'badge_bg',
			[
				'label'     => __( 'Sale badge color', 'luwipress-gold' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#a33b3e',
				'selectors' => [
		...
		$this->add_control(
			'price_color',
			[
				'label'     => __( 'Price color', 'luwipress-gold' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#735c00',
				'selectors' => [
	...
	private function get_product_options() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return [ '' => __( 'Install WooCommerce to pick products', 'luwipress-gold' ) ];
		}
		$products = get_posts( [
			'post_type'      => 'product',
			'post_status'    => 'publish',
		...
		$out = [ '' => __( '— Select a product —', 'luwipress-gold' ) ];
	...
	protected function render() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			$this->render_placeholder( __( 'WooCommerce required.', 'luwipress-gold' ) );
			return;
		}
		$settings = $this->get_settings_for_display();
		$pid      = (int) ( $settings['product_id'] ?? 0 );
		if ( ! $pid ) {
			$this->render_placeholder( __( 'Pick a product in the panel →', 'luwipress-gold' ) );
			return;
		}
		$product = wc_get_product( $pid );
		if ( ! $product ) {
			$this->render_placeholder( __( 'Product not found.', 'luwipress-gold' ) );
			return;
		}
		...
		<a class="lwp-pc product-card" href="<?php echo esc_url( get_permalink( $pid ) ); ?>">
			<div class="lwp-pc-img">
				...
				<?php if ( $settings['show_favourite'] === 'yes' ) : ?>
					<button type="button" class="lwp-pc-fav" aria-label="<?php esc_attr_e( 'Save', 'luwipress-gold' ); ?>" onclick="event.preventDefault();event.stopPropagation();">♡</button>
				<?php endif; ?>
				<?php echo $product->get_image( 'woocommerce_thumbnail' ); ?>
				<?php if ( $settings['show_quick_add'] === 'yes' ) : ?>
					<span class="lwp-pc-quick-add quick-add" data-product-id="<?php echo esc_attr( $pid ); ?>"><?php esc_html_e( 'Quick add', 'luwipress-gold' ); ?></span>
				<?php endif; ?>
			</div>
			<div class="lwp-pc-meta">
				...
				<?php if ( $maker && $settings['show_maker'] === 'yes' ) : ?>
					<span class="lwp-pc-maker"><?php
						...
						printf( esc_html__( 'by %s', 'luwipress-gold' ), esc_html( $maker ) );
					?></span>
				<?php endif; ?>
