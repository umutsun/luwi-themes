...
class LuwiPress_Gold_Widget_Product_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-product-grid'; }
	public function get_title()       { return __( 'Product Grid', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'heading', [
			'label'       => __( 'Heading (optional)', 'luwipress-gold' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => __( 'e.g. Most popular', 'luwipress-gold' ),
		] );
		...
		$this->add_control( 'source', [
			...
			'default' => 'query',
			'options' => [
				'query'    => __( 'Auto query (by orderby)', 'luwipress-gold' ),
				'featured' => __( 'Featured products only', 'luwipress-gold' ),
				'manual'   => __( 'Manual ID list', 'luwipress-gold' ),
			],
		] );

		$this->add_control( 'manual_ids', [
			'label'       => __( 'Product IDs (comma or space separated)', 'luwipress-gold' ),
			'type'        => Controls_Manager::TEXTAREA,
			'default'     => '',
			'placeholder' => '123, 456, 789',
			'condition'   => [ 'source' => 'manual' ],
			'description' => __( 'Order in this list is preserved. Limit + Orderby ignored.', 'luwipress-gold' ),
		] );

		$this->add_control( 'limit', [
			'label'     => __( 'Number of products', 'luwipress-gold' ),
		...
		$this->add_control( 'orderby', [
			...
			'default'   => 'popularity',
			'options'   => [
				'popularity' => __( 'Popularity (sales count)', 'luwipress-gold' ),
				'rating'     => __( 'Average rating', 'luwipress-gold' ),
				'date'       => __( 'Newest first', 'luwipress-gold' ),
				'price'      => __( 'Price (low → high)', 'luwipress-gold' ),
				'price-desc' => __( 'Price (high → low)', 'luwipress-gold' ),
				'title'      => __( 'Title A → Z', 'luwipress-gold' ),
				'menu_order' => __( 'Menu order', 'luwipress-gold' ),
		...
		$this->add_control( 'category', [
			'label'       => __( 'Limit to category (slug, optional)', 'luwipress-gold' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'darbuka',
			'condition'   => [ 'source!' => 'manual' ],
			'description' => __( 'Single product_cat slug. Leave empty for all categories.', 'luwipress-gold' ),
		] );

		$this->add_control( 'exclude_out_of_stock', [
			'label'        => __( 'Hide out-of-stock', 'luwipress-gold' ),
		...
		$this->add_responsive_control( 'gap', [
			'label'      => __( 'Grid gap', 'luwipress-gold' ),
		...
		$this->add_control( 'extra_class', [
			'label'       => __( 'Extra wrapper class', 'luwipress-gold' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'tap-products',
			'description' => __( 'Preserves legacy CSS hooks when migrating from a [products] shortcode (e.g. "tap-products"). Optional.', 'luwipress-gold' ),
		] );
	...
	protected function render() {
		...
		if ( empty( $products ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div class="lwp-product-grid lwp-product-grid--empty"><em>' . esc_html__( '[Product Grid] No products match the current query.', 'luwipress-gold' ) . '</em></div>';
			}
			return;
		}
		...
		<li class="product lwp-pcard">
			<a href="<?php echo esc_url( $link ); ?>" class="lwp-pcard-link">
				<div class="lwp-pcard-media">
					...
					<?php if ( $onsale ) : ?>
						<span class="lwp-pcard-badge"><?php esc_html_e( 'Sale', 'luwipress-gold' ); ?></span>
					<?php endif; ?>
				</div>
