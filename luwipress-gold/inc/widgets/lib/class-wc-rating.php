...
class LuwiPress_Gold_Widget_WC_Rating extends Widget_Base {

	public function get_name()        { return 'lwp-wc-rating'; }
	public function get_title()       { return __( 'WC Product Rating', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Rating', 'luwipress-gold' ) ] );

		$this->add_control( 'show_count', [
			'label' => __( 'Show review count', 'luwipress-gold' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );

		$this->add_control( 'hide_when_empty', [
			'label' => __( 'Hide when no reviews', 'luwipress-gold' ),
		...
		$this->add_control( 'star_color', [
			'label' => __( 'Star color', 'luwipress-gold' ), 'type' => Controls_Manager::COLOR, 'default' => '#9A7B3A',
	...
	protected function render() {
		...
		if ( ( $s['show_count'] ?? 'yes' ) === 'yes' ) {
			$count_label = $count === 0
				? esc_html__( 'No reviews yet', 'luwipress-gold' )
				: sprintf( _n( '%d review', '%d reviews', $count, 'luwipress-gold' ), $count );
