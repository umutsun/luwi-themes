 ...
 *      under LuwiPress Gold → Footer → Social, so the user enters it
...
class LuwiPress_Gold_Widget_YouTube_Channel extends Widget_Base {

	public function get_name()        { return 'lwp-youtube-channel'; }
	public function get_title()       { return __( 'YouTube Channel', 'luwipress-gold' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'YouTube channel →', 'luwipress-gold' ),
			]
		);
		$this->add_control(
			'cta_url',
		...
		$this->add_control(
			'show_subscribe',
			[
				'label'        => __( 'Show Subscribe button', 'luwipress-gold' ),
				'description'  => __( 'Only renders if a channel URL is set (here or in Customize → Footer → YouTube URL).', 'luwipress-gold' ),
		...
		$this->add_control(
			'subscribe_label',
			[
				'label'     => __( 'Subscribe label', 'luwipress-gold' ),
				'type'      => Controls_Manager::TEXT,
				'default'   => __( 'Subscribe', 'luwipress-gold' ),
		...
		$this->start_controls_section( 'section_videos', [ 'label' => __( 'Videos', 'luwipress-gold' ) ] );

		$repeater = new Repeater();
		$repeater->add_control(
			'video',
			[
				'label'       => __( 'YouTube URL or ID', 'luwipress-gold' ),
				'description' => __( 'Paste the full URL (youtu.be/… or youtube.com/watch?v=…) or just the 11-char ID.', 'luwipress-gold' ),
				'type'        => Controls_Manager::TEXT,
		...
		$repeater->add_control(
			'duration',
			[
				'label'       => __( 'Duration badge', 'luwipress-gold' ),
				'description' => __( 'Optional. Shown as a small badge on the thumbnail (e.g. "3:42"). Leave empty to hide.', 'luwipress-gold' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'thumb_quality',
			[
				'label'   => __( 'Thumbnail quality', 'luwipress-gold' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'hq',
				'options' => [
					'hq'      => __( 'HQ (480×360)', 'luwipress-gold' ),
					'mq'      => __( 'Standard (320×180)', 'luwipress-gold' ),
					'maxres'  => __( 'Maxres (1280×720, may 404 on old videos)', 'luwipress-gold' ),
				],
			]
		);
		...
		$this->add_control(
			'videos',
			[
				'label'       => __( 'Videos', 'luwipress-gold' ),
		...
		$this->add_control(
			'card_bg',
			[
				'label'     => __( 'Card background', 'luwipress-gold' ),
		...
		$this->add_control(
			'card_border',
			[
				'label'     => __( 'Card border', 'luwipress-gold' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
	...
	protected function resolve_channel_url( $widget_url ) {
		...
		$mod = get_theme_mod( 'luwipress_gold_social_youtube', '' );
	...
	protected function render() {
		...
		$subscribe_label = trim( (string) ( $s['subscribe_label'] ?? '' ) ) ?: __( 'Subscribe', 'luwipress-gold' );
		...
		<section class="lwp-ytc" data-columns="<?php echo esc_attr( $columns ); ?>">
					...
					<div class="lwp-ytc-head__actions">
						<?php if ( $show_subscribe ) : ?>
							<a class="lwp-ytc-sub"
								...
								aria-label="<?php echo esc_attr( sprintf( __( '%s on YouTube', 'luwipress-gold' ), $subscribe_label ) ); ?>">
			...
			<?php if ( empty( $resolved ) ) : ?>
				<div class="lwp-ytc-placeholder" role="note">
					<strong><?php esc_html_e( 'No YouTube videos configured yet.', 'luwipress-gold' ); ?></strong>
					<span><?php esc_html_e( 'Open this widget and paste a YouTube URL or video ID into each Videos row. This notice is only visible to logged-in editors.', 'luwipress-gold' ); ?></span>
				</div>
			<?php else : ?>
			<div class="lwp-ytc-grid">
				<?php foreach ( $resolved as $item ) :
					...
					$aria     = $title !== '' ? sprintf( __( 'Play video — %s', 'luwipress-gold' ), $title ) : __( 'Play video', 'luwipress-gold' );
					?>
