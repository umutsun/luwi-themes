...
$tabs_default = [
	'all'      => __( 'All', 'luwipress-gold' ),
	'product'  => __( 'Products', 'luwipress-gold' ),
	'master'   => __( 'Masters', 'luwipress-gold' ),
	'post'     => __( 'Journal', 'luwipress-gold' ),
];
 ...
 * @since 1.5.4
 */
$tabs = apply_filters( 'luwipress_gold_search_tabs', $tabs_default );
...
<main class="lwp-page lwp-search-page" id="primary">
	...
	<div class="lwp-search-bar">
		<button type="button" class="back" onclick="history.length > 1 ? history.back() : (location.href='<?php echo esc_js( home_url( '/' ) ); ?>')" aria-label="<?php esc_attr_e( 'Back', 'luwipress-gold' ); ?>" style="width:36px;height:36px;border-radius:50%;border:1px solid var(--line);font-family:var(--mono);font-size:14px;flex:none;background:var(--bg);cursor:pointer">‹</button>
		<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="field" style="margin:0">
			<span aria-hidden="true">⌕</span>
			<label for="lwp-search-input" class="screen-reader-text"><?php esc_html_e( 'Search', 'luwipress-gold' ); ?></label>
			<input id="lwp-search-input" name="s" type="search" value="<?php echo esc_attr( $query ); ?>" autocapitalize="off" spellcheck="false" placeholder="<?php esc_attr_e( 'Search instruments, masters…', 'luwipress-gold' ); ?>" />
			<?php if ( defined( 'LUWIPRESS_VERSION' ) ) : ?>
				<span class="ai-tag" aria-label="<?php esc_attr_e( 'AI-powered search', 'luwipress-gold' ); ?>">AI</span>
			<?php endif; ?>
		</form>
	</div>

	<?php if ( count( $tabs ) > 1 ) : ?>
	<nav class="lwp-search-tabs" aria-label="<?php esc_attr_e( 'Search filters', 'luwipress-gold' ); ?>">
	...
	<div style="padding:10px 16px;font-family:var(--mono);font-size:10.5px;letter-spacing:.14em;color:var(--muted);text-transform:uppercase;display:flex;justify-content:space-between;background:var(--bg-alt);border-bottom:1px solid var(--line)">
		<span><?php
			...
			printf(
				esc_html( _n( '%1$d result · "%2$s"', '%1$d results · "%2$s"', $total_found, 'luwipress-gold' ) ),
				$total_found,
				esc_html( $query )
			);
		?></span>
	...
	<?php if ( have_posts() ) : ?>
		...
		the_posts_pagination( [
			'mid_size'  => 1,
			'prev_text' => __( '← Newer', 'luwipress-gold' ),
			'next_text' => __( 'Older →', 'luwipress-gold' ),
		] );
		?>
