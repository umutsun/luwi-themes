...
while ( have_posts() ) : the_post();
	...
	$audio_label  = function_exists( 'get_field' ) ? get_field( 'master_audio_label', $post_id ) : __( 'Hicaz', 'luwipress-gold' );
...
<main class="lwp-page lwp-master-single" id="primary">
	...
	<div class="master-portrait" <?php if ( $portrait ) : ?>style="background-image:linear-gradient(135deg,rgba(61,47,31,.4),rgba(122,90,44,.6)),url('<?php echo esc_url( $portrait ); ?>');background-size:cover;background-position:center"<?php endif; ?>>
		<div style="display:flex;flex-direction:column;gap:6px">
			<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--primary-light, #D4AF37)">
				<?php esc_html_e( 'Master luthier', 'luwipress-gold' ); ?>
			</span>
			<h1><?php
				$title = get_the_title();
	...
	<div class="master-meta">
		...
		<div class="cell">
			...
			<span class="l"><?php esc_html_e( 'Years', 'luwipress-gold' ); ?></span>
		</div>
		<?php endif; ?>
		<?php if ( $pieces ) : ?>
		<div class="cell">
			<span class="n"><?php echo esc_html( $pieces ); ?></span>
			<span class="l"><?php esc_html_e( 'Pieces', 'luwipress-gold' ); ?></span>
		</div>
		<?php endif; ?>
		<?php if ( $joined_year ) : ?>
		<div class="cell">
			<span class="n"><?php echo esc_html( $joined_year ); ?></span>
			<span class="l"><?php esc_html_e( 'Joined', 'luwipress-gold' ); ?></span>
		</div>
		<?php endif; ?>
	</div>

	<section style="padding:24px 20px;display:flex;flex-direction:column;gap:12px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( 'Bio', 'luwipress-gold' ); ?></span>
	...
	<section style="background:var(--bg-alt);padding:24px 20px;display:flex;flex-direction:column;gap:14px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( 'Signature instruments', 'luwipress-gold' ); ?></span>
		<h2 style="font-family:var(--serif);font-size:22px;font-weight:500;margin:0"><?php
			/* translators: %d signature piece count */
			printf( esc_html( _n( '%d piece in circulation', '%d pieces in circulation', (int) $pieces, 'luwipress-gold' ) ), (int) $pieces );
		?></h2>
		...
		<a href="<?php echo esc_url( add_query_arg( [ 'master' => $post_id ], wc_get_page_permalink( 'shop' ) ) ); ?>" class="cta-link" style="font-family:var(--mono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--primary);align-self:flex-start"><?php
			...
			printf( esc_html__( 'Browse all from %s →', 'luwipress-gold' ), esc_html( get_the_title( $post_id ) ) );
		?></a>
	</section>
	...
	<?php if ( $audio_url ) : ?>
	<section style="padding:24px 20px;display:flex;flex-direction:column;gap:12px">
		<span class="eyebrow" style="font-family:var(--mono);font-size:9.5px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted)"><?php esc_html_e( 'Listen', 'luwipress-gold' ); ?></span>
		<h2 style="font-family:var(--serif);font-size:22px;font-weight:500;margin:0"><?php
			/* translators: %s: makam name (e.g. Hicaz) */
			printf( esc_html__( 'Tuning sample · %s', 'luwipress-gold' ), '<em>' . esc_html( $audio_label ) . '</em>' );
		?></h2>
		<div class="lwp-audio-card" data-audio-src="<?php echo esc_url( $audio_url ); ?>">
			<button class="play" aria-label="<?php esc_attr_e( 'Play audio sample', 'luwipress-gold' ); ?>">▶</button>
			<div class="progress">
	...
	<section style="background:#15110b;color:#f6f3f2;padding:24px 20px;text-align:center;display:flex;flex-direction:column;gap:12px">
		...
		<a href="#" class="cta-pill" style="align-self:center;padding:12px 22px;background:var(--primary);color:#fff;border-radius:999px;font-family:var(--mono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;text-decoration:none">
			...
			printf( esc_html__( "Visit %s's atelier →", 'luwipress-gold' ), esc_html( get_the_title( $post_id ) ) );
			?>
		</a>
	</section>
	<?php endif; ?>
