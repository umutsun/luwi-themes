...
<div class="lwp-auth-grid">
	...
	<section class="lwp-auth-card">
		<h2><?php esc_html_e( 'Welcome back', 'luwipress-gold' ); ?></h2>
		<p class="sub"><?php esc_html_e( 'Sign in to view orders, saved instruments and addresses.', 'luwipress-gold' ); ?></p>
		...
		<form class="woocommerce-form woocommerce-form-login login" method="post">
			...
			<div class="lwp-form-actions">
				...
				<button type="submit" class="lwp-btn-primary woocommerce-button button woocommerce-form-login__submit" name="login" value="<?php esc_attr_e( 'Log in', 'luwipress-gold' ); ?>"><?php esc_html_e( 'Log in', 'luwipress-gold' ); ?></button>
				<label class="lwp-remember">
					<input class="woocommerce-form__input woocommerce-form__input-checkbox" name="rememberme" type="checkbox" id="rememberme" value="forever" />
					<?php esc_html_e( 'Remember me', 'luwipress-gold' ); ?>
				</label>
			</div>

			<p class="alt"><a href="<?php echo esc_url( wp_lostpassword_url() ); ?>"><?php esc_html_e( 'Forgot your password?', 'luwipress-gold' ); ?></a></p>
	...
	<?php if ( $registration_enabled ) : ?>
		<section class="lwp-auth-card">
			<h2><?php
				...
				printf( esc_html__( 'New to %s', 'luwipress-gold' ), esc_html( get_bloginfo( 'name' ) ) );
			?></h2>
			<p class="sub"><?php esc_html_e( 'Create an account to track orders, save instruments to your wishlist, and check out faster next time.', 'luwipress-gold' ); ?></p>
			...
			<form method="post" class="woocommerce-form woocommerce-form-register register">
				...
				<?php if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) : ?>
					<label class="lwp-form-label">
						<?php esc_html_e( 'Username', 'luwipress-gold' ); ?>
				...
				<label class="lwp-form-label">
					<?php esc_html_e( 'Email', 'luwipress-gold' ); ?>
				...
				<div class="lwp-form-actions">
					...
					<button type="submit" class="lwp-btn-primary woocommerce-Button woocommerce-button button woocommerce-form-register__submit" name="register" value="<?php esc_attr_e( 'Create account', 'luwipress-gold' ); ?>"><?php esc_html_e( 'Create account', 'luwipress-gold' ); ?></button>
				</div>
	...
	<?php else :
		...
		$brand_blurb = get_theme_mod(
			'luwipress_gold_login_blurb',
			__( 'Hand-crafted instruments shipped from our atelier in Brisighella. Sign in to keep track of orders, save the pieces you\'re watching, and check out faster.', 'luwipress-gold' )
		);
		?>
		<section class="lwp-auth-card lwp-auth-card--brand" aria-label="<?php esc_attr_e( 'About', 'luwipress-gold' ); ?>">
			<span class="lwp-eyebrow"><?php esc_html_e( '— Why sign in', 'luwipress-gold' ); ?></span>
			<h2 class="lwp-auth-card__brand-title"><?php
				/* translators: %s: site name */
				printf( esc_html__( '%s · members.', 'luwipress-gold' ), esc_html( get_bloginfo( 'name' ) ) );
			?></h2>
			<p class="lwp-auth-card__brand-blurb"><?php echo esc_html( $brand_blurb ); ?></p>
			<ul class="lwp-auth-card__perks">
				<li><?php esc_html_e( '✓  Track every order from atelier to door', 'luwipress-gold' ); ?></li>
				<li><?php esc_html_e( '✓  Save instruments to your wishlist for later', 'luwipress-gold' ); ?></li>
				<li><?php esc_html_e( '✓  One-click checkout with stored addresses', 'luwipress-gold' ); ?></li>
				<li><?php esc_html_e( '✓  Early access to new arrivals', 'luwipress-gold' ); ?></li>
			</ul>
			<?php
			// Featured product image — operator pin one product as
			// `_lwp_gold_login_feature` (post meta) or fall back to the
			// most recent in-stock featured product. Empty graceful: card
			// just shows perks without imagery.
			$feat_id = (int) get_option( 'luwipress_gold_login_feature_id', 0 );
					...
					if ( $thumb ) : ?>
						<a href="<?php echo esc_url( get_permalink( $feat_id ) ); ?>" class="lwp-auth-card__feat">
							...
							<span class="lwp-auth-card__feat-meta">
								<small><?php esc_html_e( 'Featured', 'luwipress-gold' ); ?></small>
