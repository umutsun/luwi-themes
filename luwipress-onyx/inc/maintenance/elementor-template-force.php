 ...
 *
 * When the operator sets `luwipress_onyx_pdp_template_id` or
 * `luwipress_onyx_archive_template_id` via the LuwiPress → Theme settings
...
add_filter( 'elementor/theme/get_location_templates/template_id', function ( $template_id, $location ) {
	...
	if ( in_array( $location, array( 'single', 'product' ), true ) || ( is_string( $location ) && false !== strpos( $location, 'single-product' ) ) ) {
		$forced = (int) get_theme_mod( 'luwipress_onyx_pdp_template_id', 0 );
	...
	if ( in_array( $location, array( 'archive', 'product-archive', 'products-archive' ), true ) || ( is_string( $location ) && false !== strpos( $location, 'product-archive' ) ) ) {
		$forced = (int) get_theme_mod( 'luwipress_onyx_archive_template_id', 0 );
		if ( $forced > 0 && get_post( $forced ) ) {
			return $forced;
		}
	}
 ...
 * `luwipress_onyx_wpml_subcat_strict_404` setting is on, render an explicit
...
add_action( 'template_redirect', function () {
	...
	if ( ! get_theme_mod( 'luwipress_onyx_wpml_subcat_strict_404', false ) ) {
		return;
	}
	...
	echo '<p>This subcategory archive has not been translated yet. The luwipress-onyx theme is configured to surface this gap rather than fall through to a generic 404 so it can be fixed in WPML &rarr; Taxonomy Translation.</p>';
 ...
 */
add_action( 'template_redirect', function () {
	if ( ! get_theme_mod( 'luwipress_onyx_block_orphan_landings', false ) ) {
		return;
	}
	if ( ! is_singular( array( 'page', 'post' ) ) ) {
		return;
	}
	$blocked = get_transient( 'luwipress_onyx_orphan_landing_ids' );
	...
	nocache_headers();
	wp_die(
		esc_html__( 'This page has been retired.', 'luwipress-onyx' ),
		'410 Gone',
		array( 'response' => 410 )
	);
} );
