 ...
 * frontend behaviour:
 *
 *   • luwipress_onyx_seo_strict_canonical
 ...
 *
 *   • luwipress_onyx_seo_force_trailing_slash
 ...
 *
 *   • luwipress_onyx_seo_noindex_empty_archives
 ...
 *
 *   • luwipress_onyx_block_orphan_landings   (existing — wired here)
...
add_filter( 'rank_math/frontend/canonical', function ( $canonical ) {
	if ( ! get_theme_mod( 'luwipress_onyx_seo_strict_canonical', false ) ) {
		return $canonical;
	}
	return luwipress_onyx_self_canonical_url() ?: $canonical;
}, 99 );
...
add_action( 'wp_head', function () {
	...
	$url = luwipress_onyx_self_canonical_url();
	if ( $url ) {
		echo "\n<link rel=\"canonical\" href=\"" . esc_url( $url ) . "\" />\n";
	}
}, 1 );

if ( ! function_exists( 'luwipress_onyx_self_canonical_url' ) ) {
	function luwipress_onyx_self_canonical_url() {
		if ( is_singular() ) {
...
add_action( 'template_redirect', function () {
	if ( ! get_theme_mod( 'luwipress_onyx_seo_force_trailing_slash', false ) ) {
		return;
	}
...
add_action( 'wp_head', function () {
	if ( ! get_theme_mod( 'luwipress_onyx_seo_noindex_empty_archives', false ) ) {
		return;
	}
	...
	$flagged = get_transient( 'luwipress_onyx_empty_archive_ids' );
	if ( ! is_array( $flagged ) ) {
...
add_action( 'luwipress_theme_tool_executed', function ( $tool_id, $action, $result ) {
	...
	if ( empty( $result['candidates'] ) ) {
		delete_transient( 'luwipress_onyx_empty_archive_ids' );
		return;
	}
	$ids = array_map( function ( $c ) { return (int) ( $c['id'] ?? 0 ); }, $result['candidates'] );
	$ids = array_values( array_filter( $ids ) );
	set_transient( 'luwipress_onyx_empty_archive_ids', $ids, 6 * HOUR_IN_SECONDS );
}, 10, 3 );

add_action( 'luwipress_theme_tool_executed', function ( $tool_id, $action, $result ) {
	if ( $tool_id !== 'unwanted_landing_pages' || $action !== 'scan' ) { return; }
	if ( empty( $result['candidates'] ) ) {
		delete_transient( 'luwipress_onyx_orphan_landing_ids' );
		return;
	}
	$ids = array_map( function ( $c ) { return (int) ( $c['id'] ?? 0 ); }, $result['candidates'] );
	$ids = array_values( array_filter( $ids ) );
	set_transient( 'luwipress_onyx_orphan_landing_ids', $ids, 6 * HOUR_IN_SECONDS );
}, 10, 3 );
