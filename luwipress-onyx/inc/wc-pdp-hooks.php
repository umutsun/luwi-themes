...
add_action( 'woocommerce_single_product_summary', function () {
	...
	if ( ! $maker ) {
		$m_meta = (string) get_post_meta( $product->get_id(), '_lwp_onyx_maker', true );
		if ( $m_meta ) $maker = $m_meta;
	}
	if ( $maker ) {
		/* translators: %s: master / maker name */
		$parts[] = sprintf( __( 'by %s', 'luwipress-onyx' ), $maker );
	}
...
add_filter( 'woocommerce_get_price_html', function ( $html, $product ) {
	...
	$badge = sprintf(
		...
		'<span class="lwp-pdp-save">' . esc_html__( 'Save %s', 'luwipress-onyx' ) . '</span>',
		wc_price( $saved )
	);
	return $html . ' ' . $badge;
}, 20, 2 );
 ...
 *
 * Filter `luwipress_onyx_suppress_related_when_upsells` (default true)
...
add_action( 'template_redirect', function () {
	...
	$suppress = apply_filters( 'luwipress_onyx_suppress_related_when_upsells', true, $product );
	if ( ! $suppress ) return;
