...
class LuwiPress_Onyx_Widget_Featured_Product extends Widget_Base {

	public function get_name()        { return 'lwp-featured-product'; }
	public function get_title()       { return __( 'Featured Product', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'source', [
			...
			'default' => 'registry',
			'options' => [
				'registry' => __( 'Featured registry (latest flagged product)', 'luwipress-onyx' ),
				'manual'   => __( 'Manual pick — specific product', 'luwipress-onyx' ),
				'custom'   => __( 'Custom (title + image + CTA only)', 'luwipress-onyx' ),
			],
		] );

		$this->add_control( 'product_id', [
			'label'       => __( 'Pick product', 'luwipress-onyx' ),
		...
		$this->start_controls_section( 'section_custom', [
			'label'     => __( 'Custom content', 'luwipress-onyx' ),
			'condition' => [ 'source' => 'custom' ],
		] );

		$this->add_control( 'c_image', [ 'label' => __( 'Image', 'luwipress-onyx' ), 'type' => Controls_Manager::MEDIA, 'default' => [ 'url' => '' ] ] );
		$this->add_control( 'c_title', [ 'label' => __( 'Title', 'luwipress-onyx' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$this->add_control( 'c_excerpt', [ 'label' => __( 'Short description', 'luwipress-onyx' ), 'type' => Controls_Manager::TEXTAREA, 'rows' => 3, 'default' => '' ] );
		$this->add_control( 'c_price', [ 'label' => __( 'Price (display string)', 'luwipress-onyx' ), 'type' => Controls_Manager::TEXT, 'default' => '' ] );
		$this->add_control( 'c_url', [ 'label' => __( 'Product URL', 'luwipress-onyx' ), 'type' => Controls_Manager::URL, 'default' => [ 'url' => '' ] ] );

		$this->end_controls_section();

		$this->start_controls_section( 'section_cta', [ 'label' => __( 'CTA', 'luwipress-onyx' ) ] );
		$this->add_control( 'cta_label', [
			'label'   => __( 'CTA label', 'luwipress-onyx' ),
			'type'    => Controls_Manager::TEXT,
			'default' => __( 'View product →', 'luwipress-onyx' ),
		] );
		$this->end_controls_section();
		...
		$this->add_control( 'image_side', [
			...
			'type'    => Controls_Manager::SELECT,
			'default' => 'left',
			'options' => [ 'left' => __( 'Left', 'luwipress-onyx' ), 'right' => __( 'Right', 'luwipress-onyx' ) ],
		] );
	...
	protected function get_product_options() {
		if ( ! function_exists( 'wc_get_product' ) ) { return [ '' => __( 'WooCommerce inactive', 'luwipress-onyx' ) ]; }
		$opts = [ '' => __( '— pick a product —', 'luwipress-onyx' ) ];
		$q = new WP_Query( [
			'post_type'      => 'product',
			'post_status'    => 'publish',
			'posts_per_page' => 100,
	...
	protected function resolve_product( $s ) {
		...
		if ( $source === 'manual' ) {
			...
			if ( function_exists( 'lwp_onyx_get_featured_ids' ) ) {
				$ids = lwp_onyx_get_featured_ids( [ 'number' => 1 ] );
				$pid = $ids[0] ?? 0;
			}
		}
		if ( ! $pid ) { return null; }
	...
	protected function product_to_data( $pid ) {
		...
		$note  = function_exists( 'get_post_meta' ) ? (string) get_post_meta( $pid, '_luwipress_onyx_featured_note', true ) : '';
