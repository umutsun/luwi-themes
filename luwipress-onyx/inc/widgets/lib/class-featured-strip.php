...
class LuwiPress_Onyx_Widget_Featured_Strip extends Widget_Base {

	public function get_name()        { return 'lwp-featured-strip'; }
	public function get_title()       { return __( 'Featured Strip', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'count', [
			'label'   => __( 'Max items', 'luwipress-onyx' ),
		...
		$this->add_control( 'fallback', [
			'label'       => __( 'Fallback when registry is empty', 'luwipress-onyx' ),
			'description' => __( 'What to show when no products are flipped to featured. Falls back automatically — the operator does not see the strip disappear during onboarding.', 'luwipress-onyx' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'bestseller',
			'options'     => [
				'bestseller' => __( 'Best-selling products', 'luwipress-onyx' ),
				'onsale'     => __( 'On-sale products', 'luwipress-onyx' ),
				'recent'     => __( 'Recently added', 'luwipress-onyx' ),
				'hide'       => __( 'Hide the strip', 'luwipress-onyx' ),
			],
		] );

		$this->add_control( 'show_note', [
			'label'        => __( 'Show operator note chips', 'luwipress-onyx' ),
			'description'  => __( 'Renders the operator note from the meta box ("Boxing Week pick", "New arrival") as a small chip on each card.', 'luwipress-onyx' ),
		...
		$this->start_controls_section( 'section_head', [ 'label' => __( 'Heading (optional)', 'luwipress-onyx' ) ] );
		$this->add_control( 'eyebrow', [ 'label' => __( 'Eyebrow', 'luwipress-onyx' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Hand-picked', 'luwipress-onyx' ) ] );
		$this->add_control( 'heading', [ 'label' => __( 'Heading', 'luwipress-onyx' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Featured products.', 'luwipress-onyx' ) ] );
		$this->end_controls_section();
	}
	...
	protected function get_ids( $s ) {
		...
		if ( function_exists( 'lwp_onyx_get_featured_ids' ) ) {
			$ids = lwp_onyx_get_featured_ids( [ 'number' => $count ] );
		}
		...
		<section class="lwp-fs">
			...
			<div class="lwp-fs__track">
				<?php foreach ( $ids as $pid ) :
					...
					$note = $show_note ? (string) get_post_meta( $pid, '_luwipress_onyx_featured_note', true ) : '';
					?>
