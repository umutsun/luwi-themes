...
class LuwiPress_Onyx_Widget_Post_Content extends Widget_Base {

	private static $rendering = false;

	public function get_name()        { return 'lwp-post-content'; }
	public function get_title()       { return __( 'Post Content', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'Renders the current post body (the_content). No content controls here — edit the post itself.', 'luwipress-onyx' ) . '</em>',
			]
		);
		...
		$this->add_responsive_control( 'paragraph_spacing', [
			'label'      => __( 'Paragraph spacing', 'luwipress-onyx' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px', 'em' ],
		...
		$this->start_controls_section( 'section_headings_style', [
			'label' => __( 'Headings', 'luwipress-onyx' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );
	...
	protected function render() {
		...
		$post = get_post();
		if ( ! $post ) {
			echo '<div class="lwp-post-content"><p>' . esc_html__( '[Post content goes here.]', 'luwipress-onyx' ) . '</p></div>';
			return;
		}
