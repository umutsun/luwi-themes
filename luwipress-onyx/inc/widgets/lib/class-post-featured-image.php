...
class LuwiPress_Onyx_Widget_Post_Featured_Image extends Widget_Base {

	public function get_name()        { return 'lwp-post-featured-image'; }
	public function get_title()       { return __( 'Post Featured Image', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Image', 'luwipress-onyx' ) ] );
		...
		$this->add_control(
			'link_to',
			[
				'label'   => __( 'Link', 'luwipress-onyx' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'none',
				'options' => [
					'none'  => __( 'None', 'luwipress-onyx' ),
					'post'  => __( 'Post URL', 'luwipress-onyx' ),
					'file'  => __( 'Image file', 'luwipress-onyx' ),
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					'none'       => __( 'None', 'luwipress-onyx' ),
					'attachment' => __( 'Attachment caption', 'luwipress-onyx' ),
				],
			]
		);
		...
		$this->add_responsive_control( 'max_height', [
			'label'      => __( 'Max height', 'luwipress-onyx' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px', 'vh' ],
			'range'      => [
				'px' => [ 'min' => 100, 'max' => 1200 ],
		...
		$this->add_control( 'object_fit', [
			'label'   => __( 'Object fit', 'luwipress-onyx' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'cover',
			'options' => [
				'cover'   => __( 'Cover', 'luwipress-onyx' ),
				'contain' => __( 'Contain', 'luwipress-onyx' ),
				'fill'    => __( 'Fill', 'luwipress-onyx' ),
				'none'    => __( 'None', 'luwipress-onyx' ),
			],
		...
		$this->add_responsive_control( 'align', [
			...
			'options' => [
				'left'   => [ 'title' => __( 'Left', 'luwipress-onyx' ),   'icon' => 'eicon-text-align-left' ],
				'center' => [ 'title' => __( 'Center', 'luwipress-onyx' ), 'icon' => 'eicon-text-align-center' ],
				'right'  => [ 'title' => __( 'Right', 'luwipress-onyx' ),  'icon' => 'eicon-text-align-right' ],
			],
			'default'   => 'center',
	...
	protected function render() {
		...
		$post = get_post();
		if ( ! $post ) {
			echo '<div class="lwp-post-featured-image lwp-post-featured-image--placeholder"><div class="lwp-fpi-ph">' . esc_html__( '[Featured image]', 'luwipress-onyx' ) . '</div></div>';
			return;
		}

		$thumb_id = (int) get_post_thumbnail_id( $post );
		if ( ! $thumb_id ) {
			echo '<div class="lwp-post-featured-image lwp-post-featured-image--placeholder"><div class="lwp-fpi-ph">' . esc_html__( '[No featured image]', 'luwipress-onyx' ) . '</div></div>';
			return;
		}
