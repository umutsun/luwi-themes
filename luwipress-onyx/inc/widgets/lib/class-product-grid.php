...
class LuwiPress_Onyx_Widget_Product_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-product-grid'; }
	public function get_title()       { return __( 'Product Grid', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		/* ────────────── Content / Query ────────────── */
		$this->start_controls_section( 'section_query', [ 'label' => __( 'Query', 'luwipress-onyx' ) ] );

		$this->add_control( 'heading', [
			'label'       => __( 'Heading (optional)', 'luwipress-onyx' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => __( 'e.g. Most popular', 'luwipress-onyx' ),
		] );
		...
		$this->add_control( 'source', [
			...
			'default' => 'query',
			'options' => [
				'query'    => __( 'Auto query (by orderby)', 'luwipress-onyx' ),
				'featured' => __( 'Featured products only', 'luwipress-onyx' ),
				'manual'   => __( 'Manual ID list', 'luwipress-onyx' ),
			],
		] );

		$this->add_control( 'manual_ids', [
			'label'       => __( 'Product IDs (comma or space separated)', 'luwipress-onyx' ),
			'type'        => Controls_Manager::TEXTAREA,
			'default'     => '',
			'placeholder' => '123, 456, 789',
			'condition'   => [ 'source' => 'manual' ],
			'description' => __( 'Order in this list is preserved. Limit + Orderby ignored.', 'luwipress-onyx' ),
		] );

		$this->add_control( 'limit', [
			'label'     => __( 'Number of products', 'luwipress-onyx' ),
		...
		$this->add_control( 'orderby', [
			...
			'default'   => 'popularity',
			'options'   => [
				'popularity' => __( 'Popularity (sales count)', 'luwipress-onyx' ),
				'rating'     => __( 'Average rating', 'luwipress-onyx' ),
				'date'       => __( 'Newest first', 'luwipress-onyx' ),
				'price'      => __( 'Price (low → high)', 'luwipress-onyx' ),
				'price-desc' => __( 'Price (high → low)', 'luwipress-onyx' ),
				'title'      => __( 'Title A → Z', 'luwipress-onyx' ),
				'menu_order' => __( 'Menu order', 'luwipress-onyx' ),
		...
		$this->add_control( 'category', [
			'label'       => __( 'Limit to category (slug, optional)', 'luwipress-onyx' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'darbuka',
			'condition'   => [ 'source!' => 'manual' ],
			'description' => __( 'Single product_cat slug. Leave empty for all categories.', 'luwipress-onyx' ),
		] );

		$this->add_control( 'exclude_out_of_stock', [
			'label'        => __( 'Hide out-of-stock', 'luwipress-onyx' ),
		...
		$this->add_responsive_control( 'gap', [
			'label'      => __( 'Grid gap', 'luwipress-onyx' ),
		...
		$this->add_control( 'extra_class', [
			'label'       => __( 'Extra wrapper class', 'luwipress-onyx' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'tap-products',
			'description' => __( 'Preserves legacy CSS hooks when migrating from a [products] shortcode (e.g. "tap-products"). Optional.', 'luwipress-onyx' ),
		] );
		...
		$this->add_control( 'heading_color', [
			'label'     => __( 'Heading color', 'luwipress-onyx' ),
	...
	protected function render() {
		...
		if ( empty( $products ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div class="lwp-product-grid lwp-product-grid--empty"><em>' . esc_html__( '[Product Grid] No products match the current query.', 'luwipress-onyx' ) . '</em></div>';
			}
			return;
		}
		...
		<li class="product lwp-pcard">
			<a href="<?php echo esc_url( $link ); ?>" class="lwp-pcard-link">
				<div class="lwp-pcard-media">
					...
					<?php if ( $onsale ) : ?>
						<span class="lwp-pcard-badge"><?php esc_html_e( 'Sale', 'luwipress-onyx' ); ?></span>
					<?php endif; ?>
				</div>
