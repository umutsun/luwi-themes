...
class LuwiPress_Onyx_Widget_Story_Split extends Widget_Base {

	public function get_name()        { return 'lwp-story-split'; }
	public function get_title()       { return __( 'Story Split', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'left',
				'options' => [
					'left'  => __( 'Image on left', 'luwipress-onyx' ),
					'right' => __( 'Image on right', 'luwipress-onyx' ),
				],
			]
		);
		...
		/* Image side */
		$this->start_controls_section( 'section_image', [ 'label' => __( 'Image tile', 'luwipress-onyx' ) ] );

		$this->add_control(
			'image',
			[
				'label'   => __( 'Image', 'luwipress-onyx' ),
		...
		$this->add_control(
			'image_alt_grad_from',
			[
				'label'     => __( 'Gradient start (when no image)', 'luwipress-onyx' ),
		...
		$this->add_control(
			'image_alt_grad_to',
			[
				'label'     => __( 'Gradient end (when no image)', 'luwipress-onyx' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#7a5a2c',
			]
		);
		$this->add_control(
			'card_eyebrow',
			[
				'label'   => __( 'Floating card — eyebrow', 'luwipress-onyx' ),
		...
		$this->add_control(
			'card_title',
			[
				'label'   => __( 'Floating card — title', 'luwipress-onyx' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		$this->add_control(
			'card_sub',
			[
				'label'   => __( 'Floating card — sub line', 'luwipress-onyx' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		...
		/* Story side */
		$this->start_controls_section( 'section_story', [ 'label' => __( 'Story', 'luwipress-onyx' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Use Enter for multi-line. Wrap with [em]…[/em] for italic gold accent.', 'luwipress-onyx' ),
		...
		$rep = new Repeater();
		$rep->add_control( 'title', [ 'label' => __( 'Title', 'luwipress-onyx' ), 'type' => Controls_Manager::TEXT ] );
		$rep->add_control( 'body',  [ 'label' => __( 'Body', 'luwipress-onyx' ),  'type' => Controls_Manager::TEXTAREA, 'rows' => 2 ] );

		$this->add_control(
			'bullets',
			[
				'label'       => __( 'Numbered bullets', 'luwipress-onyx' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $rep->get_controls(),
				'title_field' => '{{{ title || "Bullet" }}}',
				'default'     => [
					[ 'title' => __( 'Quality first.',          'luwipress-onyx' ), 'body' => __( 'A short paragraph that explains your first key value.', 'luwipress-onyx' ) ],
					[ 'title' => __( 'Direct from us to you.', 'luwipress-onyx' ), 'body' => __( 'Second value proposition — replace this copy from the widget editor.', 'luwipress-onyx' ) ],
					[ 'title' => __( 'Worldwide shipping.',     'luwipress-onyx' ), 'body' => __( 'Third value — what makes your delivery / fulfillment story strong.', 'luwipress-onyx' ) ],
					[ 'title' => __( 'After-sale support.',     'luwipress-onyx' ), 'body' => __( 'Fourth value — what happens after the customer buys.', 'luwipress-onyx' ) ],
				],
			]
		);
