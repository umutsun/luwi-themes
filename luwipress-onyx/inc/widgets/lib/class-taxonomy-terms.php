...
class LuwiPress_Onyx_Widget_Taxonomy_Terms extends Widget_Base {

	public function get_name()        { return 'lwp-taxonomy-terms'; }
	public function get_title()       { return __( 'Taxonomy Terms', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		$this->add_control( 'taxonomy', [
			'label'   => __( 'Taxonomy', 'luwipress-onyx' ),
		...
		$this->add_control( 'scope', [
			'label'   => __( 'Which terms', 'luwipress-onyx' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'top_level',
			'options' => [
				'top_level'   => __( 'Top-level only (no children)', 'luwipress-onyx' ),
				'all'         => __( 'All terms (flat)', 'luwipress-onyx' ),
				'children_of' => __( 'Children of a specific term', 'luwipress-onyx' ),
			],
		] );

		$this->add_control( 'parent_term', [
			'label'       => __( 'Parent term (ID or slug)', 'luwipress-onyx' ),
		...
		$this->add_control( 'hide_empty', [
			'label'        => __( 'Hide empty terms', 'luwipress-onyx' ),
		...
		$this->add_control( 'orderby', [
			...
			'default' => 'count',
			'options' => [
				'count'      => __( 'Product / post count', 'luwipress-onyx' ),
				'name'       => __( 'Name (A→Z)', 'luwipress-onyx' ),
				'slug'       => __( 'Slug', 'luwipress-onyx' ),
				'term_order' => __( 'Manual term order', 'luwipress-onyx' ),
				'menu_order' => __( 'Menu order (WC)', 'luwipress-onyx' ),
			],
		] );
		...
		$this->add_control( 'limit', [
			'label'   => __( 'Max terms', 'luwipress-onyx' ),
		...
		$this->add_control( 'include_slugs', [
			'label'       => __( 'Only these slugs (optional)', 'luwipress-onyx' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '',
			'placeholder' => 'oud, darbuka, qanun',
			'description' => __( 'Comma-separated. Overrides scope/order — shows exactly these, in this order.', 'luwipress-onyx' ),
		] );

		$this->add_control( 'exclude_slugs', [
			'label'       => __( 'Exclude these slugs (optional)', 'luwipress-onyx' ),
			'type'        => Controls_Manager::TEXT,
		...
		$this->add_control( 'count_label', [
			...
			'type'        => Controls_Manager::TEXT,
			'default'     => __( 'items', 'luwipress-onyx' ),
			'description' => __( 'e.g. "items" → "11 items". Leave empty to hide the count line.', 'luwipress-onyx' ),
		] );

		$this->add_control( 'show_image', [
			'label'        => __( 'Show term image', 'luwipress-onyx' ),
			'description'  => __( 'Uses the term thumbnail (WooCommerce category/brand images). Falls back to a gold gradient.', 'luwipress-onyx' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => 'yes',
			'default'      => 'yes',
		] );
		...
		$this->add_control( 'show_description', [
			'label'        => __( 'Show description sub-line', 'luwipress-onyx' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => 'yes',
			'default'      => '',
		] );
	...
	protected function render() {
		...
		if ( empty( $terms ) ) {
			if ( class_exists( '\\Elementor\\Plugin' ) && \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div class="lwp-cgrid lwp-cgrid--empty"><em>' . esc_html__( '[Taxonomy Terms] No terms match the current settings.', 'luwipress-onyx' ) . '</em></div>';
			}
			return;
		}
