...
class LuwiPress_Onyx_Widget_WC_Add_To_Cart extends Widget_Base {

	public function get_name()        { return 'lwp-wc-add-to-cart'; }
	public function get_title()       { return __( 'WC Add to Cart', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Add to Cart', 'luwipress-onyx' ) ] );

		$this->add_control( 'note', [
			'type' => Controls_Manager::RAW_HTML,
			'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'Form type is auto-detected from the product (simple / variable / grouped / external). Variation JS + AJAX add inherit from WooCommerce core.', 'luwipress-onyx' ) . '</em>',
		] );

		$this->add_control( 'show_quantity_label', [
			'label' => __( 'Show quantity label', 'luwipress-onyx' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes',
		] );

		$this->add_control( 'quantity_label', [
			'label' => __( 'Quantity label', 'luwipress-onyx' ),
			'type'  => Controls_Manager::TEXT, 'default' => __( 'Quantity', 'luwipress-onyx' ),
			'condition' => [ 'show_quantity_label' => 'yes' ],
		] );

		$this->end_controls_section();

		$this->start_controls_section( 'section_button_style', [ 'label' => __( 'Button', 'luwipress-onyx' ), 'tab' => Controls_Manager::TAB_STYLE ] );
		...
		$this->add_control( 'btn_bg_hover', [
			'label' => __( 'Background (hover)', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#9A7B3A',
		...
		$this->add_control( 'btn_color', [
			'label' => __( 'Text color', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#ffffff',
		...
		$this->add_control( 'btn_radius', [
			'label' => __( 'Radius', 'luwipress-onyx' ), 'type' => Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px' ],
		...
		$this->add_responsive_control( 'btn_padding', [
			'label' => __( 'Padding', 'luwipress-onyx' ), 'type' => Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px', 'em' ],
		...
		$this->add_control( 'btn_full_width', [
			'label' => __( 'Full width', 'luwipress-onyx' ),
		...
		$this->start_controls_section( 'section_qty_style', [ 'label' => __( 'Quantity', 'luwipress-onyx' ), 'tab' => Controls_Manager::TAB_STYLE ] );

		$this->add_control( 'qty_label_color', [
			'label' => __( 'Label color', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#8b7f6a',
			'selectors' => [ '{{WRAPPER}} .lwp-wc-atc__qty-label' => 'color: {{VALUE}};' ],
		] );
		$this->add_control( 'qty_border', [
			'label' => __( 'Input border color', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#E8DFC8',
	...
	protected function render() {
		...
		if ( ! $product ) {
			echo '<div class="lwp-wc-atc lwp-wc-atc--placeholder"><button class="button single_add_to_cart_button" disabled>' . esc_html__( 'Add to Cart', 'luwipress-onyx' ) . '</button></div>';
			return;
		}
		...
		$quantity_label   = (string) ( $s['quantity_label'] ?? __( 'Quantity', 'luwipress-onyx' ) );
