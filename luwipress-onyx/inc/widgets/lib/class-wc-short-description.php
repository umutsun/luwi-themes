...
class LuwiPress_Onyx_Widget_WC_Short_Description extends Widget_Base {

	public function get_name()        { return 'lwp-wc-short-desc'; }
	public function get_title()       { return __( 'WC Product Short Description', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Description', 'luwipress-onyx' ) ] );

		$this->add_control( 'note', [
			'type' => Controls_Manager::RAW_HTML,
			'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'Renders the product short description (excerpt). Edit on the product page itself.', 'luwipress-onyx' ) . '</em>',
		] );
		...
		$this->add_control( 'color', [
			'label' => __( 'Color', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#3a342c',
		...
		$this->add_responsive_control( 'max_width', [
			'label' => __( 'Max width', 'luwipress-onyx' ),
	...
	protected function render() {
		...
		if ( ! $product ) {
			echo '<div class="lwp-wc-short-desc"><p>' . esc_html__( '[Product short description goes here.]', 'luwipress-onyx' ) . '</p></div>';
			return;
		}
