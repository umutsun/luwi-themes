...
class LuwiPress_Onyx_Widget_WC_Tabs extends Widget_Base {

	public function get_name()        { return 'lwp-wc-tabs'; }
	public function get_title()       { return __( 'WC Product Tabs', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Tabs', 'luwipress-onyx' ) ] );
		...
		$this->add_control( 'layout', [
			...
			'default' => 'tabs',
			'options' => [
				'tabs'      => __( 'Horizontal tabs', 'luwipress-onyx' ),
				'accordion' => __( 'Accordion', 'luwipress-onyx' ),
			],
		] );

		$this->add_control( 'hide_description', [
			'label' => __( 'Hide Description tab', 'luwipress-onyx' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => '',
		] );
		$this->add_control( 'hide_additional', [
			'label' => __( 'Hide Additional Info tab', 'luwipress-onyx' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => '',
		] );
		$this->add_control( 'hide_reviews', [
			'label' => __( 'Hide Reviews tab', 'luwipress-onyx' ),
			'type'  => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => '',
		] );

		$this->end_controls_section();

		$this->start_controls_section( 'section_tabs_style', [ 'label' => __( 'Tabs', 'luwipress-onyx' ), 'tab' => Controls_Manager::TAB_STYLE ] );

		$this->add_control( 'tab_color', [
			'label' => __( 'Inactive tab color', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#8b7f6a',
			'selectors' => [ '{{WRAPPER}} .lwp-wc-tabs__tab' => 'color: {{VALUE}};' ],
		] );
		$this->add_control( 'tab_active_color', [
			'label' => __( 'Active tab color', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
		...
		$this->start_controls_section( 'section_panel_style', [ 'label' => __( 'Panel', 'luwipress-onyx' ), 'tab' => Controls_Manager::TAB_STYLE ] );

		$this->add_control( 'panel_color', [
			'label' => __( 'Text color', 'luwipress-onyx' ), 'type' => Controls_Manager::COLOR, 'default' => '#3a342c',
	...
	protected function render() {
		...
		if ( ! $product ) {
			echo '<div class="lwp-wc-tabs lwp-wc-tabs--placeholder"><div class="lwp-wc-tabs__ph">' . esc_html__( '[Product tabs]', 'luwipress-onyx' ) . '</div></div>';
			return;
		}
	...
	private function call_tab_callback( $tab, $key ) {
		if ( isset( $tab['callback'] ) && is_callable( $tab['callback'] ) ) {
			...
			echo '<p>' . esc_html__( 'No content.', 'luwipress-onyx' ) . '</p>';
		}
	}
