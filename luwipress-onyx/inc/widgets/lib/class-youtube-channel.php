 ...
 *      under LuwiPress Onyx → Footer → Social, so the user enters it
...
class LuwiPress_Onyx_Widget_YouTube_Channel extends Widget_Base {

	public function get_name()        { return 'lwp-youtube-channel'; }
	public function get_title()       { return __( 'YouTube Channel', 'luwipress-onyx' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'From our channel', 'luwipress-onyx' ),
			]
		);
		$this->add_control(
			'heading',
			[
				...
				'description' => __( 'Use a line break (Enter) for a 2-line title.', 'luwipress-onyx' ),
				'type'        => Controls_Manager::TEXTAREA,
				'rows'        => 2,
				'default'     => __( "Featured videos\nfrom our YouTube channel.", 'luwipress-onyx' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::TEXT,
				'default' => __( 'YouTube channel →', 'luwipress-onyx' ),
			]
		);
		$this->add_control(
			'cta_url',
			[
				...
				'description' => __( 'Leave empty to pull the channel URL from Customize → LuwiPress Onyx → Footer → YouTube URL.', 'luwipress-onyx' ),
		...
		$this->add_control(
			'show_subscribe',
			[
				'label'        => __( 'Show Subscribe button', 'luwipress-onyx' ),
				'description'  => __( 'Only renders if a channel URL is set (here or in Customize → Footer → YouTube URL).', 'luwipress-onyx' ),
		...
		$this->add_control(
			'subscribe_label',
			[
				'label'     => __( 'Subscribe label', 'luwipress-onyx' ),
				'type'      => Controls_Manager::TEXT,
				'default'   => __( 'Subscribe', 'luwipress-onyx' ),
		...
		$this->start_controls_section( 'section_videos', [ 'label' => __( 'Videos', 'luwipress-onyx' ) ] );

		$repeater = new Repeater();
		$repeater->add_control(
			'video',
			[
				'label'       => __( 'YouTube URL or ID', 'luwipress-onyx' ),
				'description' => __( 'Paste the full URL (youtu.be/… or youtube.com/watch?v=…) or just the 11-char ID.', 'luwipress-onyx' ),
				'type'        => Controls_Manager::TEXT,
		...
		$repeater->add_control(
			'byline',
			[
				'label'       => __( 'Byline', 'luwipress-onyx' ),
				'description' => __( 'e.g. "Channel name · 14k views"', 'luwipress-onyx' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'duration',
			[
				'label'       => __( 'Duration badge', 'luwipress-onyx' ),
				'description' => __( 'Optional. Shown as a small badge on the thumbnail (e.g. "3:42"). Leave empty to hide.', 'luwipress-onyx' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		$repeater->add_control(
			'thumb_quality',
			[
				'label'   => __( 'Thumbnail quality', 'luwipress-onyx' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'hq',
				'options' => [
					'hq'      => __( 'HQ (480×360)', 'luwipress-onyx' ),
					'mq'      => __( 'Standard (320×180)', 'luwipress-onyx' ),
					'maxres'  => __( 'Maxres (1280×720, may 404 on old videos)', 'luwipress-onyx' ),
				],
			]
		);
		...
		$this->add_control(
			'videos',
			[
				'label'       => __( 'Videos', 'luwipress-onyx' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $repeater->get_controls(),
				'title_field' => '{{{ title || video || "Video" }}}',
				'default'     => [
					[ 'video' => '', 'title' => __( 'Featured video 1', 'luwipress-onyx' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
					[ 'video' => '', 'title' => __( 'Featured video 2', 'luwipress-onyx' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
					[ 'video' => '', 'title' => __( 'Featured video 3', 'luwipress-onyx' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
					[ 'video' => '', 'title' => __( 'Featured video 4', 'luwipress-onyx' ), 'byline' => '', 'duration' => '', 'thumb_quality' => 'hq' ],
				],
			]
		);
		...
		$this->add_control(
			'card_bg',
			[
				'label'     => __( 'Card background', 'luwipress-onyx' ),
		...
		$this->add_control(
			'card_border',
			[
				'label'     => __( 'Card border', 'luwipress-onyx' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
	...
	protected function resolve_channel_url( $widget_url ) {
		...
		$mod = get_theme_mod( 'luwipress_onyx_social_youtube', '' );
	...
	protected function render() {
		...
		$subscribe_label = trim( (string) ( $s['subscribe_label'] ?? '' ) ) ?: __( 'Subscribe', 'luwipress-onyx' );
		...
		<section class="lwp-ytc" data-columns="<?php echo esc_attr( $columns ); ?>">
					...
					<div class="lwp-ytc-head__actions">
						<?php if ( $show_subscribe ) : ?>
							<a class="lwp-ytc-sub"
								...
								aria-label="<?php echo esc_attr( sprintf( __( '%s on YouTube', 'luwipress-onyx' ), $subscribe_label ) ); ?>">
			...
			<?php if ( empty( $resolved ) ) : ?>
				<div class="lwp-ytc-placeholder" role="note">
					<strong><?php esc_html_e( 'No YouTube videos configured yet.', 'luwipress-onyx' ); ?></strong>
					<span><?php esc_html_e( 'Open this widget and paste a YouTube URL or video ID into each Videos row. This notice is only visible to logged-in editors.', 'luwipress-onyx' ); ?></span>
				</div>
			<?php else : ?>
			<div class="lwp-ytc-grid">
				<?php foreach ( $resolved as $item ) :
					...
					$aria     = $title !== '' ? sprintf( __( 'Play video — %s', 'luwipress-onyx' ), $title ) : __( 'Play video', 'luwipress-onyx' );
					?>
