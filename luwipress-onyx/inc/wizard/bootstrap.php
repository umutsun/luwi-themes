 ...
 * `Appearance → LuwiPress Onyx` admin page, and shows a one-time activation
...
if ( ! class_exists( 'LuwiPress_Onyx_Wizard' ) ) :

class LuwiPress_Onyx_Wizard {

	const DISMISSED_OPTION = 'luwipress_onyx_wizard_dismissed';
	const COMPLETED_OPTION = 'luwipress_onyx_wizard_completed';
	const NONCE            = 'luwipress_onyx_wizard';
	const SLUG             = 'luwipress-onyx-wizard';
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_luwipress_onyx_wizard_step', [ $this, 'handle_ajax_step' ] );
		add_action( 'wp_ajax_luwipress_onyx_wizard_dismiss', [ $this, 'handle_ajax_dismiss' ] );
	}
	...
	public function register_admin_page() {
		add_submenu_page(
			'',                                    // hidden — no parent
			__( 'LuwiPress Onyx setup', 'luwipress-onyx' ),
		...
		add_management_page(
			__( 'LuwiPress Onyx setup (re-run)', 'luwipress-onyx' ),
			__( 'LuwiPress Onyx', 'luwipress-onyx' ),
			self::CAPABILITY,
			self::SLUG,
			[ $this, 'render_page' ]
	...
	public function render_activation_notice() {
		...
		?>
		<div class="notice notice-info luwipress-onyx-notice is-dismissible" data-lwp-wizard-notice>
			<p style="font-size:14px">
				<strong style="font-family:'Playfair Display',Georgia,serif;font-size:18px;">
					<?php esc_html_e( 'Welcome to LuwiPress Onyx.', 'luwipress-onyx' ); ?>
				</strong>
				<br>
				<?php echo esc_html( $summary ); ?>
				<?php esc_html_e( 'Run the setup wizard — we\'ll detect what you have and adapt the theme to it (no data is overwritten).', 'luwipress-onyx' ); ?>
			</p>
			<p>
				<a href="<?php echo $url; ?>" class="button button-primary">
					<?php esc_html_e( 'Start setup wizard', 'luwipress-onyx' ); ?>
				</a>
				<a href="#" class="button button-secondary" data-lwp-wizard-dismiss>
					<?php esc_html_e( 'Skip for now', 'luwipress-onyx' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	public function enqueue_assets( $hook ) {
		...
		wp_enqueue_style(
			'luwipress-onyx-wizard-notice',
			LUWIPRESS_ONYX_URI . '/assets/wizard-css/notice.css',
			[],
			LUWIPRESS_ONYX_VERSION
		);
		wp_enqueue_script(
			'luwipress-onyx-wizard-notice',
			LUWIPRESS_ONYX_URI . '/assets/wizard-js/notice.js',
			[ 'jquery' ],
			LUWIPRESS_ONYX_VERSION,
			true
		);
		wp_localize_script( 'luwipress-onyx-wizard-notice', 'LWP_ONYX_WIZARD', [
		...
		wp_enqueue_style(
			'luwipress-onyx-wizard',
			LUWIPRESS_ONYX_URI . '/assets/wizard-css/wizard.css',
			[ 'luwipress-onyx-wizard-notice' ],
			LUWIPRESS_ONYX_VERSION
		);
		wp_enqueue_script(
			'luwipress-onyx-wizard',
			LUWIPRESS_ONYX_URI . '/assets/wizard-js/wizard.js',
			[ 'jquery', 'luwipress-onyx-wizard-notice' ],
			LUWIPRESS_ONYX_VERSION,
			true
		);
		wp_localize_script( 'luwipress-onyx-wizard', 'LWP_ONYX_WIZARD_DATA', [
			'restUrl' => esc_url_raw( rest_url() ),
			'i18n'    => [
				'detecting'  => __( 'Scanning your site…', 'luwipress-onyx' ),
				'mapping'    => __( 'Mapping content to templates…', 'luwipress-onyx' ),
				'importing'  => __( 'Importing templates…', 'luwipress-onyx' ),
				'finishing'  => __( 'Almost there…', 'luwipress-onyx' ),
				'completed'  => __( 'All done!', 'luwipress-onyx' ),
				'errored'    => __( 'Something went wrong. Check the WP debug log.', 'luwipress-onyx' ),
			],
		] );
	}
	...
	public function handle_ajax_step() {
		...
		switch ( $step ) {
			...
			case 'map':
				...
				$d = new LuwiPress_Onyx_Detector();
				$snap = $d->snapshot();
				$mapper = new LuwiPress_Onyx_Mapper();
			...
			case 'ai_status':
				wp_send_json_success( [
					'available' => LuwiPress_Onyx_AI_Content::is_luwipress_available(),
					'enabled'   => LuwiPress_Onyx_AI_Content::is_enabled(),
					'slots'     => LuwiPress_Onyx_AI_Content::slots(),
					'meta'      => get_option( LuwiPress_Onyx_AI_Content::META_OPTION, [] ),
				] );

			case 'ai_toggle':
				$enable = ! empty( $payload['enable'] );
				update_option( LuwiPress_Onyx_AI_Content::ENABLED_OPTION, $enable ? 1 : 0 );
				if ( ! $enable ) {
					LuwiPress_Onyx_AI_Content::flush_cache();
				}
				wp_send_json_success( [ 'enabled' => $enable ] );

			case 'ai_generate':
				if ( ! LuwiPress_Onyx_AI_Content::is_luwipress_available() ) {
					wp_send_json_error( [ 'message' => 'LuwiPress AI engine not installed.' ] );
				}
				update_option( LuwiPress_Onyx_AI_Content::ENABLED_OPTION, 1 );
				$results = [];
				foreach ( LuwiPress_Onyx_AI_Content::slots() as $slot => $cfg ) {
					$text = LuwiPress_Onyx_AI_Content::resolve( $slot );
					$results[ $slot ] = [
						'label' => $cfg['label'],
						'text'  => $text,
			...
			case 'apply':
				...
				$importer = new LuwiPress_Onyx_Importer();
...
LuwiPress_Onyx_Wizard::instance();
