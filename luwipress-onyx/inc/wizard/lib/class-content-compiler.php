...
class LuwiPress_Onyx_Content_Compiler {

	/**
	 * @var array Snapshot from LuwiPress_Onyx_Detector::snapshot().
	 */
	private $snap;
	...
	/**
	 * AI slot resolver — bridges into LuwiPress_Onyx_AI_Content.
	...
	private function resolve_ai( $matches ) {
		...
		if ( ! class_exists( 'LuwiPress_Onyx_AI_Content' ) ) {
			$ai_path = LUWIPRESS_ONYX_DIR . '/inc/wizard/lib/class-ai-content.php';
		...
		$text = LuwiPress_Onyx_AI_Content::resolve( $slot, $context );
	...
	private function loop_categories( $limit ) {
		...
		foreach ( array_slice( $src, 0, $limit ) as $c ) {
			...
			$rows[] = [
				...
				'count_label'=> sprintf(
					/* translators: %d: items */
					_n( '%d item', '%d items', $c['count'], 'luwipress-onyx' ),
					$c['count']
				),
	...
	private function loop_posts( $limit ) {
		...
		while ( $query->have_posts() ) {
			...
			$cat  = $cats ? $cats[0]->name : __( 'Journal', 'luwipress-onyx' );
	...
	private function scalars() {
		...
		$master_phrase = $master_count > 0
			? sprintf(
				...
				_n( '%d master', '%d masters', $master_count, 'luwipress-onyx' ),
				$master_count
			)
			: '';

		// Country count — read from a theme_mod or fall back to a sane default.
		$countries = (int) get_theme_mod( 'luwipress_onyx_countries_shipped', 0 );
		if ( $countries === 0 ) {
		...
		$this->scalars = [
			...
			'primary_color'       => get_theme_mod( 'luwipress_onyx_color_primary', '#735c00' ),
			'primary_light'       => get_theme_mod( 'luwipress_onyx_color_primary_light', '#D4AF37' ),
			'sale_color'          => get_theme_mod( 'luwipress_onyx_color_sale', '#a33b3e' ),
