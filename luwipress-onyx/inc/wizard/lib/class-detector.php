...
class LuwiPress_Onyx_Detector {
	...
	private function detect_theme_state() {
		...
		return [
			...
			'is_luwipress_onyx' => stripos( $current->get( 'Name' ), 'luwipress gold' ) !== false,
		];
	}
	...
	public function summary_phrase( $snap ) {
		...
		if ( ! empty( $snap['wc']['active'] ) ) {
			...
			$bits[] = __( 'no WooCommerce yet', 'luwipress-onyx' );
		}

		if ( ! empty( $snap['content']['posts'] ) ) {
			$bits[] = sprintf(
				/* translators: %d posts */
				_n( '%d post', '%d posts', $snap['content']['posts'], 'luwipress-onyx' ),
				$snap['content']['posts']
			);
		}
		...
		if ( ! empty( $snap['i18n']['is_multi'] ) ) {
			$bits[] = sprintf(
				...
				_n( '%d active language', '%d active languages', count( $snap['i18n']['active'] ), 'luwipress-onyx' ),
		...
		if ( ! empty( $snap['masters'] ) ) {
			$bits[] = sprintf(
				...
				_n( '%d master luthier', '%d master luthiers', count( $snap['masters'] ), 'luwipress-onyx' ),
				count( $snap['masters'] )
			);
		}

		if ( empty( $bits ) ) {
			return __( 'Fresh install — perfect for the demo content path.', 'luwipress-onyx' );
		}

		return sprintf(
			/* translators: comma-joined list of detected things */
			__( 'We detected: %s.', 'luwipress-onyx' ),
			implode( ', ', $bits )
		);
	}
}
