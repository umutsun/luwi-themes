...
class LuwiPress_Onyx_Mapper {
	...
	private function plan_use_existing( &$plan ) {
		...
		$plan['actions'][] = [
			...
			'name' => 'LuwiPress Onyx — Animations',
			'type' => 'page',
		];
		...
		if ( ! empty( $snap['wc']['active'] ) ) {
			$plan['actions'][] = [
				...
				'name'   => 'LuwiPress Onyx — Single Product',
				'type'   => 'single-product',
			...
			if ( ! empty( $snap['top_sellers'] ) ) {
				$plan['options']['luwipress_onyx_featured_products'] = wp_list_pluck( $snap['top_sellers'], 'id' );
			}

			// Pre-fill megabar from top sub-categories.
			if ( ! empty( $snap['top_terms'] ) ) {
				$plan['options']['luwipress_onyx_megabar_terms'] = wp_list_pluck( $snap['top_terms'], 'slug' );
			}
		}
		...
		if ( $has_home ) {
			$plan['pages']['home'] = [
				...
				'title'       => $snap['content']['front_page']['home_title'] ?: __( 'Home', 'luwipress-onyx' ),
			...
			if ( $existing ) {
				...
				$plan['warnings'][] = [
					...
					'message' => sprintf(
						...
						__( 'Existing "%1$s" left untouched. A parallel "%2$s" page was created so you can compare layouts side by side.', 'luwipress-onyx' ),
						$existing->post_title,
						$slug . '-gold'
					),
				];
		...
		if ( ! empty( $snap['masters'] ) ) {
			...
			$plan['actions'][] = [
				...
				'name'   => 'LuwiPress Onyx — Master Profile',
				'type'   => 'page',
				'note'   => sprintf(
					/* translators: %d: master count */
					_n( 'Used as the base layout for %d luthier page', 'Used as the base layout for %d luthier pages', count( $snap['masters'] ), 'luwipress-onyx' ),
					count( $snap['masters'] )
				),
			];
		...
		if ( ! empty( $snap['i18n']['is_multi'] ) ) {
			$plan['warnings'][] = [
				...
				'message' => sprintf(
					...
					__( '%s detected. New pages will be created in the default language only — duplicate them via your translation manager when ready.', 'luwipress-onyx' ),
		...
		if ( ! empty( $snap['theme_state']['switching_from_hello'] ) ) {
			$plan['warnings'][] = [
				...
				'message' => __( 'Switching from Hello Elementor — your menus and Elementor pages will be preserved. Header / footer locations will be re-assigned to the LuwiPress Onyx templates.', 'luwipress-onyx' ),
			];
	...
	private function plan_tapadum_demo( &$plan ) {
		...
		$plan['warnings'][] = [
			...
			'message' => __( 'Demo content will be tagged with _lwp_demo_data so you can remove it later via Tools → LuwiPress Onyx → Clean demo data.', 'luwipress-onyx' ),
		];
	}
	...
	private function plan_empty( &$plan ) {
		...
		$plan['warnings'][] = [
			'kind'    => 'empty_path',
			'message' => __( 'No pages or products will be imported. You can re-run this wizard later or manually import templates from Elementor → Tools → Kit Library.', 'luwipress-onyx' ),
		];
	}
	...
	private function static_page_definitions() {
		return [
			'about'    => [ 'title' => __( 'About', 'luwipress-onyx' ),    'kit' => '06-about.json' ],
			'masters'  => [ 'title' => __( 'Masters', 'luwipress-onyx' ),  'kit' => '07-master-profile.json' ],
			'journal'  => [ 'title' => __( 'Journal', 'luwipress-onyx' ),  'kit' => '08-journal.json' ],
			'contact'  => [ 'title' => __( 'Contact', 'luwipress-onyx' ),  'kit' => '09-contact.json' ],
		];
	}
