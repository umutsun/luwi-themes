...
<main class="lwp-page lwp-shop-archive" id="primary">
	<div class="lwp-page-container">
		...
		<nav class="lwp-crumb" aria-label="<?php esc_attr_e( 'Breadcrumb', 'luwipress-onyx' ); ?>">
			...
			<?php if ( function_exists( 'wc_get_page_id' ) ) :
				...
				if ( $shop_id > 0 && ! is_shop() ) : ?>
					<span class="sep">›</span>
					<a href="<?php echo esc_url( get_permalink( $shop_id ) ); ?>"><?php esc_html_e( 'Shop', 'luwipress-onyx' ); ?></a>
				<?php endif;
			endif; ?>
			<?php
		 ...
		 */
		do_action( 'luwipress_onyx_archive_intro_before', $is_term ? $current_obj : null );
		?>
		...
		<header class="lwp-archive-header<?php echo $header_image_html ? ' lwp-archive-header--split' : ''; ?>">
			<div class="lwp-archive-header__copy">
				<?php if ( $is_term ) : ?>
					...
					echo esc_html(
						$current_obj->parent
							? __( 'Sub-category', 'luwipress-onyx' )
							: __( 'Collection', 'luwipress-onyx' )
					);
					?></span>
				<?php endif; ?>
				 ...
				 */
				do_action( 'luwipress_onyx_archive_intro_after', $is_term ? $current_obj : null );
				?>
			</div>
		...
		<?php if ( ! empty( $subcats ) ) : ?>
			 ...
			 */
			do_action( 'luwipress_onyx_archive_subcat_tiles_before', $subcats, $current_obj );
			?>
			<section class="lwp-archive-subcat-grid" aria-label="<?php esc_attr_e( 'Sub-categories', 'luwipress-onyx' ); ?>">
					...
					<a class="lwp-archive-subcat" href="<?php echo esc_url( get_term_link( $sc ) ); ?>">
						...
						<div class="lwp-archive-subcat__meta">
							...
							<span class="lwp-archive-subcat__count">
								...
								printf( esc_html( _n( '%d piece', '%d pieces', (int) $sc->count, 'luwipress-onyx' ) ), (int) $sc->count );
								?>
							</span>
						</div>
					</a>
			...
			<?php
			do_action( 'luwipress_onyx_archive_subcat_tiles_after', $subcats, $current_obj );
			?>
		<?php endif; ?>

		<div class="lwp-shop-grid-wrap">

			<aside class="lwp-shop-sidebar" aria-label="<?php esc_attr_e( 'Filters', 'luwipress-onyx' ); ?>">
				<?php
				...
				} elseif ( function_exists( 'luwipress_onyx_render_smart_filters' ) ) {
					luwipress_onyx_render_smart_filters( $is_term ? $current_obj : null );
				}
				?>
			</aside>
					...
					if ( get_theme_mod( 'luwipress_onyx_shop_loadmore', true ) ) {
						...
						if ( $max_pages > 1 ) {
							...
							printf(
								...
								$current_page,
								$max_pages,
								esc_html__( 'Load more products', 'luwipress-onyx' ),
								esc_attr__( 'Loading', 'luwipress-onyx' )
							);
						}
					}
				} else {
