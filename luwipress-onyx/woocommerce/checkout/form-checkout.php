...
if ( ! $checkout->is_registration_enabled() && $checkout->is_registration_required() && ! is_user_logged_in() ) {
	echo esc_html( apply_filters( 'woocommerce_checkout_must_be_logged_in_message', __( 'You must be logged in to checkout.', 'luwipress-onyx' ) ) );
	return;
}
?>
...
<details class="lwp-order-summary-collapsible" open>
	<summary>
		<span class="lbl"><?php
			...
			printf( esc_html( _n( 'Order summary · %d item', 'Order summary · %d items', $cart_count, 'luwipress-onyx' ) ), $cart_count );
		?></span>
...
<form name="checkout" method="post" class="checkout woocommerce-checkout lwp-co-form" action="<?php echo esc_url( wc_get_checkout_url() ); ?>" enctype="multipart/form-data">
	...
	<div class="lwp-co-grid">
		...
		<div class="lwp-co-main">
			<?php if ( $checkout->get_checkout_fields() ) : ?>
				...
				<section class="lwp-co-step">
					<h3><span class="lwp-co-stnum">1</span> <?php esc_html_e( 'Contact', 'luwipress-onyx' ); ?></h3>
					<p class="lwp-co-stsub">
						<?php esc_html_e( "We'll send the order confirmation here.", 'luwipress-onyx' ); ?>
						<?php if ( ! is_user_logged_in() ) : ?>
							<?php
							/* translators: %s sign-in link */
							printf( esc_html__( 'Already a customer? %s', 'luwipress-onyx' ), '<a href="' . esc_url( wp_login_url( wc_get_checkout_url() ) ) . '" style="color:var(--primary);text-decoration:underline">' . esc_html__( 'Sign in', 'luwipress-onyx' ) . '</a>' );
							?>
						<?php endif; ?>
					</p>
					<?php
				...
				<section class="lwp-co-step">
					<h3><span class="lwp-co-stnum">2</span> <?php esc_html_e( 'Shipping address', 'luwipress-onyx' ); ?></h3>
					<p class="lwp-co-stsub"><?php esc_html_e( 'DHL Express · arrives in 3–7 working days worldwide.', 'luwipress-onyx' ); ?></p>
			...
			<section class="lwp-co-step">
				<h3><span class="lwp-co-stnum">3</span> <?php esc_html_e( 'Payment', 'luwipress-onyx' ); ?></h3>
				<p class="lwp-co-stsub"><?php esc_html_e( 'All transactions are encrypted. We never store your card.', 'luwipress-onyx' ); ?></p>
		...
		<aside class="lwp-co-summary" aria-label="<?php esc_attr_e( 'Order summary', 'luwipress-onyx' ); ?>">
			<h3><?php esc_html_e( 'Your order', 'luwipress-onyx' ); ?></h3>
			<?php
			...
			<div class="lwp-co-totals">
				<div class="line"><span><?php esc_html_e( 'Subtotal', 'luwipress-onyx' ); ?></span><span><?php wc_cart_totals_subtotal_html(); ?></span></div>
				...
				<?php if ( WC()->cart->needs_shipping() && WC()->cart->show_shipping() ) : ?>
					<div class="line"><span><?php esc_html_e( 'Shipping', 'luwipress-onyx' ); ?></span><span><?php wc_cart_totals_shipping_html(); ?></span></div>
				<?php endif; ?>
				...
				<div class="line tot"><span><?php esc_html_e( 'Total', 'luwipress-onyx' ); ?></span><strong><?php wc_cart_totals_order_total_html(); ?></strong></div>
			</div>
		</aside>
...
?>
<div class="lwp-sticky-cart-bar" role="region" aria-label="<?php esc_attr_e( 'Pay total and place order', 'luwipress-onyx' ); ?>">
	<div class="px">
		<span class="lbl"><?php esc_html_e( 'Pay total', 'luwipress-onyx' ); ?></span>
		<span class="v"><?php echo wp_kses_post( WC()->cart ? WC()->cart->get_total() : '' ); ?></span>
	</div>
	<button type="button" class="button" onclick="var b=document.getElementById('place_order');if(b){b.click();}"><?php esc_html_e( 'Place order →', 'luwipress-onyx' ); ?></button>
</div>
