...
$get_addresses         = apply_filters( 'woocommerce_my_account_get_addresses', [
	'billing'  => __( 'Billing', 'luwipress-onyx' ),
	'shipping' => __( 'Shipping', 'luwipress-onyx' ),
], $customer_id );
...
<h2><?php esc_html_e( 'Saved addresses', 'luwipress-onyx' ); ?></h2>
<p class="lwp-acct-sub"><?php esc_html_e( 'These addresses are used by default at checkout. You can update them at any time, or add a new address from the cart.', 'luwipress-onyx' ); ?></p>

<?php if ( ! $ship_to_billing_only ) : ?>
	<p class="lwp-acct-sub" style="margin-top:-16px;font-size:13px;">
		<?php esc_html_e( 'The following addresses will be used on the checkout page by default.', 'luwipress-onyx' ); ?>
	</p>
<?php endif; ?>
		...
		<div class="lwp-addr-card">
			<span class="lwp-addr-card__lbl">
				...
				if ( $is_default ) {
					...
					printf( esc_html__( 'Default · %s', 'luwipress-onyx' ), esc_html( strtolower( $title ) ) );
				} else {
					echo esc_html( $title );
				}
				?>
			...
			<?php else : ?>
				<p class="lwp-addr-card__empty"><?php esc_html_e( 'You have not set up this type of address yet.', 'luwipress-onyx' ); ?></p>
			<?php endif; ?>
			<a href="<?php echo esc_url( wc_get_endpoint_url( 'edit-address', $name ) ); ?>" class="lwp-addr-card__edit">
				<?php echo $address ? esc_html__( 'Edit address →', 'luwipress-onyx' ) : esc_html__( 'Add address →', 'luwipress-onyx' ); ?>
			</a>
		</div>
	<?php endforeach; ?>
</div>
