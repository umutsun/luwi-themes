...
<main class="lwp-people-archive" id="primary">
	...
	<header class="lwp-people-archive__header">
		<p class="lwp-people-archive__eyebrow">
			...
			esc_html_e( '— The hands behind the work', 'luwipress-sapphire' );
			?>
		</p>
	...
	<?php if ( have_posts() ) : ?>
							...
							<?php if ( $years > 0 ) : ?>
								<span class="lwp-people-card__years">
									...
									echo esc_html( sprintf(
										...
										_n( '%d year', '%d years', $years, 'luwipress-sapphire' ),
										$years
									) );
									?>
								</span>
		...
		the_posts_pagination( array(
			'mid_size'  => 1,
			'prev_text' => __( '← Previous', 'luwipress-sapphire' ),
			'next_text' => __( 'Next →', 'luwipress-sapphire' ),
		) );

		// Infinite scroll / load-more over the real paginated links above.
		if ( function_exists( 'lwp_sapphire_loadmore_render' ) ) {
			lwp_sapphire_loadmore_render( array(
	...
	<?php else : ?>
		<p class="lwp-people-archive__empty">
			...
			echo esc_html( sprintf(
				...
				__( 'No %s yet.', 'luwipress-sapphire' ),
				strtolower( $plural )
			) );
			?>
		</p>
