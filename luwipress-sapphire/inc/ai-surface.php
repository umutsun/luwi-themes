...
define( 'LUWIPRESS_SAPPHIRE_REST_NS', 'luwipress-sapphire/v1' );
 ...
 */
function lwp_sapphire_storefront_rate_gate( $bucket ) {
	$ip      = sanitize_text_field( $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0' );
	$key     = 'lwp_sapphire_rl_' . $bucket . '_' . md5( $ip );
	$count   = (int) get_transient( $key );
	$limit   = (int) apply_filters( 'luwipress_sapphire_storefront_rate_limit', 30, $bucket );

	if ( $count >= $limit ) {
		return new WP_Error(
			'rate_limit',
			__( 'Too many requests. Please wait a moment.', 'luwipress-sapphire' ),
			array( 'status' => 429 )
		);
	}
...
add_action( 'rest_api_init', function () {
	register_rest_route( LUWIPRESS_SAPPHIRE_REST_NS, '/search-suggest', array(
		'methods'             => 'POST',
		'callback'            => 'lwp_sapphire_handle_search_suggest',
...
/**
 * POST /wp-json/luwipress-sapphire/v1/search-suggest
 * Body: { q: "darbuka", limit?: 5 }
 * Returns: { products: [{id,title,url,thumb,price_html,eyebrow}], suggestions: [string] }
 */
function lwp_sapphire_handle_search_suggest( WP_REST_Request $request ) {
	$gate = lwp_sapphire_storefront_rate_gate( 'search' );
	if ( is_wp_error( $gate ) ) {
		return $gate;
	}
	...
	$cache_key = 'lwp_sapphire_ss_' . md5( $query . '|' . $limit );
	...
	$products = lwp_sapphire_search_products_fast( $query, $limit );
	...
	$suggestions = array();
	if ( ! empty( $products ) && lwp_sapphire_lp_active() ) {
		$suggestions = lwp_sapphire_ai_search_suggestions( $query, $products );
	}
 ...
 */
function lwp_sapphire_search_products_fast( $query, $limit ) {
	...
	foreach ( $wpq->posts as $post ) {
		...
		$thumb_url = $thumb_id ? wp_get_attachment_image_url( $thumb_id, 'luwipress-sapphire-card' ) : '';
 ...
 * @return string[]
 */
function lwp_sapphire_ai_search_suggestions( $query, array $products ) {
	$titles = array();
	...
	$result = lwp_sapphire_lp_ai_dispatch( 'customer-chat', $messages, array(
		'max_tokens'  => 160,
		'temperature' => 0.4,
		'json_mode'   => true,
	) );
...
add_action( 'wp_footer', function () {
	...
	?>
	<template id="lwp-sapphire-search-suggest-template">
		<div class="lwp-search-suggest" hidden>
			<div class="lwp-search-suggest__products" role="list"></div>
			<div class="lwp-search-suggest__chips" role="list" aria-label="<?php esc_attr_e( 'AI suggestions', 'luwipress-sapphire' ); ?>"></div>
			<div class="lwp-search-suggest__empty" hidden>
				<?php esc_html_e( 'No products match yet — try a broader term.', 'luwipress-sapphire' ); ?>
			</div>
		</div>
	</template>
	<?php
 ...
 *
 * Filter `luwipress_sapphire_show_kg_related_rail` lets a child theme or
...
add_action( 'woocommerce_after_single_product_summary', function () {
	...
	$related = lwp_sapphire_kg_related_products( $product, 4 );
	if ( empty( $related ) ) {
		return;
	}
	...
	if ( ! apply_filters( 'luwipress_sapphire_show_kg_related_rail', $by_master, $product, $related ) ) {
		return;
	}

	/* translators: %s: master / luthier name */
	$rail_eyebrow = sprintf( esc_html__( 'More from %s', 'luwipress-sapphire' ), $related['master_name'] );
	?>
	<section class="lwp-related-rail" aria-labelledby="lwp-related-rail-title">
		<header class="lwp-related-rail__head">
			...
			<h2 id="lwp-related-rail-title" class="lwp-related-rail__title">
				<?php esc_html_e( 'Pieces with the same hand', 'luwipress-sapphire' ); ?>
			</h2>
			<?php if ( ! empty( $related['cat_url'] ) ) : ?>
				<a class="lwp-related-rail__more lwp-ulink" href="<?php echo esc_url( $related['cat_url'] ); ?>">
					<?php esc_html_e( 'Browse the family', 'luwipress-sapphire' ); ?> →
				</a>
			<?php endif; ?>
		</header>
 ...
 */
function lwp_sapphire_kg_related_products( WC_Product $product, $limit = 4 ) {
	$cache_key = 'lwp_sapphire_rel_' . $product->get_id() . '_' . $limit;
	$cached    = get_transient( $cache_key );
	if ( is_array( $cached ) ) {
		return $cached;
	}
	...
	foreach ( $wpq->posts as $post ) {
		...
		$thumb    = $thumb_id ? wp_get_attachment_image_url( $thumb_id, 'luwipress-sapphire-card' ) : '';
...
add_action( 'save_post_product', function ( $post_id ) {
	delete_transient( 'lwp_sapphire_rel_' . (int) $post_id . '_4' );
} );
...
add_action( 'edited_term', function ( $term_id, $tt_id, $taxonomy ) {
	...
	foreach ( (array) $pids as $pid ) {
		delete_transient( 'lwp_sapphire_rel_' . (int) $pid . '_4' );
	}
}, 10, 3 );
...
add_action( 'luwipress_after_product_enrich', function ( $product_id ) {
	delete_transient( 'lwp_sapphire_rel_' . (int) $product_id . '_4' );
	delete_transient( 'lwp_sapphire_ss_' . md5( '' ) ); // touch nothing — placeholder for future SS scoping
}, 10, 1 );
 ...
 * priority 10000 so they land after `luwipress-sapphire-widgets` (9999) and
...
add_action( 'wp_enqueue_scripts', function () {
	...
	$ver = LUWIPRESS_SAPPHIRE_VERSION;

	wp_enqueue_style(
		'luwipress-sapphire-ai-surface',
		LUWIPRESS_SAPPHIRE_URI . '/assets/css/ai-surface.css',
		array( 'luwipress-sapphire-widgets' ),
		$ver
	);

	wp_enqueue_script(
		'luwipress-sapphire-ai-surface',
		LUWIPRESS_SAPPHIRE_URI . '/assets/js/ai-surface.js',
		array( 'luwipress-sapphire-frontend' ),
		$ver,
		true
	);
	...
	wp_localize_script(
		...
		'lwpGoldAi',
		array(
			'searchEndpoint' => esc_url_raw( rest_url( LUWIPRESS_SAPPHIRE_REST_NS . '/search-suggest' ) ),
			'i18n'           => array(
				'noResults' => __( 'No products match yet — try a broader term.', 'luwipress-sapphire' ),
				'searching' => __( 'Searching…', 'luwipress-sapphire' ),
				'tryThese'  => __( 'Try these', 'luwipress-sapphire' ),
			),
		)
	);
}, 10000 );
