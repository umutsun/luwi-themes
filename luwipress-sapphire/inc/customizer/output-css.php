...
add_action( 'wp_enqueue_scripts', function () {
	if ( ! wp_style_is( 'luwipress-sapphire-tokens', 'enqueued' ) ) return;
	wp_add_inline_style( 'luwipress-sapphire-tokens', luwipress_sapphire_inline_token_overrides() );
}, 30 );

// Also inject into the block editor preview.
add_action( 'enqueue_block_editor_assets', function () {
	if ( ! wp_style_is( 'luwipress-sapphire-editor-tokens', 'enqueued' ) ) return;
	wp_add_inline_style( 'luwipress-sapphire-editor-tokens', luwipress_sapphire_inline_token_overrides() );
} );

/**
 * Compose the :root override block.
 */
function luwipress_sapphire_inline_token_overrides() {
	$mods = [
		'primary'       => get_theme_mod( 'luwipress_sapphire_color_primary' ),
		'primary-light' => get_theme_mod( 'luwipress_sapphire_color_primary_light' ),
		'accent'        => get_theme_mod( 'luwipress_sapphire_color_accent' ),
		'sale'          => get_theme_mod( 'luwipress_sapphire_color_sale' ),
		'icon-red'      => get_theme_mod( 'luwipress_sapphire_color_icon_red' ),
		'ink'           => get_theme_mod( 'luwipress_sapphire_color_ink' ),
		'bg'            => get_theme_mod( 'luwipress_sapphire_color_bg' ),
		'black'         => get_theme_mod( 'luwipress_sapphire_color_black' ),
	];
