 ...
 *      LuwiPress Sapphire and react accordingly.
...
add_action( 'admin_notices', function () {
	...
	<div class="notice notice-warning">
		<p>
			<strong><?php esc_html_e( 'LuwiPress Sapphire needs Elementor.', 'luwipress-sapphire' ); ?></strong>
			<?php
			printf(
				/* translators: %s: install plugin link */
				esc_html__( 'Install Elementor (free) to use the LuwiPress Sapphire templates: %s.', 'luwipress-sapphire' ),
				'<a href="' . esc_url( $plugin_url ) . '">' . esc_html__( 'Install Elementor', 'luwipress-sapphire' ) . '</a>'
			);
			?>
		</p>
	</div>
...
add_filter( 'body_class', function ( $classes ) {
	$classes[] = 'luwipress-sapphire';
	if ( did_action( 'elementor/loaded' ) ) {
		$classes[] = 'luwipress-sapphire--elementor';
	}
	return $classes;
} );
...
add_filter( 'template_include', function ( $template ) {
	...
	if ( is_singular() ) {
		...
		if ( $page_tpl === 'elementor_header_footer' || $page_tpl === 'elementor_header_footer.php' ) {
			$ours = LUWIPRESS_SAPPHIRE_DIR . '/elementor_header_footer.php';
			if ( file_exists( $ours ) ) {
				return $ours;
			}
		}
 ...
 * ------------------------------------------------------------------------- */
add_filter( 'luwipress_sapphire_kit_path', function () {
	return LUWIPRESS_SAPPHIRE_DIR . '/elementor-kit/';
} );
 ...
 */
function luwipress_sapphire_kit_files() {
	$dir = apply_filters( 'luwipress_sapphire_kit_path', LUWIPRESS_SAPPHIRE_DIR . '/elementor-kit/' );
