 ...
 *
 * Customizer panel: Appearance → Customize → LuwiPress Sapphire → Footer.
 *
...
if ( ! function_exists( 'luwipress_sapphire_footer_customizer' ) ) {

	function luwipress_sapphire_footer_customizer( $wp_customize ) {

		// Use the existing top-level "LuwiPress Sapphire" panel if present;
		...
		$section_id = 'luwipress_sapphire_footer';
		...
		/* --- Social URLs ------------------------------------------- */
		$socials = luwipress_sapphire_footer_social_platforms();
		foreach ( $socials as $key => $cfg ) {
			$setting_id = 'luwipress_sapphire_social_' . $key;
			...
			$wp_customize->add_control( $setting_id, [
				'label'       => sprintf( /* translators: %s: platform name */ __( '%s URL', 'luwipress-sapphire' ), $cfg['label'] ),
		...
		/* --- Newsletter ------------------------------------------- */
		$wp_customize->add_setting( 'luwipress_sapphire_newsletter_action', [
			'type'              => 'theme_mod',
			'default'           => '',
			'sanitize_callback' => 'esc_url_raw',
			'transport'         => 'refresh',
		] );
		$wp_customize->add_control( 'luwipress_sapphire_newsletter_action', [
			'label'       => __( 'Newsletter form action URL', 'luwipress-sapphire' ),
			'description' => __( 'POST endpoint that receives the email address — Mailchimp action URL, ConvertKit form URL, or any service that accepts a single "EMAIL" or "email" field. Leave empty to hide the newsletter strip.', 'luwipress-sapphire' ),
			'section'     => $section_id,
			'type'        => 'url',
			'priority'    => 30,
		] );

		$wp_customize->add_setting( 'luwipress_sapphire_newsletter_field_name', [
			'type'              => 'theme_mod',
			'default'           => 'EMAIL',
			'sanitize_callback' => 'sanitize_text_field',
			'transport'         => 'refresh',
		] );
		$wp_customize->add_control( 'luwipress_sapphire_newsletter_field_name', [
			'label'       => __( 'Newsletter field name', 'luwipress-sapphire' ),
			'description' => __( 'Form input "name" attribute. Mailchimp uses EMAIL, ConvertKit uses email_address, native WP "Subscribe" plugins typically use email.', 'luwipress-sapphire' ),
			'section'     => $section_id,
			'type'        => 'text',
			'priority'    => 31,
		] );

		$wp_customize->add_setting( 'luwipress_sapphire_newsletter_headline', [
			'type'              => 'theme_mod',
			'default'           => __( 'Letters from the atelier', 'luwipress-sapphire' ),
			'sanitize_callback' => 'sanitize_text_field',
			'transport'         => 'refresh',
		] );
		$wp_customize->add_control( 'luwipress_sapphire_newsletter_headline', [
			'label'    => __( 'Newsletter headline', 'luwipress-sapphire' ),
			'section'  => $section_id,
			'type'     => 'text',
			'priority' => 32,
		] );

		$wp_customize->add_setting( 'luwipress_sapphire_newsletter_blurb', [
			'type'              => 'theme_mod',
			'default'           => __( 'New pieces, master profiles and field notes — once a month, no spam.', 'luwipress-sapphire' ),
			'sanitize_callback' => 'sanitize_text_field',
			'transport'         => 'refresh',
		] );
		$wp_customize->add_control( 'luwipress_sapphire_newsletter_blurb', [
			'label'    => __( 'Newsletter blurb', 'luwipress-sapphire' ),
			'section'  => $section_id,
			'type'     => 'textarea',
			'priority' => 33,
		] );
		...
		for ( $i = 1; $i <= 3; $i++ ) {
			$wp_customize->add_setting( "luwipress_sapphire_trust_signal_{$i}", [
				'type'              => 'theme_mod',
				'default'           => '',
				'sanitize_callback' => 'sanitize_text_field',
				'transport'         => 'refresh',
			] );
			$wp_customize->add_control( "luwipress_sapphire_trust_signal_{$i}", [
				'label'    => sprintf( /* translators: %d: position */ __( 'Trust signal %d', 'luwipress-sapphire' ), $i ),
				'description' => $i === 1 ? __( 'Short value-prop lines (e.g. "Free DHL over €450", "Hand-tuned in our atelier", "30-day returns"). Empty = hide.', 'luwipress-sapphire' ) : '',
				'section'  => $section_id,
				'type'     => 'text',
				'priority' => 40 + $i,
			] );
		...
		/* --- Payment row toggle ----------------------------------- */
		$wp_customize->add_setting( 'luwipress_sapphire_show_payment_row', [
			'type'              => 'theme_mod',
		...
		$wp_customize->add_setting( 'luwipress_sapphire_show_shop_col', [
			'type'              => 'theme_mod',
			'default'           => true,
			'sanitize_callback' => function ( $v ) { return ! empty( $v ); },
			'transport'         => 'refresh',
		] );
		$wp_customize->add_control( 'luwipress_sapphire_show_shop_col', [
			'label'       => __( 'Show "Shop" column', 'luwipress-sapphire' ),
			'description' => __( 'Adds a 5th footer column listing the top WooCommerce product categories by sales count. Hidden when WC is inactive or no categories exist.', 'luwipress-sapphire' ),
			'section'     => $section_id,
			'type'        => 'checkbox',
			'priority'    => 55,
		] );
		$wp_customize->add_setting( 'luwipress_sapphire_shop_col_limit', [
			'type'              => 'theme_mod',
			'default'           => 6,
			'sanitize_callback' => function ( $v ) { return max( 3, min( 12, (int) $v ) ); },
			'transport'         => 'refresh',
		] );
		$wp_customize->add_control( 'luwipress_sapphire_shop_col_limit', [
			'label'       => __( 'Shop column item count', 'luwipress-sapphire' ),
			'description' => __( 'How many top product categories to list. Range 3–12, default 6.', 'luwipress-sapphire' ),
			'section'     => $section_id,
			'type'        => 'number',
		...
		$wp_customize->add_setting( 'luwipress_sapphire_footer_address', [
			'type'              => 'theme_mod',
			'default'           => '',
			'sanitize_callback' => 'sanitize_textarea_field',
			'transport'         => 'refresh',
		] );
		$wp_customize->add_control( 'luwipress_sapphire_footer_address', [
			'label'       => __( 'Contact — address', 'luwipress-sapphire' ),
			'description' => __( 'Postal address under the footer logo (multi-line OK). Leave the whole contact group empty to hide it.', 'luwipress-sapphire' ),
			'section'     => $section_id,
			'type'        => 'textarea',
			'priority'    => 60,
		] );
		$contact_fields = [
			'phone'  => [ __( 'Contact — phone', 'luwipress-sapphire' ),            '+39 0123 456789' ],
			'mobile' => [ __( 'Contact — mobile', 'luwipress-sapphire' ),           '+39 333 1234567' ],
			'email'  => [ __( 'Contact — email', 'luwipress-sapphire' ),            'info@example.com' ],
			'hours'  => [ __( 'Contact — working hours', 'luwipress-sapphire' ),    'Mon–Fri, 10:00–17:00' ],
			'vat'    => [ __( 'Contact — VAT / company no.', 'luwipress-sapphire' ), 'VAT IT 00000000000' ],
		];
		$cp = 61;
		foreach ( $contact_fields as $ck => $ccfg ) {
			$csid = 'luwipress_sapphire_footer_' . $ck;
			...
			$wp_customize->add_control( $csid, [
				...
				'description' => sprintf( __( 'e.g. %s — leave empty to hide this line.', 'luwipress-sapphire' ), $ccfg[1] ),
				'section'     => $section_id,
				'type'        => 'text',
				'priority'    => $cp++,
			] );
	...
	add_action( 'customize_register', 'luwipress_sapphire_footer_customizer', 30 );
}
...
if ( ! function_exists( 'luwipress_sapphire_footer_social_platforms' ) ) {
	 ...
	 *
	 * Filterable via `luwipress_sapphire_footer_social_platforms` so
	 ...
	 */
	function luwipress_sapphire_footer_social_platforms() {
		$defaults = [
			'instagram' => [
				'label'       => 'Instagram',
		...
		return (array) apply_filters( 'luwipress_sapphire_footer_social_platforms', $defaults );
	}
}

if ( ! function_exists( 'luwipress_sapphire_footer_render_social_icons' ) ) {

	/**
	 * Render the configured social icons. Skips platforms with empty
	 * URLs. Returns nothing when no social URL is set on the site.
	 */
	function luwipress_sapphire_footer_render_social_icons() {
		$platforms = luwipress_sapphire_footer_social_platforms();
		$any       = false;
		ob_start();
		echo '<div class="lwp-site-footer-social">';
		foreach ( $platforms as $key => $cfg ) {
			$url = (string) get_theme_mod( 'luwipress_sapphire_social_' . $key, '' );
			if ( $url === '' ) {
				continue;
			}
			$any = true;
...
if ( ! function_exists( 'luwipress_sapphire_footer_render_newsletter' ) ) {

	function luwipress_sapphire_footer_render_newsletter() {
		$action = (string) get_theme_mod( 'luwipress_sapphire_newsletter_action', '' );
		if ( $action === '' ) {
			return;
		}
		$field    = (string) get_theme_mod( 'luwipress_sapphire_newsletter_field_name', 'EMAIL' );
		$headline = (string) get_theme_mod( 'luwipress_sapphire_newsletter_headline',
			__( 'Letters from the atelier', 'luwipress-sapphire' )
		);
		$blurb    = (string) get_theme_mod( 'luwipress_sapphire_newsletter_blurb',
			__( 'New pieces, master profiles and field notes — once a month, no spam.', 'luwipress-sapphire' )
		);
		?>
		<section class="lwp-site-footer-newsletter" aria-label="<?php esc_attr_e( 'Newsletter signup', 'luwipress-sapphire' ); ?>">
			...
			<form class="lwp-site-footer-newsletter__form" action="<?php echo esc_url( $action ); ?>" method="post" target="_blank" novalidate>
				<label class="screen-reader-text" for="lwp-newsletter-email"><?php esc_html_e( 'Your email', 'luwipress-sapphire' ); ?></label>
				<input type="email" required
					id="lwp-newsletter-email"
					name="<?php echo esc_attr( $field ?: 'EMAIL' ); ?>"
					placeholder="<?php esc_attr_e( 'you@example.com', 'luwipress-sapphire' ); ?>"
				>
				<button type="submit" class="lwp-site-footer-newsletter__submit"><?php esc_html_e( 'Subscribe', 'luwipress-sapphire' ); ?></button>
			</form>
		</section>
		<?php
	}
...
if ( ! function_exists( 'luwipress_sapphire_footer_render_trust_strip' ) ) {

	function luwipress_sapphire_footer_render_trust_strip() {
		$signals = [];
		for ( $i = 1; $i <= 3; $i++ ) {
			$v = trim( (string) get_theme_mod( "luwipress_sapphire_trust_signal_{$i}", '' ) );
			if ( $v !== '' ) {
				$signals[] = $v;
			}
		}
		...
		if ( empty( $signals ) && function_exists( 'WC' ) ) {
			$signals = (array) apply_filters( 'luwipress_sapphire_footer_trust_signal_defaults', [
				__( 'Secure checkout', 'luwipress-sapphire' ),
				__( 'Worldwide shipping', 'luwipress-sapphire' ),
				__( '30-day returns', 'luwipress-sapphire' ),
			] );
		}
		if ( empty( $signals ) ) {
			return;
...
if ( ! function_exists( 'luwipress_sapphire_footer_render_shop_categories' ) ) {
	 ...
	 */
	function luwipress_sapphire_footer_render_shop_categories() {
		if ( ! (bool) get_theme_mod( 'luwipress_sapphire_show_shop_col', true ) ) {
			return;
		}
		if ( ! function_exists( 'WC' ) || ! taxonomy_exists( 'product_cat' ) ) {
			return;
		}
		$limit = max( 3, min( 12, (int) get_theme_mod( 'luwipress_sapphire_shop_col_limit', 6 ) ) );
		$terms = get_terms( [
		...
		<div class="lwp-site-footer-col lwp-site-footer-shop">
			<h4><?php esc_html_e( 'Shop', 'luwipress-sapphire' ); ?></h4>
			<ul>
				...
				<?php if ( $shop = wc_get_page_permalink( 'shop' ) ) : ?>
					<li><a href="<?php echo esc_url( $shop ); ?>" class="lwp-site-footer-col-allink"><?php esc_html_e( 'View all collections →', 'luwipress-sapphire' ); ?></a></li>
				<?php endif; ?>
			</ul>
		</div>
		<?php
...
if ( ! function_exists( 'luwipress_sapphire_footer_render_payment_row' ) ) {
	 ...
	 */
	function luwipress_sapphire_footer_render_payment_row() {
		if ( ! (bool) get_theme_mod( 'luwipress_sapphire_show_payment_row', false ) ) {
			return;
		}
			...
			if ( count( $methods ) === 1 ) {
				...
				$entries[] = [
					...
					'tooltip' => sprintf(
						...
						__( 'Includes: %s', 'luwipress-sapphire' ),
		 ...
		 */
		$entries = (array) apply_filters( 'luwipress_sapphire_footer_payment_entries', $entries );
		if ( empty( $entries ) ) {
			return;
		}

		echo '<ul class="lwp-site-footer-payments" aria-label="' . esc_attr__( 'Accepted payment methods', 'luwipress-sapphire' ) . '">';
...
if ( ! function_exists( 'luwipress_sapphire_footer_render_contact' ) ) {

	/**
	 * Render the contact / address block in the footer brand column.
	 * Every field is Customizer-driven (LuwiPress Sapphire → Footer); the whole
	 ...
	 */
	function luwipress_sapphire_footer_render_contact() {
		$address = trim( (string) get_theme_mod( 'luwipress_sapphire_footer_address', '' ) );
		$phone   = trim( (string) get_theme_mod( 'luwipress_sapphire_footer_phone', '' ) );
		$mobile  = trim( (string) get_theme_mod( 'luwipress_sapphire_footer_mobile', '' ) );
		$email   = trim( (string) get_theme_mod( 'luwipress_sapphire_footer_email', '' ) );
		$hours   = trim( (string) get_theme_mod( 'luwipress_sapphire_footer_hours', '' ) );
		$vat     = trim( (string) get_theme_mod( 'luwipress_sapphire_footer_vat', '' ) );
...
if ( ! function_exists( 'luwipress_sapphire_footer_print_styles' ) ) {

	/**
	 * Inline styles for the new footer blocks. Scoped to footer
	 * selectors so it can't leak into other layouts. ~2 KB.
	 */
	function luwipress_sapphire_footer_print_styles() {
		if ( is_admin() ) {
			return;
		}
		?>
<style id="lwp-sapphire-footer-enhancements">
...
.lwp-social-pill:hover{background:var(--lwp-sapphire-accent,#735c00);color:#fff;transform:translateY(-1px)}
...
.lwp-site-footer-newsletter__title{
	margin:0 0 4px;font-family:var(--lwp-sapphire-serif,"Playfair Display",serif);
...
.lwp-site-footer-newsletter__form input:focus{outline:0;border-color:var(--lwp-sapphire-accent,#735c00)}
.lwp-site-footer-newsletter__submit{
	background:var(--lwp-sapphire-accent,#735c00);color:#fff;border:0;
...
.lwp-site-footer-contact .lwp-fc-vat{font-size:11.5px;letter-spacing:.04em;color:rgba(255,255,255,.45);padding-left:28px}
	...
	add_action( 'wp_head', 'luwipress_sapphire_footer_print_styles', 99 );
}
