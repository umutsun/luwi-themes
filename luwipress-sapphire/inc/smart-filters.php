 ...
 *   - Hook `luwipress_sapphire_smart_filters_blocks` to add / remove blocks
 *   - Use the `luwipress_sapphire_smart_filter_quick_toggles` filter to
...
if ( ! function_exists( 'luwipress_sapphire_render_smart_filters' ) ) {
	 ...
	 */
	function luwipress_sapphire_render_smart_filters( $current_term = null ) {
		...
		$blocks = apply_filters( 'luwipress_sapphire_smart_filters_blocks', [
			'active_filters' => true,
			'price'          => true,
			'attributes'     => true,
			'tags'           => true,
		...
		echo '<div class="lwp-smart-filters" role="group" aria-label="' . esc_attr__( 'Filter results', 'luwipress-sapphire' ) . '">';

		if ( ! empty( $blocks['active_filters'] ) ) {
			luwipress_sapphire_render_active_filters_chip( $widget_args );
		}
		if ( ! empty( $blocks['price'] ) ) {
			luwipress_sapphire_render_price_filter( $widget_args );
		}
		if ( ! empty( $blocks['attributes'] ) ) {
			luwipress_sapphire_render_attribute_filters( $widget_args, $current_term );
		}
		if ( ! empty( $blocks['tags'] ) ) {
			luwipress_sapphire_render_archive_tag_cloud( $widget_args, $current_term );
		}
		if ( ! empty( $blocks['quick_toggles'] ) ) {
			luwipress_sapphire_render_quick_toggles( $widget_args );
		}
...
if ( ! function_exists( 'luwipress_sapphire_render_active_filters_chip' ) ) {

	function luwipress_sapphire_render_active_filters_chip( $widget_args ) {
		// Native WC widget — only renders when at least one filter is active.
		if ( ! class_exists( 'WC_Widget_Layered_Nav_Filters' ) ) {
			return;
		}
		the_widget( 'WC_Widget_Layered_Nav_Filters', [
			'title' => __( 'Active filters', 'luwipress-sapphire' ),
		], $widget_args );
	}
}

if ( ! function_exists( 'luwipress_sapphire_render_price_filter' ) ) {

	function luwipress_sapphire_render_price_filter( $widget_args ) {
		if ( ! class_exists( 'WC_Widget_Price_Filter' ) ) {
			return;
		}
		the_widget( 'WC_Widget_Price_Filter', [
			'title' => __( 'Price', 'luwipress-sapphire' ),
		], $widget_args );
	}
}

if ( ! function_exists( 'luwipress_sapphire_render_attribute_filters' ) ) {
	 ...
	 */
	function luwipress_sapphire_render_attribute_filters( $widget_args, $current_term = null ) {
...
if ( ! function_exists( 'luwipress_sapphire_render_archive_tag_cloud' ) ) {
	 ...
	 * @param WP_Term|null $current_term
	 */
	function luwipress_sapphire_render_archive_tag_cloud( $widget_args, $current_term = null ) {
		...
		echo esc_html__( 'Popular tags', 'luwipress-sapphire' );
...
if ( ! function_exists( 'luwipress_sapphire_render_quick_toggles' ) ) {
	 ...
	 * @param array $widget_args
	 */
	function luwipress_sapphire_render_quick_toggles( $widget_args ) {
		// Operators may extend / replace the toggles via filter.
		$toggles = apply_filters( 'luwipress_sapphire_smart_filter_quick_toggles', [
			[
				'param' => 'onsale',
				'label' => __( 'On sale only', 'luwipress-sapphire' ),
			],
			[
				'param' => 'instock',
				'label' => __( 'In stock only', 'luwipress-sapphire' ),
			],
		] );
		if ( empty( $toggles ) ) {
			return;
		...
		echo $widget_args['before_widget']; // phpcs:ignore
		echo $widget_args['before_title']; // phpcs:ignore
		echo esc_html__( 'Quick filters', 'luwipress-sapphire' );
		echo $widget_args['after_title']; // phpcs:ignore
...
if ( ! function_exists( 'luwipress_sapphire_smart_filters_apply_query' ) ) {
	 ...
	 * @param WP_Query $q
	 */
	function luwipress_sapphire_smart_filters_apply_query( $q ) {
	...
	add_action( 'woocommerce_product_query', 'luwipress_sapphire_smart_filters_apply_query' );
}

if ( ! function_exists( 'luwipress_sapphire_smart_filters_print_styles' ) ) {

	/**
	 * Sidebar styling — inline-printed only on product archives so it
	 * doesn't pollute other pages. ~1.5 KB.
	 */
	function luwipress_sapphire_smart_filters_print_styles() {
		if ( is_admin() ) {
			return;
		}
		...
		?>
<style id="lwp-sapphire-smart-filters">
...
.lwp-smart-filters ul li a:hover{border-bottom-color:var(--accent,#735c00);color:var(--accent,#735c00);}
...
.lwp-smart-filters .price_slider_amount .button{
	background:var(--accent,#735c00);color:#fff;border:0;padding:6px 14px;border-radius:999px;
...
.lwp-tag-cloud li a:hover{background:var(--accent,#735c00);color:#fff;}
...
.lwp-quick-toggle.is-active{
	background:var(--accent,#735c00);color:#fff;border-color:var(--accent,#735c00);
}
.lwp-quick-toggle__check{font-weight:700;}
.lwp-quick-toggle:not(.is-active):hover{border-color:var(--accent,#735c00);color:var(--accent,#735c00);}
.lwp-smart-filters .woocommerce-widget-layered-nav-dropdown__submit{
	margin-top:8px;background:var(--accent,#735c00);color:#fff;border:0;
...
@media (max-width:900px){
	...
	add_action( 'wp_head', 'luwipress_sapphire_smart_filters_print_styles', 99 );
}
