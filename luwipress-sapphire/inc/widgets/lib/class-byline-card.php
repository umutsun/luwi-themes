...
class LuwiPress_Sapphire_Widget_Byline_Card extends Widget_Base {

	public function get_name()        { return 'lwp-byline-card'; }
	public function get_title()       { return __( 'Byline Card', 'luwipress-sapphire' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Byline', 'luwipress-sapphire' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'default' => 'auto',
				'options' => [
					'auto'   => __( 'Auto (current post)', 'luwipress-sapphire' ),
					'manual' => __( 'Manual', 'luwipress-sapphire' ),
				],
			]
		);
		$this->add_control(
			'manual_avatar',
			[ 'label' => __( 'Avatar image', 'luwipress-sapphire' ), 'type' => Controls_Manager::MEDIA, 'default' => [ 'url' => '' ], 'condition' => [ 'source' => 'manual' ] ]
		);
		$this->add_control(
			'manual_name',
			[ 'label' => __( 'Author name', 'luwipress-sapphire' ), 'type' => Controls_Manager::TEXT, 'default' => 'Author', 'condition' => [ 'source' => 'manual' ] ]
		);
		$this->add_control(
			'manual_role',
			[ 'label' => __( 'Author role / byline sub', 'luwipress-sapphire' ), 'type' => Controls_Manager::TEXT, 'default' => 'Editor', 'condition' => [ 'source' => 'manual' ] ]
		);
		$this->add_control(
			'manual_date',
			[ 'label' => __( 'Date (free text)', 'luwipress-sapphire' ), 'type' => Controls_Manager::TEXT, 'default' => '', 'condition' => [ 'source' => 'manual' ] ]
		);

		$this->add_control(
			'show_read_time',
			[ 'label' => __( 'Show read time', 'luwipress-sapphire' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);
		...
		$this->add_control( 'name_color',   [ 'label' => __( 'Name color', 'luwipress-sapphire' ),   'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
			'selectors' => [ '{{WRAPPER}} .lwp-byline__name' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'meta_color',   [ 'label' => __( 'Meta color', 'luwipress-sapphire' ),   'type' => Controls_Manager::COLOR, 'default' => '#8b7f6a',
			'selectors' => [ '{{WRAPPER}} .lwp-byline__meta' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'avatar_gradient', [ 'label' => __( 'Fallback avatar gradient', 'luwipress-sapphire' ), 'type' => Controls_Manager::COLOR, 'default' => '#9A7B3A',
	...
	protected function render() {
		...
		if ( $source === 'auto' ) {
			...
			$name       = $user ? $user->display_name : __( 'Author', 'luwipress-sapphire' );
		...
		if ( $show_read && $read_min ) {
			$meta_parts[] = sprintf( _n( '%d min read', '%d min read', $read_min, 'luwipress-sapphire' ), $read_min );
		}
