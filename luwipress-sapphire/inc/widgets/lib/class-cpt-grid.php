...
class LuwiPress_Sapphire_Widget_CPT_Grid extends Widget_Base {

	public function get_name()          { return 'lwp-cpt-grid'; }
	public function get_title()         { return __( 'CPT Grid', 'luwipress-sapphire' ); }
	...
	private function post_type_options() {
		...
		if ( empty( $opts ) ) {
			$opts['lwp_vendor'] = __( 'Vendor', 'luwipress-sapphire' );
		}
		return $opts;
	}
	...
	protected function register_controls() {
		...
		$tax_options = [ '' => __( '— No taxonomy filter —', 'luwipress-sapphire' ) ];
		if ( class_exists( 'LuwiPress_CPT_Engine' ) ) {
		...
		$this->add_control(
			...
			[
				...
				'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'Pulls live from a LuwiPress CPT Engine type. Add or edit entries under LuwiPress → the matching CPT; this grid updates automatically.', 'luwipress-sapphire' ) . '</em>',
			]
		);
		...
		$this->add_control(
			'post_type',
			[
				'label'   => __( 'Content type', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'limit',
			[
				'label'   => __( 'Max entries', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'filter_taxonomy',
			[
				'label'     => __( 'Filter by taxonomy', 'luwipress-sapphire' ),
				'type'      => Controls_Manager::SELECT,
				'default'   => '',
				'options'   => $tax_options,
		...
		$this->add_control(
			'filter_term',
			[
				'label'       => __( 'Term slug', 'luwipress-sapphire' ),
				'description' => __( 'Slug of the term in the chosen taxonomy (e.g. luthiers). Leave empty to show all.', 'luwipress-sapphire' ),
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Comma-separated post IDs to show in this order. Leave empty to auto-query.', 'luwipress-sapphire' ),
				'type'        => Controls_Manager::TEXT,
			...
			$this->add_control(
				...
				[
					...
					'raw'  => '<small style="color:#8b7f6a;line-height:1.6;">'
						. esc_html__( 'Available field keys per type:', 'luwipress-sapphire' )
						. '<br>' . $field_hint . '</small>',
				]
			);
		}
		...
		$this->add_control(
			'meta_line_1',
			[
				'label'       => __( 'Sub-line 1 — meta key', 'luwipress-sapphire' ),
				'description' => __( 'Meta key shown under the title (e.g. _lwp_vendor_location). Leave empty to hide.', 'luwipress-sapphire' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		...
		$this->add_control(
			'meta_line_2',
			[
				'label'       => __( 'Sub-line 2 — meta key', 'luwipress-sapphire' ),
				'description' => __( 'Second meta key (e.g. _lwp_vendor_specialty). Leave empty to hide.', 'luwipress-sapphire' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'description'  => __( 'Adds a CTA below the grid linking to the type archive. Useful when the grid is capped.', 'luwipress-sapphire' ),
		...
		$this->add_control(
			...
			[
				...
				'type'        => Controls_Manager::TEXT,
				'default'     => __( 'View all →', 'luwipress-sapphire' ),
				'placeholder' => __( 'View all →', 'luwipress-sapphire' ),
	...
	protected function render() {
		...
		if ( '' === $pt || ! post_type_exists( $pt ) ) {
			echo '<div class="lwp-mgrid lwp-mgrid--placeholder"><em>'
				. esc_html__( '[CPT Grid] Pick a content type. None is registered — add one in LuwiPress → CPT types.', 'luwipress-sapphire' )
				. '</em></div>';
			return;
		}
		...
		if ( ! $q->have_posts() ) {
			...
			echo '<div class="lwp-mgrid lwp-mgrid--empty"><em>'
				...
				. esc_html( sprintf( __( '[CPT Grid] No %s published yet.', 'luwipress-sapphire' ), $label ) )
				. '</em></div>';
			wp_reset_postdata();
			return;
		}
		...
		if ( ( $s['view_all_link'] ?? '' ) === 'yes' ) {
			...
			if ( $archive_url ) {
				...
				if ( '' === $label ) {
					$label = __( 'View all →', 'luwipress-sapphire' );
				}
				?>
