...
class LuwiPress_Sapphire_Widget_Featured_Post extends Widget_Base {

	public function get_name()        { return 'lwp-featured-post'; }
	public function get_title()       { return __( 'Featured Post', 'luwipress-sapphire' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Source', 'luwipress-sapphire' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'default' => 'auto-sticky',
				'options' => [
					'auto-sticky' => __( 'Auto: first sticky post', 'luwipress-sapphire' ),
					'auto-latest' => __( 'Auto: latest post', 'luwipress-sapphire' ),
					'manual'      => __( 'Manual: pick post by ID', 'luwipress-sapphire' ),
				],
			]
		);
		$this->add_control(
			'manual_post_id',
			[
				'label'     => __( 'Post ID', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'post_type',
			[
				'label'   => __( 'Post type', 'luwipress-sapphire' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'post',
		...
		$this->add_control(
			...
			[
				...
				'type'    => Controls_Manager::SELECT,
				'default' => 'left',
				'options' => [ 'left' => __( 'Image left', 'luwipress-sapphire' ), 'right' => __( 'Image right', 'luwipress-sapphire' ) ],
			]
		);
		$this->add_control(
			'show_excerpt',
			[ 'label' => __( 'Show excerpt', 'luwipress-sapphire' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);
		$this->add_control(
			'show_meta',
			[ 'label' => __( 'Show meta (category · read time)', 'luwipress-sapphire' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);
		$this->add_control(
			'cta_label',
			[ 'label' => __( 'CTA label', 'luwipress-sapphire' ), 'type' => Controls_Manager::TEXT, 'default' => __( 'Read on →', 'luwipress-sapphire' ) ]
		);
		...
		$this->add_control( 'title_color', [ 'label' => __( 'Title color', 'luwipress-sapphire' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
			'selectors' => [ '{{WRAPPER}} .lwp-fp__title' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'eyebrow_color', [ 'label' => __( 'Eyebrow color', 'luwipress-sapphire' ), 'type' => Controls_Manager::COLOR, 'default' => '#9A7B3A',
			'selectors' => [ '{{WRAPPER}} .lwp-fp__eyebrow' => 'color: {{VALUE}};' ] ] );
		$this->add_control( 'cta_color', [ 'label' => __( 'CTA color', 'luwipress-sapphire' ), 'type' => Controls_Manager::COLOR, 'default' => '#1A1612',
	...
	protected function render() {
		...
		if ( ! $post ) {
			echo '<div class="lwp-fp lwp-fp--empty"><p>' . esc_html__( 'No featured post found.', 'luwipress-sapphire' ) . '</p></div>';
			return;
		}
			...
			<div class="lwp-fp__body">
				<?php if ( $show_meta && ( $cat_label || $read_min ) ) : ?>
					<span class="lwp-fp__eyebrow">
						...
						<?php if ( $read_min ) : ?><?php printf( esc_html( _n( '%d min read', '%d min read', $read_min, 'luwipress-sapphire' ) ), $read_min ); ?><?php endif; ?>
					</span>
				<?php endif; ?>
