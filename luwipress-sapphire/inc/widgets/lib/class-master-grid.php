...
class LuwiPress_Sapphire_Widget_Master_Grid extends Widget_Base {

	public function get_name()        { return 'lwp-master-grid'; }
	public function get_title()       { return __( 'Master Grid', 'luwipress-sapphire' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_query', [ 'label' => __( 'Source', 'luwipress-sapphire' ) ] );

		$this->add_control(
			'info',
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw'  => '<em style="color:#8b7f6a;">' . esc_html__( 'Pulls live from the Vendors CPT (LuwiPress → Vendors). Add or edit profiles there; this grid updates automatically.', 'luwipress-sapphire' ) . '</em>',
			]
		);
		...
		$this->add_control(
			'limit',
			[
				'label'   => __( 'Max profiles', 'luwipress-sapphire' ),
		...
		$group_options = [ '' => __( '— All vendors —', 'luwipress-sapphire' ) ];
		...
		$this->add_control(
			'vendor_group',
			[
				'label'       => __( 'Vendor group', 'luwipress-sapphire' ),
				'description' => __( 'Filter to one vendor group (Luthiers, Team, etc.). Manage groups under LuwiPress → Vendors → Vendor Groups.', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'include_ids',
			[
				'label'       => __( 'Curated IDs (optional)', 'luwipress-sapphire' ),
				'description' => __( 'Comma-separated CPT IDs to show in this order. Leave empty to auto-query (menu_order ASC).', 'luwipress-sapphire' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => 'menu_order',
				'options' => [
					'menu_order' => __( 'Manual (menu order)', 'luwipress-sapphire' ),
					'date'       => __( 'Date published', 'luwipress-sapphire' ),
					'title'      => __( 'Name (A→Z)', 'luwipress-sapphire' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => 'dark',
				'options' => [
					'dark'  => __( 'Dark (for #1A1612 section bg)', 'luwipress-sapphire' ),
					'light' => __( 'Light (for cream section bg)', 'luwipress-sapphire' ),
				],
			]
		);
		...
		$this->add_control(
			'show_location',
			[ 'label' => __( 'Show location', 'luwipress-sapphire' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);

		$this->add_control(
			'show_specialty',
			[ 'label' => __( 'Show specialty', 'luwipress-sapphire' ), 'type' => Controls_Manager::SWITCHER, 'return_value' => 'yes', 'default' => 'yes' ]
		);

		$this->add_control(
			'show_count',
			[
				'label'       => __( 'Show product count', 'luwipress-sapphire' ),
				'description' => __( 'Counts WC products attributed to this vendor via _lwp_vendor_ids meta.', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'view_all_link',
			[
				'label'        => __( 'Show "View all" link', 'luwipress-sapphire' ),
				'description'  => __( 'Adds a CTA below the grid linking to the full Vendors archive (e.g. /luthiers/). Useful when the grid is capped at 4–8 and visitors need a path to the rest.', 'luwipress-sapphire' ),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'default'      => 'yes',
			]
		...
		$this->add_control(
			'view_all_label',
			[
				'label'       => __( '"View all" label', 'luwipress-sapphire' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => __( 'View all vendors →', 'luwipress-sapphire' ),
				'placeholder' => __( 'View all vendors →', 'luwipress-sapphire' ),
	...
	protected function render() {
		...
		if ( ! post_type_exists( 'lwp_vendor' ) ) {
			echo '<div class="lwp-mgrid lwp-mgrid--placeholder"><em>' . esc_html__( '[Master Grid] LuwiPress Vendors module is inactive — enable it in LuwiPress → Vendors.', 'luwipress-sapphire' ) . '</em></div>';
			return;
		}
		...
		$q = new WP_Query( $args );
		if ( ! $q->have_posts() ) {
			echo '<div class="lwp-mgrid lwp-mgrid--empty"><em>' . esc_html__( '[Master Grid] No profiles yet. Add them in LuwiPress → Vendors.', 'luwipress-sapphire' ) . '</em></div>';
			return;
		}
				...
				if ( $show_count ) {
					...
					if ( $cnt > 0 ) {
						$count_label = sprintf(
							...
							_n( '%d work', '%d works', $cnt, 'luwipress-sapphire' ),
							$cnt
						);
					}
				}
		...
		if ( ( $s['view_all_link'] ?? 'yes' ) === 'yes' ) {
			...
			if ( $archive_url ) {
				...
				if ( $label === '' ) {
					$label = __( 'View all vendors →', 'luwipress-sapphire' );
				}
				?>
