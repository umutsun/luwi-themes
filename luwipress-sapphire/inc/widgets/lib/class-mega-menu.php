<?php
/**
 * Widget: LuwiPress Sapphire Mega Menu.
 *
 ...
 * (custom field `_lwp_sapphire_mega_featured` saved per top item).
 *
...
class LuwiPress_Sapphire_Widget_Mega_Menu extends Widget_Base {

	public function get_name()        { return 'lwp-mega-menu'; }
	public function get_title()       { return __( 'Mega Menu', 'luwipress-sapphire' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Build the menu in Appearance → Menus, then pick it here. The widget auto-detects which top-level items deserve a mega panel.', 'luwipress-sapphire' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'min'         => 2,
				'max'         => 12,
				'description' => __( 'A top-level item becomes a mega panel when it has at least this many children, or when any child has its own children.', 'luwipress-sapphire' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => 'yes',
				'description' => __( 'When a menu item points at a product category, show that category\'s product count next to the link.', 'luwipress-sapphire' ),
			]
		);
		...
		$this->add_control(
			'top_color',
			[
				'label'     => __( 'Top item color', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'top_hover_color',
			[
				'label'     => __( 'Top item hover color', 'luwipress-sapphire' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#735c00',
		...
		$this->add_control(
			'panel_bg',
			[
				'label'     => __( 'Panel background', 'luwipress-sapphire' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#ffffff',
	...
	private function get_menu_options() {
		$out = [ '' => __( '— Pick a menu —', 'luwipress-sapphire' ) ];
	...
	protected function render() {
		...
		// Fall back to Customizer (Görünüm → Customize → LuwiPress Sapphire →
		...
		if ( ! $menu_id ) {
			$menu_id = (int) get_theme_mod( 'luwipress_sapphire_mega_menu_id', 0 );
		}
		if ( ! $menu_id ) {
			$this->placeholder( __( 'Pick a menu in the panel → (or set the global default in Customize → LuwiPress Sapphire → Mega menu)', 'luwipress-sapphire' ) );
			return;
		}

		$items = wp_get_nav_menu_items( $menu_id );
		if ( empty( $items ) ) {
			$this->placeholder( __( 'This menu has no items yet.', 'luwipress-sapphire' ) );
			return;
		}

		$threshold = isset( $s['mega_threshold'] ) && (int) $s['mega_threshold'] > 0
			? max( 2, (int) $s['mega_threshold'] )
			: max( 2, (int) get_theme_mod( 'luwipress_sapphire_mega_threshold', 4 ) );

		$cols_pref = ! empty( $s['mega_columns'] )
			? $s['mega_columns']
			: (string) get_theme_mod( 'luwipress_sapphire_mega_columns', 'auto' );

		$show_counts = isset( $s['show_counts'] )
			? ( $s['show_counts'] === 'yes' )
			: (bool) get_theme_mod( 'luwipress_sapphire_mega_show_counts', true );
	...
	public static function render_navigation_html( $menu_id, $opts = [] ) {
		...
		// from Customize → LuwiPress Sapphire → Mega menu.
		if ( (bool) get_theme_mod( 'luwipress_sapphire_mega_auto_inject_blog', true ) ) {
		...
		echo '<nav class="lwp-mm" aria-label="' . esc_attr__( 'Primary', 'luwipress-sapphire' ) . '">';
	...
	public static function maybe_inject_blog_categories( $node ) {
		...
		if ( ! $is_blog_hub && ! empty( $node['url'] ) ) {
			...
			$last     = end( $segments );
			if ( $last !== false && $last !== '' ) {
				$slugs = function_exists( 'luwipress_sapphire_get_blog_page_slugs' )
					? luwipress_sapphire_get_blog_page_slugs()
		...
		$cats = (array) apply_filters( 'luwipress_sapphire_blog_menu_categories', $cats, $node );
	...
	public static function render_mega_panel( $node, $cols_pref, $show_counts, $head_meta = [] ) {
		...
		if ( $top_url !== '' && ( $top_count !== '' || $top_label !== '' ) ) {
			...
			if ( $top_count !== '' ) {
				printf(
					...
					esc_attr( sprintf( /* translators: %s: count */ _n( '%s item in this collection', '%s items in this collection', (int) $top_count, 'luwipress-sapphire' ), $top_count ) ),
					esc_html( $top_count )
				);
			}
			printf(
				...
				esc_url( $top_url ),
				esc_html( $top_label ),
				esc_html__( 'View all →', 'luwipress-sapphire' )
			);
			echo '</div>';
		}
		...
		//   1. Per-menu-item manual override (`_lwp_sapphire_mega_featured`)
		...
		$featured_id = (int) get_post_meta( $node['id'], '_lwp_sapphire_mega_featured', true );
		if ( ! $featured_id ) {
	...
	public static function render_featured_slot( $product_id ) {
		...
		$eyebrow    = $is_product ? __( 'Featured', 'luwipress-sapphire' ) : __( 'Read', 'luwipress-sapphire' );
		?>
		<a class="lwp-mm-feat" href="<?php echo esc_url( get_permalink( $product_id ) ); ?>">
			...
			<div class="lwp-mm-feat-meta">
				...
				<span class="lwp-mm-feat-link"><?php esc_html_e( 'Open →', 'luwipress-sapphire' ); ?></span>
			</div>
		</a>
		<?php
	}
	 ...
	 * legacy function (`luwipress_sapphire_get_hub_redirects`) early-returns
	...
	private static function get_slug_resolver_map() {
		...
		} elseif ( function_exists( 'luwipress_sapphire_get_hub_redirects' ) ) {
			$cache = (array) luwipress_sapphire_get_hub_redirects();
		} else {
			$cache = array();
		}
		return $cache;
	...
	private static function registry_featured_for_top( $node ) {
		...
		$ids = lwp_sapphire_get_featured_ids( [ 'number' => 30 ] );
		if ( empty( $ids ) ) {
			return 0;
		}
