...
class LuwiPress_Sapphire_Widget_Story_Split extends Widget_Base {

	public function get_name()        { return 'lwp-story-split'; }
	public function get_title()       { return __( 'Story Split', 'luwipress-sapphire' ); }
	...
	protected function register_controls() {
		...
		$this->add_control(
			'image_side',
			[
				'label'   => __( 'Image side', 'luwipress-sapphire' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'left',
				'options' => [
					'left'  => __( 'Image on left', 'luwipress-sapphire' ),
					'right' => __( 'Image on right', 'luwipress-sapphire' ),
				],
			]
		);
		...
		/* Image side */
		$this->start_controls_section( 'section_image', [ 'label' => __( 'Image tile', 'luwipress-sapphire' ) ] );
		...
		$this->add_control(
			'image_alt_grad_from',
			[
				'label'     => __( 'Gradient start (when no image)', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'image_alt_grad_to',
			[
				'label'     => __( 'Gradient end (when no image)', 'luwipress-sapphire' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#7a5a2c',
			]
		);
		$this->add_control(
			'card_eyebrow',
			[
				'label'   => __( 'Floating card — eyebrow', 'luwipress-sapphire' ),
		...
		$this->add_control(
			'card_title',
			[
				'label'   => __( 'Floating card — title', 'luwipress-sapphire' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		$this->add_control(
			'card_sub',
			[
				'label'   => __( 'Floating card — sub line', 'luwipress-sapphire' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '',
			]
		);
		...
		/* Story side */
		$this->start_controls_section( 'section_story', [ 'label' => __( 'Story', 'luwipress-sapphire' ) ] );
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Use Enter for multi-line. Wrap with [em]…[/em] for italic gold accent.', 'luwipress-sapphire' ),
		...
		$rep = new Repeater();
		$rep->add_control( 'title', [ 'label' => __( 'Title', 'luwipress-sapphire' ), 'type' => Controls_Manager::TEXT ] );
		$rep->add_control( 'body',  [ 'label' => __( 'Body', 'luwipress-sapphire' ),  'type' => Controls_Manager::TEXTAREA, 'rows' => 2 ] );

		$this->add_control(
			'bullets',
			[
				'label'       => __( 'Numbered bullets', 'luwipress-sapphire' ),
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $rep->get_controls(),
				'title_field' => '{{{ title || "Bullet" }}}',
				'default'     => [
					[ 'title' => __( 'Quality first.',          'luwipress-sapphire' ), 'body' => __( 'A short paragraph that explains your first key value.', 'luwipress-sapphire' ) ],
					[ 'title' => __( 'Direct from us to you.', 'luwipress-sapphire' ), 'body' => __( 'Second value proposition — replace this copy from the widget editor.', 'luwipress-sapphire' ) ],
					[ 'title' => __( 'Worldwide shipping.',     'luwipress-sapphire' ), 'body' => __( 'Third value — what makes your delivery / fulfillment story strong.', 'luwipress-sapphire' ) ],
					[ 'title' => __( 'After-sale support.',     'luwipress-sapphire' ), 'body' => __( 'Fourth value — what happens after the customer buys.', 'luwipress-sapphire' ) ],
				],
			]
		);
