 ...
 * `Appearance → LuwiPress Sapphire` admin page, and shows a one-time activation
...
if ( ! class_exists( 'LuwiPress_Sapphire_Wizard' ) ) :

class LuwiPress_Sapphire_Wizard {

	const DISMISSED_OPTION = 'luwipress_sapphire_wizard_dismissed';
	const COMPLETED_OPTION = 'luwipress_sapphire_wizard_completed';
	const NONCE            = 'luwipress_sapphire_wizard';
	const SLUG             = 'luwipress-sapphire-wizard';
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_luwipress_sapphire_wizard_step', [ $this, 'handle_ajax_step' ] );
		add_action( 'wp_ajax_luwipress_sapphire_wizard_dismiss', [ $this, 'handle_ajax_dismiss' ] );
	}
	...
	public function register_admin_page() {
		add_submenu_page(
			'',                                    // hidden — no parent
			__( 'LuwiPress Sapphire setup', 'luwipress-sapphire' ),
		...
		add_management_page(
			__( 'LuwiPress Sapphire setup (re-run)', 'luwipress-sapphire' ),
			__( 'LuwiPress Sapphire', 'luwipress-sapphire' ),
			self::CAPABILITY,
			self::SLUG,
			[ $this, 'render_page' ]
	...
	public function render_activation_notice() {
		...
		?>
		<div class="notice notice-info luwipress-sapphire-notice is-dismissible" data-lwp-wizard-notice>
			<p style="font-size:14px">
				<strong style="font-family:'Playfair Display',Georgia,serif;font-size:18px;">
					<?php esc_html_e( 'Welcome to LuwiPress Sapphire.', 'luwipress-sapphire' ); ?>
				</strong>
				<br>
				<?php echo esc_html( $summary ); ?>
				<?php esc_html_e( 'Run the setup wizard — we\'ll detect what you have and adapt the theme to it (no data is overwritten).', 'luwipress-sapphire' ); ?>
			</p>
			<p>
				<a href="<?php echo $url; ?>" class="button button-primary">
					<?php esc_html_e( 'Start setup wizard', 'luwipress-sapphire' ); ?>
				</a>
				<a href="#" class="button button-secondary" data-lwp-wizard-dismiss>
					<?php esc_html_e( 'Skip for now', 'luwipress-sapphire' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	public function enqueue_assets( $hook ) {
		...
		wp_enqueue_style(
			'luwipress-sapphire-wizard-notice',
			LUWIPRESS_SAPPHIRE_URI . '/assets/wizard-css/notice.css',
			[],
			LUWIPRESS_SAPPHIRE_VERSION
		);
		wp_enqueue_script(
			'luwipress-sapphire-wizard-notice',
			LUWIPRESS_SAPPHIRE_URI . '/assets/wizard-js/notice.js',
			[ 'jquery' ],
			LUWIPRESS_SAPPHIRE_VERSION,
			true
		);
		wp_localize_script( 'luwipress-sapphire-wizard-notice', 'LWP_SAPPHIRE_WIZARD', [
		...
		wp_enqueue_style(
			'luwipress-sapphire-wizard',
			LUWIPRESS_SAPPHIRE_URI . '/assets/wizard-css/wizard.css',
			[ 'luwipress-sapphire-wizard-notice' ],
			LUWIPRESS_SAPPHIRE_VERSION
		);
		wp_enqueue_script(
			'luwipress-sapphire-wizard',
			LUWIPRESS_SAPPHIRE_URI . '/assets/wizard-js/wizard.js',
			[ 'jquery', 'luwipress-sapphire-wizard-notice' ],
			LUWIPRESS_SAPPHIRE_VERSION,
			true
		);
		wp_localize_script( 'luwipress-sapphire-wizard', 'LWP_SAPPHIRE_WIZARD_DATA', [
			'restUrl' => esc_url_raw( rest_url() ),
			'i18n'    => [
				'detecting'  => __( 'Scanning your site…', 'luwipress-sapphire' ),
				'mapping'    => __( 'Mapping content to templates…', 'luwipress-sapphire' ),
				'importing'  => __( 'Importing templates…', 'luwipress-sapphire' ),
				'finishing'  => __( 'Almost there…', 'luwipress-sapphire' ),
				'completed'  => __( 'All done!', 'luwipress-sapphire' ),
				'errored'    => __( 'Something went wrong. Check the WP debug log.', 'luwipress-sapphire' ),
			],
		] );
	}
	...
	public function handle_ajax_step() {
		...
		switch ( $step ) {
			case 'detect':
				$d = new LuwiPress_Sapphire_Detector();
				wp_send_json_success( [
					'snapshot' => $d->snapshot(),
			...
			case 'map':
				...
				$d = new LuwiPress_Sapphire_Detector();
				$snap = $d->snapshot();
				$mapper = new LuwiPress_Sapphire_Mapper();
			...
			case 'ai_status':
				wp_send_json_success( [
					'available' => LuwiPress_Sapphire_AI_Content::is_luwipress_available(),
					'enabled'   => LuwiPress_Sapphire_AI_Content::is_enabled(),
					'slots'     => LuwiPress_Sapphire_AI_Content::slots(),
					'meta'      => get_option( LuwiPress_Sapphire_AI_Content::META_OPTION, [] ),
				] );

			case 'ai_toggle':
				$enable = ! empty( $payload['enable'] );
				update_option( LuwiPress_Sapphire_AI_Content::ENABLED_OPTION, $enable ? 1 : 0 );
				if ( ! $enable ) {
					LuwiPress_Sapphire_AI_Content::flush_cache();
				}
				wp_send_json_success( [ 'enabled' => $enable ] );

			case 'ai_generate':
				if ( ! LuwiPress_Sapphire_AI_Content::is_luwipress_available() ) {
					wp_send_json_error( [ 'message' => 'LuwiPress AI engine not installed.' ] );
				}
				update_option( LuwiPress_Sapphire_AI_Content::ENABLED_OPTION, 1 );
				$results = [];
				foreach ( LuwiPress_Sapphire_AI_Content::slots() as $slot => $cfg ) {
					$text = LuwiPress_Sapphire_AI_Content::resolve( $slot );
					$results[ $slot ] = [
						'label' => $cfg['label'],
						'text'  => $text,
			...
			case 'apply':
				...
				$importer = new LuwiPress_Sapphire_Importer();
...
LuwiPress_Sapphire_Wizard::instance();
