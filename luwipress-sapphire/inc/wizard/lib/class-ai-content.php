 ...
 *
 * Cache key: `lwp_sapphire_ai_<slot>_<context_hash>` so the operator
 * regenerates simply by clearing transients (which the wizard offers).
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

class LuwiPress_Sapphire_AI_Content {

	const CACHE_TTL = WEEK_IN_SECONDS;
	const ENABLED_OPTION = 'luwipress_sapphire_ai_enabled';
	const PROVIDER_OPTION = 'luwipress_sapphire_ai_provider';
	const META_OPTION = 'luwipress_sapphire_ai_meta';
	...
	public static function slots() {
		return [
			'hero_lead' => [
				'label'     => __( 'Hero — lead paragraph', 'luwipress-sapphire' ),
				'max_words' => 35,
			...
			'about_story' => [
				'label'     => __( 'About — short brand story', 'luwipress-sapphire' ),
				'max_words' => 90,
			...
			'master_bio' => [
				'label'     => __( 'Master profile — short bio', 'luwipress-sapphire' ),
				'max_words' => 50,
			...
			'faq_intro' => [
				'label'     => __( 'Contact — FAQ intro', 'luwipress-sapphire' ),
				'max_words' => 25,
			...
			'shipping_blurb' => [
				'label'     => __( 'Info-bar — shipping note', 'luwipress-sapphire' ),
				'max_words' => 12,
	...
	private static function site_context() {
		...
		$sector_blob = ! empty( $sectors )
			? implode( ', ', $sectors )
			: ( $tagline ?: __( 'hand-crafted goods', 'luwipress-sapphire' ) );
	...
	private static function cache_key( $slot, $context = [] ) {
		...
		return 'lwp_sapphire_ai_' . $slot . '_' . substr( md5( $blob ), 0, 12 );
	}
