...
class LuwiPress_Sapphire_Importer {

	/**
	 * @var LuwiPress_Sapphire_Content_Compiler
	 */
	private $compiler;
	...
	public function apply( $path, $brand = [], $opts = [] ) {
		...
		$this->compiler = new LuwiPress_Sapphire_Content_Compiler( $this->snapshot );
		...
		// latest compiled content, so old `_lwp_sapphire_managed` templates from
		// previous runs become orphans. Trash them now to avoid the user seeing
		// 13+ duplicate "LuwiPress Sapphire — Header" entries in Templates → Library.
		...
		if ( function_exists( 'luwipress_sapphire_sync_kit' ) ) {
			$kit_result = luwipress_sapphire_sync_kit();
			$log['actions'][] = [
				...
				'detail' => is_wp_error( $kit_result )
					...
					: [ 'kit_id' => function_exists( 'luwipress_sapphire_get_active_kit_id' ) ? luwipress_sapphire_get_active_kit_id() : null ],
			];
		}
		...
		if ( ! empty( $opts['resolve_slug_conflicts'] ) ) {
			$conflicts = isset( $this->snapshot['slug_conflicts'] ) && is_array( $this->snapshot['slug_conflicts'] )
				...
				: [];
			set_theme_mod( 'luwipress_sapphire_resolve_slug_conflicts', true );
			$log['actions'][] = [
		...
		update_option( 'luwipress_sapphire_wizard_log', $log, false );
	 ...
	 * `_lwp_sapphire_managed = 1`. Run at the start of every Apply so the
	 * Templates → Library doesn't accumulate one extra "LuwiPress Sapphire —
	 * Header" per wizard run.
	 *
	 * Skip the auto-applied homepage container (id 35754 et al) — anything
	 * with `_lwp_sapphire_keep` meta is preserved.
	 */
	private function cleanup_managed_templates( &$log ) {
		...
		foreach ( $query->posts as $tid ) {
			if ( get_post_meta( $tid, '_lwp_sapphire_keep', true ) ) continue;
	...
	private function apply_brand( $brand, &$log ) {
		...
		foreach ( $mods as $key => $val ) {
			if ( $val !== null && $val !== '' ) {
				set_theme_mod( 'luwipress_sapphire_' . $key, sanitize_hex_color( $val ) ?: $val );
		...
		if ( ! empty( $brand['phone'] ) ) {
			update_option( 'luwipress_sapphire_phone', sanitize_text_field( $brand['phone'] ) );
			$log['options'][] = 'luwipress_sapphire_phone';
		}
		if ( ! empty( $brand['email'] ) && is_email( $brand['email'] ) ) {
			// Don't overwrite admin_email — store in our namespace instead.
			update_option( 'luwipress_sapphire_contact_email', sanitize_email( $brand['email'] ) );
			$log['options'][] = 'luwipress_sapphire_contact_email';
		}
	}
	...
	private function kit_dir() {
		return apply_filters( 'luwipress_sapphire_kit_path', LUWIPRESS_SAPPHIRE_DIR . '/elementor-kit/' );
	}
	...
	private function import_template( $action ) {
		...
		update_post_meta( $post_id, '_lwp_sapphire_managed',        1 );
	 ...
	 *
	 * Single Product templates land in `luwipress_sapphire_pdp_template_id`, which
	...
	private function wire_template_to_theme_mod( $post_id, $type, $condition_type ) {
		if ( $type === 'single-product' || $condition_type === 'product' ) {
			set_theme_mod( 'luwipress_sapphire_pdp_template_id', (int) $post_id );
		} elseif ( $type === 'archive' || $type === 'product-archive' ) {
			set_theme_mod( 'luwipress_sapphire_archive_template_id', (int) $post_id );
		}
	}
	...
	private function import_demo_xml( $action ) {
		$file = LUWIPRESS_SAPPHIRE_DIR . '/' . ltrim( $action['file'] ?? '', '/' );
		if ( ! file_exists( $file ) ) {
		...
		if ( ! file_exists( $wp_importer_path ) ) {
			return [
				...
				'hint'    => __( 'Install the "WordPress Importer" plugin from wp-admin → Tools → Import → WordPress.', 'luwipress-sapphire' ),
			];
		}
	...
	private function mark_master( $action ) {
		...
		update_term_meta( $term->term_id, '_lwp_sapphire_featured_master', 1 );
	...
	private function process_page( $slug, $page ) {
		...
		if ( ( $page['action'] ?? '' ) === 'create_and_import' || ! empty( $page['set_as_home'] ) ) {
			...
			$meta = [ '_lwp_sapphire_managed' => 1 ];
			if ( ! empty( $page['parallel_to'] ) ) {
				$meta['_lwp_sapphire_parallel_to'] = (int) $page['parallel_to'];
			}
			if ( ! empty( $page['compiler_context']['master_slug'] ) ) {
				$meta['_lwp_sapphire_master_slug'] = sanitize_title( $page['compiler_context']['master_slug'] );
			}
	...
	private function apply_template_to_page( $page_id, $kit_file, $context = [] ) {
		...
		update_post_meta( $page_id, '_lwp_sapphire_managed',        1 );
