...
$lwp_singular = ( $lwp_def && ! empty( $lwp_def['labels']['singular'] ) )
	...
	: ( $lwp_pt_obj ? (string) $lwp_pt_obj->labels->singular_name : __( 'Item', 'luwipress-sapphire' ) );
$lwp_plural   = ( $lwp_def && ! empty( $lwp_def['labels']['plural'] ) )
	? (string) $lwp_def['labels']['plural']
	: ( $lwp_pt_obj ? (string) $lwp_pt_obj->labels->name : __( 'Items', 'luwipress-sapphire' ) );
		...
		if ( 'url' === $type || 'sameAs' === $sprop ) {
			if ( preg_match( '#^https?://#i', trim( (string) $val ) ) ) {
				$socials[] = array(
					'label' => $label,
					'url'   => (string) $val,
					'icon'  => function_exists( 'lwp_sapphire_cpt_social_icon' ) ? lwp_sapphire_cpt_social_icon( $label ) : 'eicon-globe',
				);
			}
				...
				<section class="lwp-person__work">
					<h2 class="lwp-person__work-heading">
						...
						echo esc_html( sprintf(
							...
							__( 'Related to this %s', 'luwipress-sapphire' ),
